...
- triggers, Scheduler detection/arbitration, watches, automations, circuit policy, Schedule/Event/WorldCondition reconciliation semantics, cold-start pacing, or non-interactive execution — File 33 owns those; this file owns workers, timers, supervision, boot ordering, and shutdown, invokes File 33's startup coordinator, and places its workers in the graph
...
- the `Shell`, the presentation-context registry, presentation composition, the rendering of any runtime surface, or accessibility presentation — Files 37 and 38 own those; this file owns the frontend-bridge transport and the data contracts they render
- the installer, update channels and distribution, the sidecar binary inventory, platform integration, the platform crash-handler registration, the native-messaging-host registration, or release artifacts — the Packaging, Platform, and Distribution spec (File 43) owns those; this file owns the *running* lifecycle, the apply-on-restart and relaunch mechanics, and the managed-service lifecycle contract sidecars are run under
...
Resolution rule: this file owns runtime *mechanics* and application *lifecycle*; it re-owns no policy, business logic, or consequential-fact store. The run model, child runs, cancellation, budgets, and orphan-reconciliation policy stay File 04's; the ledger, bus, hooks, events, and replay stay File 10's; the settings cascade and bootstrap-versus-runtime classification stay File 15's; provider health, retry, and credential pool stay File 17's; storage engine, transactions, projections, migration, garbage collection, and reconstruction internals stay File 20's; vault, trust, egress, and audit cryptography stay File 22's; sandbox, process spawning, confinement, killability, and elevated helper stay File 23's; triggers, Scheduler detection/arbitration, watches, automations, circuit policy, Schedule/Event/WorldCondition reconciliation semantics, cold-start pacing, and source-specific automation-worker restart hooks stay File 33's; connectors, session, transport protocol, and reconnect policy stay File 36's; observability projections and health contract stay File 41's; UI rendering stays Files 37 and 38's; installer, update channels, sidecar inventory, crash/native-messaging registration, and release artifacts stay File 43's. This file introduces the `Runtime`, `BackgroundWorker` and supervision, `WorkQueue`, `RuntimeTimer`, `ServiceGraph`, transports, `BootstrapConfig`, boot/shutdown contract, intervention-cancellation realization, and operational-health remediation.
...
- **In-place update versus apply-on-restart with rollback.** Mutating a running installation risks a half-applied update and a corrupt run. Resolved toward apply-on-restart: File 43 stages the update, this file sequences the relaunch, and File 20's last-known-good marker and self-check rollback (File 20 §12.3) guard a bad update.
...
Files 04, 10, 17, 20, 23, 33, and 36 each declare a startup phase, a background worker, a transport, a connection pool, a reaper, or a supervision need and defer the mechanics to this file. File 04 needs a process that hosts the run loop and a token that cancels it; File 10 needs the bus delivered over a transport, the lifecycle events emitted, and a startup and shutdown sequence; File 17 needs a worker and connection-pool substrate and provider warmers; File 20 needs the broader application lifecycle its storage phases sit within; File 23 needs the application lifecycle around its process startup and shutdown participation and the kill-and-reap invocation; File 33 needs the worker-scheduling primitives and the single-armed-timer the scheduler runs on and its placement in the boot graph; File 36 needs the transport implementation, the connection pool, and the reaper. This file is the single place those mechanics become concrete, so that no later spec invents a parallel process model, a private worker loop, an ungoverned transport, a second scheduler runtime, or an unsupervised daemon.
...
- The `Runtime` owns mechanics, not policy or business logic. Business logic stays in the owning service (the service-layer-ownership rule, File 01 §7.7); run and capability execution retry policy stays in File 04, provider transport retry stays in File 17, scheduler delivery retry stays in File 33, and connector transport retry stays in File 36. The `Runtime` executes supervision, worker restart mechanics, timers, and queues as substrate.
...
`Runtime` supersedes any earlier vocabulary that named the same substrate: "the app", "the host", "the main process", "the backend", "the kernel", "the daemon", "the tokio runtime", "the supervisor", "the supervisor tree". `BackgroundWorker` supersedes "background task", "daemon thread", "background goroutine", "tokio task", "poller", "sweeper", "maintenance loop", "worker", "sidecar loop". `WorkQueue` supersedes "job queue", "task queue", "dispatch queue", "command queue", "submission queue", "concurrency-control queue", "mailbox". `RuntimeTimer` supersedes "timer", "ticker", "scheduled callback", "delay", "deadline". `ServiceGraph` supersedes "service container", "AppState", "dependency-injection container", "service registry", "module graph". `Transport` supersedes "IPC channel", "event channel", "bridge", "Tauri channel", "WebSocket channel", "Unix socket", "named pipe", "broadcast channel", "wire". These are the canonical typed shapes the rest of this file uses; earlier names from source material map into them.
...
File 33 owns triggers, Scheduler detection/arbitration, watches, automations, circuit coordination, Schedule/Event/WorldCondition reconciliation semantics, cold-start pacing, and non-interactive posture. This file owns runtime mechanics and ordering: it invokes `AutomationStartupCoordinator` during boot recovery, then spawns the Scheduler/Event/WorldCondition workers as `BackgroundWorker`s; supplies timers, queues, supervision, and shutdown; and invokes File-33 restart hooks on those workers. It owns no trigger detection, watch evaluation, delivery-class decision, circuit policy, or automation semantics.
...
- Process reuse follows the ownership tree; it does not waive it (`process.groups`, File 23 §6.3). A process owned by a run or child run stays in that run's `ProcessGroup` and dies with that ownership tree; it is never retained to serve another run. Cross-run reuse is a registered managed service with a non-run owner and `ProcessGroup`, one confinement fixed for the process's lifetime — isolation tier, filesystem policy, network policy, and `ResourceLimits` — and it accepts work from more than one run only where the owning service provides per-request cancellation, request-state partitioning or reset, and failure containment. A unit that requires narrower confinement, process-fresh state, or process death as its categorical cancellation is spawned for that unit rather than served from a reused process.
- A reusable managed service retains no request-scoped mutable state across work units unless that state is part of its declared contract. Its start condition accounts for the steady resident memory, processor, and process-count cost of remaining alive, bounded by its `SandboxProfile` and `ResourceLimits` (`sandbox.resource-limits`, File 23 §9); this file adds no parallel resource budget. Reuse enlarges the set of simultaneous dependents one failure reaches, and that blast radius is part of the lifecycle declaration rather than an untaxed benefit of reuse.
...
- The sidecar binary inventory, packaging, and distribution are File 43's; the start-health-supervise-reap lifecycle is this file's.

### 4.6 Boundary

This section owns the process and inter-process topology, single-instance enforcement, and the managed-service lifecycle contract. File 23 owns the `ManagedProcess`, the `ProcessGroup`, and the kill-and-reap; File 20 owns the data root and the lock-file placement; File 36 owns the native-messaging session protocol; File 21 owns cross-device coordination; File 43 owns the sidecar binary inventory. The frontend-bridge transport is §10.
...
The state is event-first: a heartbeat returns the worker to `Healthy`; a typed failure transitions to `Failed`; a watchdog deadline transitions to `Stalled`; the supervisor's restart and circuit transitions are recorded. This reuses `ProviderHealth` (File 17 §12) and connector `ConnectionState` (File 36 §7.1) rather than inventing a third operational-liveness vocabulary. That rule does not constrain File 33's `CircuitProjection`, which derives task-failure and automation-enablement policy and is neither connection nor worker liveness.

### 6.3 Worker Registration Catalogue

The `Runtime` hosts registered `BackgroundWorker` declarations, each owned by its subsystem and registered with the `WorkerSupervisor`. A worker registration declares the owner subsystem, purpose, enablement setting, liveness signal, `SupervisionPolicy`, durable reconstruction source, shutdown order, and whether its work is idempotent or completion-marker-guarded. Known required registrations from existing specs include: the one `Scheduler` and the watch poller (File 33 §9); the audit-overlay writer and the lineage tracker (File 10, File 09); the memory consolidator (File 14); the retrieval and knowledge indexer (File 12); the storage garbage-collection and reconciliation sweeper and the blob verifier (File 20 §6, §11); the sync loop (File 21); the usage and rate-limit accounting worker and the model-catalogue refresh (File 17 §15.3); the diagnostic-log rotation (File 41 §4.6) and the resource-gauge sampler (File 41 §8.5); and the inbound-webhook receiver (File 33 §5.2, File 36). Subsystem-specific workers register the same way through their owning specs — but scheduling and monitoring CONCERNS (spaced-repetition review-due, data-source monitors, page watchdogs) are automations over the one `Scheduler` (File 33 §16.1), never sibling scheduler workers; a subsystem registers a worker only for work that is not a trigger over the one Scheduler. This file owns the worker substrate and supervision; each worker's *work* is its owning spec's.
...
- Each worker declares a `SupervisionPolicy`: whether it restarts on failure, the restart backoff (a typed strategy — exponential with cap and jitter, fixed, or decorrelated — reusing File 17 §11 and File 36 §7 shapes; delay sampling per §6.5.1), the circuit-breaker threshold and cooldown, and the escalation when the circuit opens (disable the worker, degrade its feature, notify the user). The policy is settings-owned with conservative defaults; it is never a hardcoded constant where meaningful variation exists.
...
- A worker restart is distinct from a run resume. The supervisor restarts a *worker*; it never auto-resumes an orphan *run* (File 04 §28) — orphan runs are surfaced for the user (§13). Before a restarted File 33 worker resumes production, it invokes its File-33 source-specific hook: Schedule reconciles due occurrences before re-arming; Event resumes from its checkpoint; WorldCondition preserves resumable continuity or records/resolves the applicable gap before re-arming. This is not a rerun of boot step 12. It never advances a checkpoint without an outcome, invents a crossing, or recreates a lost non-replayable debounce candidate.

### 6.5.1 Backoff Delay Sampling

The delay a restart backoff computes is sampled under one shared contract — stated here for the supervision consumer and mirrored at `provider.transport-level-retry-backoff` (File 17 §11.2.1) so File 17 §11, File 36 §7, and this section share one sampling semantics. The supervision strategy set is exactly the three variants (`Fixed`, exponential, decorrelated); File 17 §11.2's `Custom { namespace, name }` does not extend to worker supervision.

- **Band first, draw second.** A strategy defines, per restart decision, a closed inclusive integer delay band in milliseconds; the band is computed and capped first, then the delay is drawn uniformly over it, free of modulo bias. A post-draw clamp is forbidden — it concentrates probability mass on the band edge and re-synchronizes exactly the restarts jitter exists to spread. Equal endpoints are a degenerate band: the delay is that value and no randomness is drawn. The randomness source is an injected runtime seam, never a process-global — folds are reproducible under injection, and entropy failure is reachable by fault injection.
- **Exponential band.** base = min(initial_ms · multiplier^(n−1), cap_ms) for 1-based attempt n, saturating; the cap applies to the base, before jitter. Jitter is downward-only: the band is [base − span, base] with span = ⌊base · jitter_milliunits / 1000⌋ evaluated at non-overflowing width, both endpoints attainable — so every armed delay is at most cap_ms by construction, the finite configurable ceiling File 17 §11.5 promises, with no second clamp. The span is floored, never rounded, so the (1 − j)·base floor holds exactly at integer milliseconds. jitter_milliunits = 0 draws nothing; 1000 yields [0, base], and a drawn 0 is a legal immediate restart, bounded by the circuit threshold. `Fixed { delay_ms }` carries no jitter field and is never jittered.
- **Decorrelated band.** A per-episode `previous` delay, initialized to initial_ms, so the first draw is already spread. Each draw is uniform over [low, high] with low = min(initial_ms, cap_ms) and high = max(low, min(3 · previous, cap_ms)); `previous` then becomes the armed (= recorded) delay, so the sequence is verifiable from the fact stream alone — when the horizon reduction below applies, the reduced value is what is armed, recorded, and carried. The growth factor 3 is the variant's definition, not a setting. The normalization is mandatory — never an empty or inverted band; initial_ms > cap_ms degenerates to [cap_ms, cap_ms] (the ceiling is the canon guarantee; the floor is not) and surfaces a typed configuration diagnostic at policy resolution, once, never per sample. The restart-attempt counter is not an input; the sequence is non-monotonic by design. initial_ms = 0 is absorbing (a constant 0), legal and total, with the same resolution-time diagnostic.
- **Reset lifecycle — one episode notion.** Sampling state (the restart-attempt counter and the decorrelated `previous`) is per-episode ephemeral runtime state beside the worker's supervision state, never a durable field of a canonically encoded record (the File 17 §13.7 locality precedent). It resets exactly when the restart-attempt counter resets and at no other event: a plain heartbeat with the circuit closed, a plain heartbeat during a half-open probe, a succeeded probe, an explicit user reset, and construction or re-registration of the supervision slot. Explicit non-resets: degraded heartbeats in any state; observed `Failed`/`Stalled` (they advance it); circuit open or re-open; probe failure or probe start; and the elapse of any cooldown or of the delay itself (§6.5 — elapsed time alone never closes a circuit). A restart that merely spawns is not a reset — only a heartbeat or a succeeded probe proves recovery; reset-on-spawn turns a crash loop into an unbounded fast-restart loop. A policy change that alters the backoff variant or parameters resets `previous` (a carried value is not a sample of the new strategy's range); today a policy change arrives only through re-registration, which replaces the slot and satisfies the rule structurally. The half-open probe's timing is the cooldown, not a draw from this sequence; the explicit-reset immediate restart is likewise unsampled.
- **Randomness-source failure.** On a failed draw the band's upper endpoint is armed — never the floor or midpoint: a degraded draw loses spread but never restarts a failing worker sooner than the policy intended, and a starved host is a plausible shared cause of both the entropy fault and the worker failures. No lower-quality source is substituted, nothing retries indefinitely, and nothing is attributed to the worker or its circuit — an entropy fault is not evidence about worker health (§16.3). The restart fact records the armed delay and that it was unsampled; the degraded-entropy condition surfaces as typed state, never one notification per restart and never a silent fallback (§6.6). Degenerate bands draw nothing and cannot reach this path.
- **Totality and the armed deadline.** All delay arithmetic is unsigned 64-bit milliseconds and never wraps or panics; products are evaluated wide or saturating, and every path ends at min(…, cap_ms). Intermediate saturation is not an error and raises no diagnostic — it yields cap_ms, the declared ceiling, and the fact stays true. Degenerate parameters are total and legal — multiplier = 0 (initial at the first attempt, then 0), multiplier = 1 (equivalent to `Fixed`), cap_ms = 0 (immediate restarts) — each with a typed resolution-time diagnostic where anomalous. The delay is drawn once per restart decision: the value on the recorded fact and the value arming the deadline are one value, armed as monotonic-baseline-at-observation plus delay (§8.3 computed-once, File 17 §13.7 clock discipline). Should now + delay be unrepresentable, the delay is reduced to the representable one before it is committed — the reduced delay is what the fact records and what any downstream sampling state carries, and the reduction is typed and surfaced; the runtime never records a delay it did not arm. A resolution-time clamp of cap_ms to a settings-owned arming horizon (the File 17 §13.7 pattern) is a permitted mechanism, not a requirement.
- **Direction asymmetry, stated once.** Jitter applied to an honored external hint (a provider `retry_after_ms` — File 17 §11.2 waits at least that long; a probe-deadline margin — File 17 §13.6) only ever adds; self-computed backoff jitters downward. No hidden jitter kill-switch exists: reproducible debugging is jitter_milliunits = 0 or `Fixed` per policy, never a global toggle.
...
A **`FrontendClientSession`** is one attached client of the core — a graphical shell process, a terminal interface, a command-line client, or a programmatic client — owning its transport, subscription, buffer, and resume identity (§10.3). A client session has one or more presentation contexts when it attends; a non-attending session has none and resolves surface state through the typed noninteractive presentation context (`world.surface-state`, File 18 §5.1). At establishment the session declares, for each presentation context it opens, the profiles that context implements and the roles it serves (`ui.frontend-profile`, File 37 §1.3); attachedness is declared, never inferred from the transport or the medium. The session, like every runtime handle, is a runtime-handle projection: rebuilt on reconnect, never durable (§21.1).
...
Every persistence and registration contract requires deterministic reconstruction across restart (File 20 §13.2). Startup orders storage/projection recovery, service wiring, orphan reconciliation, automation circuit and Schedule/Event/WorldCondition reconciliation handoff, transient trigger re-arming, and worker spawning so new work sees coherent state and each phase failure is isolated and surfaced.
...
3. **Logging, tracing, and startup-interaction initialization** — initialize the structured logging and tracing baseline (File 41 §4) so all subsequent phases are instrumented, then initialize the bootstrap-independent `StartupInteractionBroker` over the startup-interaction adapter the process shell supplies. The adapter is selected by the runtime capability actually present, never by build kind: an in-process native modal dialog when a usable graphical session exists — constructed after `DisplayAndWindowMechanics` is registered (`packaging.platform-integration`, File 43 §7.3, which orders that registration before any platform-interface or window call) and without creating the ordinary webview or opening storage; an interactive-terminal adapter when attached to an interactive terminal; otherwise a noninteractive headless adapter that never waits for a human. Because a startup failure before this step must reach the user on the same channel, the shell constructs the adapter before `Runtime` boot and this step wraps that already-constructed adapter rather than creating a second mechanism. Graphical presentation is selected only when a usable graphical session exists; absence of one selects Terminal or Headless and is not StartupInteractionUnavailable. StartupInteractionUnavailable { adapter_kind, cause } means that the already-selected adapter failed to initialize or present its request. A DisplayAndWindowMechanics registration failure remains the originating startup failure: no platform-interface or window call follows it, and any inability to present that failure graphically is secondary and never replaces the originating failure or exit status. NativeDialog runs on the platform UI/event-loop context required by the selected adapter; the contract does not require a blocking dialog call on the process main thread. Absence of an adapter or of a response is never consent, and each request follows its own declared fail-closed or continue-locked behavior. The broker carries typed startup consent and failure requests before the ordinary frontend approval surface exists and depends on neither the storage substrate nor the capability-policy approval bus. It is not a second application shell and owns no policy decision.
4. **Storage open and integrity** — open and integrity-check the substrate; take the pre-migration backup; run forward-only migrations after backup; on migration failure roll back and stop with a typed error (File 20 §10, §12, §13.3). If verified recovery requires restoring to a point that may discard changes after the backup, the affected engine remains closed while the runtime issues a typed, recovery-plan-bound request through `StartupInteractionBroker`. No response or an unavailable interactive adapter produces `RecoveryConsentRequired`; it never means consent. Quarantine-and-fresh-start remains the final surfaced ladder step and is never automatic.
...
9. **Settings resolution** — resolve the settings cascade and spawn settings-change watchers (File 15), over the settings definitions already registered: a core service registers its settings definitions as it is constructed (step 7), and a plugin's or extension's within its registration phase (step 8), so every definition exists before the cascade resolves here.
10. **Secret-capable-startup gate and vault unlock** — inspect the registered platform memory-image routes and resolve `security.memory_dump_residual_strictness` (File 22 §4.3, §17.1). `Disclose` records and surfaces the residual risk and permits the configured unlock policy; `ConfirmOncePerMachine` permits unlock only when a device-local `MemoryImageRouteAcknowledgement` exactly matches the observed route set, or when the broker obtains and durably records one; `RefuseSecretCapableStartup` stops startup with a typed refusal. That refusal is a policy outcome of this step, recorded and surfaced through the broker — not a §11.4 phase failure, so the degrade-the-feature branch never applies to it. When confirmation is unavailable or declined under `ConfirmOncePerMachine`, startup may continue with the vault deferred and locked, and a headless adapter emits a machine-readable `StartupConsentRequired` rather than waiting. Then unlock the secret vault per its eager, on-demand, or deferred policy (File 22); credentials resolve at point of use thereafter.
...
12. **Recovery and reconciliation** — recover interrupted runs and reconcile orphans, surface resume-or-discard affordances, and never auto-resume them (File 04 §17.3; this file §13); invoke File 33's `AutomationStartupCoordinator` after orphan terminalization to repair circuits and establish durable handoffs for Schedule missed occurrences, Event checkpoint backlog, and WorldCondition history/gap boundaries under shared cold-start coordination (File 33 §3.4, §18); clean or revalidate stale leases (File 06 §11.6).

    This phase reaches its durable handoff boundary before any background worker spawns: every remaining lossless backlog is behind an unchanged progress marker, every open recovery session is identifiable, and no worker operates against ambiguous prior-session claim, run, circuit, checkpoint, or boundary state. This deliberately supersedes "runs to completion": bounded lossless backlog may continue draining after worker spawn, but reconciliation truth cannot remain ambiguous.
...
14. **Frontend bridge / front-end open** — open the frontend-bridge transport and the presentation contexts the restore policy elects, or one default presentation context on first run (`ui.shell`, File 37 §4.5), or nothing in headless mode (§9.4, §10). For each elected `Interactive` context, the restore policy resolves the presented scopes, composition or `ViewPreset` references, and any safe profile-local realization payload that does not require live unit registration before that context first presents; semantic unit anchors, selections, attended-position anchors, and registered restore hints that require a live unit are deferrable after the bridge is interactive (§11.4; `ui.shell`, File 37 §4). An in-process bootstrap dialog surface that participated in `StartupInteractionBroker` is retired here; it never becomes a second application shell or a source of durable truth.
...
- Startup isolates and surfaces errors. A phase failure that compromises a core invariant (storage corruption with no recovery source, a failed migration) stops startup with a typed, actionable error and rolls back to the last-known-good state (File 20 §12.3); a phase failure that only disables a feature (a sidecar that will not start, a provider that will not authenticate, a plugin that fails to load) is recorded, degrades that feature gracefully (§16), and does not abort startup (`infrastructure/lifecycle.md`). A startup failure that stops the process is presented through the selected startup-interaction adapter before exit when that adapter is available. Failure of presentation yields StartupInteractionUnavailable as a secondary typed presentation failure; it never replaces the originating startup failure or its exit status. Terminal launches additionally render the originating failure on stderr.
...
- Startup is responsive: phases that are not prerequisites for a usable UI (provider warmers, projection verification of cold data, sidecar readiness, indexer warm-up, and presentation-unit anchor, selection, attended-position, or registered-hint restoration that requires post-bridge unit registration) run as background work after the bridge opens, so the application is interactive against cached and rebuilt state while that work streams in. The responsiveness budget is a setting; the contract is that the UI opens against a consistent state, never against a half-migrated or unverified substrate. A capability that depends on a still-warming background phase returns a typed warming/not-ready state, or waits or queues only when that capability explicitly supports doing so and the work is admitted under the `WorkQueue`, shutdown, and backpressure rules. It never returns silently incomplete results. This warming state is distinct from degraded unavailable-backend mode (§16): it is transiently not-ready and derives from startup-phase readiness facts.
- Preparation that removes a later cold host boundary runs only after the bridge is interactive, as bounded, deferrable, cancellable background work admitted through the `WorkQueue` (§7) at the lowest priority and shed first under resource governance (§17), and it weakens no mandatory gate. It does not become an eager pool whose startup work or resident set merely relocates the latency or the resource cost it was meant to remove; an eager managed-service start condition (§4.5) remains valid where that service's declared lifecycle and its File 23 resource profile carry the trade-off. Where first use precedes readiness, the owning capability exposes its typed warming state or the Packaging-owned typed acquisition state (File 43 §9.2), and the boundary it is waiting on, rather than placing the cold cost silently on the ordinary operation path. Packaging-created prefetch work may enter background preparation only after File 43's resolved delivery policy authorizes it; the `Runtime` controls work admission and scheduling only and does not change acquisition policy or installed-payload state.
...
This section owns the ordered boot graph and its determinism, isolation, and responsiveness rules. Storage phases are File 20's; bus/hook/registration phases Files 10 and 05's; settings File 15's; vault unlock File 22's; warmers and health checks Files 18, 17, and 05's; worker work each worker owner's; orphan-run policy File 04's; automation circuit repair, Schedule/Event/WorldCondition reconciliation handoff, cold-start pacing, restart hooks, and transient re-arming File 33's; and UI Files 37 and 38's; startup-interaction mechanics are this file's; the security posture and acknowledgement semantics carried through them remain File 22's; window/display-mechanics registration is File 43's.
...
The application must be ready to close at any time without losing a committed fact. A graceful shutdown completes in-flight work and flushes pending state where it cheaply can; correctness, however, comes from commit boundaries and restart reconciliation, not from waiting on a drain timer, so that a hard close, a crash, or a power loss loses only work-in-flight, never committed work. An unsent composition is conversation-scoped user content rather than disposable work-in-flight: `intent.message` (File 02 §3.4) persists it as it changes and commits it at its declared boundaries, so what has been committed survives an abrupt termination and only the uncommitted tail since the last commit is lost.

### 12.3 The Shutdown Sequence

On a shutdown signal (a termination signal, a user interrupt, an explicit quit action, an operating-system shutdown, or the typed `QuitRequested` outcome of the presentation layer's context-lifecycle resolution — closing a presentation context is a presentation operation and never by itself a shutdown signal, `ui.shell`, File 37 §4.5):
...
- A shutdown step that fails never abandons the sequence; the shutdown outcome is reported as degraded or forced, never clean.
...
- The shutdown admission classes (`ShutdownCritical`, `AlreadyAcceptedDurable`, `Deferrable`, `RejectNew`, §12.3) are a distinct taxonomy from the `WorkQueue` admission classes (§7.3): the former governs which work shutdown admits, drains, defers, or rejects, while the latter governs which work a bounded queue accepts and how it sheds on overflow; a unit of work carries one of each independently.
...
- An orderly shutdown performs the bounded local commit due for the presentation restore record before the settings/storage substrate closes (`ui.shell`, File 37 §4); abrupt termination preserves that record's last committed revision and may lose an uncommitted restore tail, which the next launch's restore report names as a possibility, never as a durable fact.
...
- **Runtime handles are reconstructed, not restored.** Active subscriptions, worker/watcher handles, pool entries, timers, and Scheduler arming are runtime-handle projections (File 20 §7.3), rebuilt from durable declarations, ledger facts, and owner-declared processing state such as File 33 checkpoints and gap/boundary records (§21). Those inputs are not runtime handles; handle loss never authorizes losing identity, accepted work, delivery progress, or circuit evidence.
...
This section owns application-level crash-recovery orchestration. Orphan-run policy is File 04's; process and sandbox reaping File 23's; storage integrity, last-known-good, and reconstruction internals File 20's; Schedule missed-fire reconciliation, Event checkpoint recovery, WorldCondition gap/boundary recovery, circuit repair, and transient re-arming File 33's; and lease revalidation File 06's.
...
- Host-mediated degradation is classified at the source-owned locus Atlas directly measured, never by enumerating the software installed on the host. `HostMediationLocus` is the closed set `ProcessStartOrReadiness`, `Filesystem`, `NetworkOrTransport`, `NativeIpcOrPlatformRegistration`, `RendererOrDevice`, and `ResourcePressure`, each owned and emitted by its source layer. The source owner selects the locus by the operation it directly measured, not by an incidental lower-layer transport: process-start/readiness producers emit `ProcessStartOrReadiness`, filesystem producers `Filesystem`, network/transport producers `NetworkOrTransport`, native-IPC/platform-registration producers `NativeIpcOrPlatformRegistration`, renderer/device producers `RendererOrDevice`, and resource-governance producers `ResourcePressure`; therefore a renderer operation carried over native IPC emits `RendererOrDevice`, because renderer/device is the measured operation. A diagnosis records that locus, and any host control Atlas directly observed, separately from two advisory optional classifications: `Option<HostControlMechanismClass>`, whose closed initial set is `FilesystemFilter`, `PowerManagement`, `ResourceInterposition`, `HostSandboxing`, `NetworkInterposition`, and `Scheduling`; and `Option<HostControlProvenanceClass>`, whose closed initial set is `SecurityTooling` and `DeviceManagementPolicy`. Both advisory classes are non-authoritative and may be absent — absence is how an unrecognized mechanism or provenance is represented, and a genuinely new class is a catalogue revision rather than an `Unknown` or `Other` member — and neither is inferred from timing alone. Where more than one provenance class is directly known, the producer records the most proximal directly observed provenance; when `SecurityTooling` and `DeviceManagementPolicy` are both directly known for the same observation, it records `SecurityTooling`. The former single `HostControlCauseClass` catalogue is deleted and is not an alias, compatibility spelling, or decoder fallback for either new catalogue. A product or vendor name is never a value: an antivirus or endpoint product, a sync client, a virtual private network, a management policy, or a competing application is not a canonical causal value, and the user-facing rendering of a locus, mechanism class, or provenance class resolves through the localizable key File 41 §18 requires. Remediation invokes the owning subsystem's existing mechanism — resource governance and load shedding (§17), routing, retry, or typed degradation — and changes no security, policy, confinement, or correctness semantics; operational health re-owns no storage, packaging, network, or observability behavior.
...
Update application is the relaunch mechanics by which a staged update becomes the running application: the `Runtime` sequences the drain, the relaunch, and the post-update verification, consuming the staged update File 43 produces and the last-known-good guard File 20 maintains.

### 18.2 Rule

- An update is applied on restart, never in place on a running core: File 43 stages the new version and signals readiness; the `Runtime` performs a graceful shutdown (§12), relaunches into the staged version, and runs the startup self-checks (§11.4). A mutate-the-running-installation update is rejected.
...
- The update channel, the download and staging, the signature and integrity verification of the new binary, the installer, and the platform crash-handler and native-messaging-host registration are File 43's; the drain, relaunch, and post-update verification mechanics are this file's.

### 18.3 Boundary

This section owns the apply-on-restart and relaunch mechanics. The update channel, staging, and binary integrity are File 43's; the last-known-good marker and self-check rollback are File 20's; the graceful shutdown and startup sequences are §§11–12.
...
Reads are `ReadOnly` and agent-invocable under the standard agent-exposure rules; operational controls (restart, relaunch) are user-gated and policy-bound — an agent may propose a restart or relaunch but does not perform one unattended, and the lifecycle controls carry the tier their blast radius warrants.

### 19.2 Events

The `Runtime` emits and consumes through the one event bus (File 10 §5), reusing the reserved lifecycle and worker entries — `AppStarted`, `AppShuttingDown`, `AppStopped`, `BackgroundWorkerSpawned`, `BackgroundWorkerHeartbeat`, `BackgroundWorkerStopped`, `BackgroundWorkerFailed` (File 10 §4.1) — and registering runtime-specific facts (a worker `CircuitOpen`, a sidecar readiness transition, a transport reconnect, an update relaunch, a remediation action) as `Custom { namespace: "runtime" }` extensions (File 10 §4.3). It introduces no new top-level event or ledger-entry kind and no parallel bus. Consequential facts (lifecycle transitions, worker failures, remediations, relaunches) commit to the ledger; transient runtime coordination (heartbeats under their delivery class, pool churn) flows on the bus per its delivery class. The unrecordable-condition case — where the condition to be recorded is itself what makes recording impossible — is the sole exception to the ledger-recording statements in §6.6, §12.4, §16.3, §19.2, §22, and §23: surfacing that condition as live health state and in the device-local diagnostic stream (`observability.logging`, File 41 §4.6) discharges the duty to record it, with neither surface carrying any of a ledger fact's authority, and the same discharge applies to a consequential fact of that boot whose ledger record the same condition makes impossible. A security-relevant operational action (an elevated-helper restart, a relaunch) records into the audit overlay (File 10 §16.4).

Runtime-owned host-boundary duration observations are registered as the File 10 §5.3 `Custom { namespace: "runtime", name: "HostBoundaryTimingObserved", payload }` event through File 05 §16.2's canonical registration mechanism; the registration declares the event transient-only, it adds no top-level event or ledger-entry kind, commits no ledger record, and uses the `sampled_diagnostic` delivery class (File 10 §5.5). The registered payload is exactly `operation_class`, `locus` (a `HostMediationLocus`, §16.3), `duration_us`, `outcome_class`, `accuracy_class` (`Exact` or `Approximate`, File 41 §6.2), `sample_validity_flags`, and `sample_origin`. `operation_class` is the canon-revision-extensible closed catalogue `ProcessStartOrReadiness`, `FilesystemOperation`, `NetworkOrTransportOperation`, `NativeIpcOrPlatformRegistration`, `RendererOrDeviceOperation`, and `ResourceGovernanceOrScheduling`. Finer operation granularity belongs to the registered `MetricInstrument` identity; neither an `operation_class` member nor its encoded value carries user content, a path, or capability-instance identity. `outcome_class` is the disjoint closed catalogue `Completed`, `Refused`, `Failed`, `Interrupted`, and `TimedOut`: `Completed` means successful terminal completion; `Refused` means explicit host-boundary refusal or denial; `TimedOut` means timeout expiration; `Interrupted` means non-timeout cancellation or interruption; and `Failed` means every other terminal typed failure. `sample_validity_flags` is a set over the closed flag catalogue `ClockSuspect` and `Truncated`; an empty set means valid and both flags may coexist. `sample_origin` is the closed catalogue `Measured` and `Emulated`, while `accuracy_class` remains the separate closed `Exact` or `Approximate` axis File 41 §6.2 owns. The payload carries no crossing count, no vendor identity, no host-control code, no resource-pressure reference, and no raw path, uniform resource locator, command string, executable or process identity, or user content. Every `HostMediationOperationClass` member has a sensitivity floor of `Sensitive`; because every current member has that floor, the effective default classification of a `HostBoundaryTimingObserved` emission is therefore `Sensitive`, not `Public`. Each emission uses at least the sensitivity of the source operation or source fact and the registered `operation_class` sensitivity floor; the timing producer may raise but never lower that classification under File 10 §5.6. The producing seam measures its own duration from a monotonic source; File 41 re-times no operation from recorded timestamps. Where Atlas directly observes a suspend, resume, or clock-domain discontinuity overlapping the measured interval and the timing primitive cannot prove the active elapsed time, the observation is `Approximate` and ineligible for envelope or baseline learning; where no discontinuity is observable, the measured value is preserved as diagnostic evidence and no external cause is inferred. A consequential readiness, refusal, policy, or execution outcome remains recorded through its existing owning fact, and a duration observation is never a remediation command, a correctness decision, or a source of authority.
...
- the registered background-preparation work class (§11.4) — its admission (`runtime.work_queue.background_preparation.enabled`), its concurrency limit (`runtime.work_queue.background_preparation.max_concurrency`), and its pressure-shed behavior (`runtime.work_queue.background_preparation.pressure_shed_behavior`) — expressed through the `WorkQueue` settings model and its existing load-shedding policy vocabulary rather than a parallel preparation scheduler;
...
- **Durable (owned by other layers, referenced here):** the `BackgroundWorker` declarations, managed-service definitions, and accepted work represented by queues — including a queued automation fire under `fire_id`, a queued circuit probe under `probe_id` through its File 33 binding and pending run, and a staged upload — remain durable in their owning substrates (Files 15, 33, 20), never a parallel runtime table. A `WorkQueue` entry is only a handle to that owning record.
...
- The `Runtime` reconstructs its runtime state deterministically at startup from durable declarations and the ledger (§11, §13); it re-derives nothing from a live mutable source and re-queries no live endpoint for a historical fact (`context.assembly-replay-snapshot`, File 13; `provider.token-source`, File 17). A worker, a timer, a pool entry, and a service handle are rebuilt, not restored from a stale handle.
...
The `Runtime` operates under these constraints; the tunable parameters within them are configurable as settings (§20) with the stated canonical defaults:
...
- the **automation** (File 33) layer's Scheduler/Event/WorldCondition workers are placed in this file's boot graph as supervised workers and use this file's runtime primitives; before they spawn, startup recovery invokes File 33's `AutomationStartupCoordinator` for circuit repair and Schedule/Event/WorldCondition durable handoff, then transient trigger state is rebuilt
...
- the **Packaging, Platform, and Distribution** spec (File 43) stages updates this file applies on restart, registers the platform crash handler and native-messaging host this file's recovery and transports consume, and ships the sidecar binary inventory this file runs under its managed-service lifecycle contract; the running lifecycle is this file's, the delivery is Packaging's

## 25. Canonical Rule Anchors

Anchor: `runtime.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `runtime.chosen-model`, `runtime.boundaries-with-adjacent-layers`, `runtime.runtime`, `runtime.process-topology`, `runtime.async-concurrency-substrate`, `runtime.background-workers`, `runtime.work-queues`, `runtime.timers`, `runtime.service-graph`, `runtime.transports`, `runtime.startup`, `runtime.shutdown`, `runtime.crash-recovery`, `runtime.bootstrap-config`, `runtime.cancellation-token`, `runtime.operational-health`, `runtime.resource-governance`, `runtime.update-relaunch`, `runtime.capability-surface-events`, `runtime.settings`, `runtime.persistence-replay`, and `runtime.operating-constraints`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
