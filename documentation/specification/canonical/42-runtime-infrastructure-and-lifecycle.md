...
14. **Frontend bridge / front-end open** — open the frontend-bridge transport and the shell windows the restore policy elects, or one default shell window on first run (`ui.shell`, File 37 §4.5), or the terminal interface, or nothing in headless mode (§9.4, §10). An in-process bootstrap dialog surface that participated in `StartupInteractionBroker` is retired here; it never becomes a second application shell or a source of durable truth.
