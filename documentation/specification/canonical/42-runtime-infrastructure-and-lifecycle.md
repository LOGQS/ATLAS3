...
12. **Recovery and reconciliation** — recover interrupted runs and reconcile orphans, surfacing resume-or-discard affordances and never auto-resuming (File 04 §17.3, §13); re-arm triggers and reconcile missed fires under the cold-start guard (File 33 §3.4, §18); clean or revalidate stale leases (File 06 §11.6). This phase runs to completion before any background worker spawns: recovery quiesces the claim and run state the workers will produce against, so no fire producer ever operates concurrently with the reconciliation of a prior session's leftovers.
