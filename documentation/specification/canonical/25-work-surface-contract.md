...
- the `PresentationUnitKind` catalogue and the `ViewPreset` — the surface's declared presentation units and startup presentation intents — and the rule that a view preset changes presentation only, never autonomy, model choice, context policy, execution policy, approval posture, or sandbox policy
...
- surface activation and the shell relationship: how routing selects the primary surface, the three guarantees the core owes every attached presentation context — one resolvable primary work context, invocation reachable without displacing it, and typed projections of run observability and the artifact pool — and multi-surface composition
...
- the per-surface specifications themselves (Coder, Web, Data Processor, Teacher, GUI Control, System Agent) — those specs declare their `SurfaceContract` to the shape this file defines and own their specialized capabilities, presentation units, runtimes, and workflows
...
- the live `SurfaceState`, `PresentationUnitState`, `Selection`, `UiMode`, the world-entity catalogue, the durability tiers, the availability evaluator, or snapshot resolution — File 18 owns those; this file declares the static shape whose live values File 18 holds
...
- UI rendering — how a presentation composition is realized in a medium, the chrome around a presented unit, the concrete contribution runtime, theming, saved-composition editor behavior, morphing animation, and accessibility presentation are owned by the UI specs; this file specifies only the data contracts they consume

## Source Resolution

Families reviewed: the domain/surface-architecture material (`agents/domain-architecture.md`, `domains/README.md`, `foundations/architecture.md`, `atlas3-core/TODO.md` domain catalogue and design filter); the per-surface `DomainSpec` instantiations (`unit08-coder.md`, `unit09-web.md`, `unit10-gui-control.md`, `unit11a-memory.md`, `unit11b-data-processor.md`, `unit11c-system-agent.md`, `unit11d-teacher.md`, and the `domains/coder`, `domains/web`, `domains/data-processor`, `domains/teacher`, `domains/system-agent`, `domains/gui-control` reference files); the state-awareness and self-registration material (`cross-cutting/state-awareness.md`, `ui/14-1-application-shell.md`, `ui/14-6-to-14-8-...md`); the view-preset, morphing, and layout material (`ui/15-1-layout-customizability.md`, `ui/15-2-domain-based-workspace-morphing.md`, `ui/15-3-and-15-4-participation-levels-personas.md`, `ui/README.md`, `unit13-ui.md`); the action and prompt material (`cross-cutting/actions.md`, `agents/prompt-engineering.md`, `agents/agent-execution.md`, `unit04-routing-agents-prompt.md`); the extensibility material (`unit11-cross-tool-learning.md`, `kuzeys-ui-customization-and-widgets-addendum.md`, `cross-cutting/service-layer.md`, `cross-cutting/composition.md`); the strategic target-state review (`codex_recommendations.md` §1, §8, §10.1, §13.1, §14.10); and the cross-surface declaration patterns in existing ecosystems (`claude_tool.md`, `opencode-compressed.md`, `continue-compressed.md`, `warp-compressed.md`, `terax-ai-compressed.md`, `operator-use-compressed.md`).
...
- **The shell — invocation always reachable beside a primary work context.** The resolved shell relationship is that invocation stays reachable independently of whatever currently holds focus, and that exactly one primary work context is resolvable per presentation context, where conversation is an always-available control rail and presentation view, not the universal container. This is consistent with `core.product-thesis` (File 01 §1), `core.system-layers` (File 01 §2.1 and §2.3), and `intent.presentation` (File 02 §8). The UI specs own rendering; this file owns the relationship.
...
- a **control rail** (`core.system-layers`, File 01 §2.1) — an entry-and-steering mechanism, in any modality, through which the user or system invokes capabilities; its `ControlRailKind` classifies the entry class it offers (`controlrail.control-rail`, File 26 §3.4) and its declared `GestureGrammar` types the gestures it accepts (`controlrail.registry`, File 26 §14.2). A control rail initiates or steers work; it is not the work model. Conversation is always available and may be the active surface, but it is a control rail and continuity surface (`intent.conversation`, File 02 §2.1, `core.current-major-area-classification` (File 01 §5.1)), not a work surface.
...
- the live **`SurfaceState`** (`world.surface-state`, File 18 §5) — the runtime values of a work surface's declaration: the active surface, the active units and their prominence, the focus target, selection, available capabilities, and ui mode. `SurfaceState` holds the live values; the `SurfaceContract` declares the static shape (§5).

Where a prior file's `Surface` world entity (`world.world-entity`, File 18 §4.3) names "an active work surface or control rail the user can be in," that entity projects from a `WorkSurface` when the active surface is a work surface, and from a control rail when it is conversation or any other registered rail.
...
`routing.run-intent` (File 03 §4.3) carries `primary_surface` and `supporting_surfaces`; `routing.surface-capability-selection` (File 03 §8) establishes that surfaces and subsystems are not hard fences and that routing selects one primary surface plus zero or more supporting surfaces. This file defines what the `primary_surface` value resolves to (a registered `WorkSurface`), and distinguishes run execution binding from user-facing presentation focus (§11). `primary_surface` is optional (`routing.run-intent`, File 03 §4.3): a run that resolves to no work surface — discovery, capability-level, or conversation-baseline work — binds to conversation, the always-available control rail and continuity surface (§1.1), and composes against the always-present baseline `SubsystemSurfaceSpec` (`surface.subsystem-surface-spec`, File 07 §5) rather than a specific surface's spec (§11.2). Routing selects the run's execution surface; File 18 owns live focus and presentation state.
...
`world.surface-state` (File 18 §5.6) and `world.consequences-for-later-specs` (File 18 §17) previously delegated this static surface-declaration boundary — how a surface declares the presentation units, capabilities/control affordances, views, and context policies it can present — to the Work Surface Contract spec; this file realizes it. §5 of this file is that static declaration; `SurfaceState` holds the live values those declarations take at runtime, and File 18 holds those live values. A surface self-registers its units and state to the world model (`world.observation-state-update`, File 18 §8.1) and is never screen-scraped to learn its own state (`perception.tiered-sensing`, File 19 §5.4). A surface declares which sensors it exposes and the privacy class of each (`perception.consequences-for-later-specs`, File 19 §19); perception holds the capture mechanics behind those declarations.
...
- **State** (§5): the `PresentationUnitKind`s the surface can present, the typed selection kinds it produces, and the static shape of the surface-state fields whose live values `world.surface-state` (File 18 §5) holds.
...
- `icon_key` — optional symbol identifier for presentation; a medium resolves it to its own symbol form (a glyph, an icon, a spoken tag) or ignores it without loss of correctness, and the concrete symbol is the UI spec's.
- `keywords` — optional list of typed keywords the router (`routing.surface-capability-selection`, File 03 §8) and any `Discovery` rail (`controlrail.control-rail`, File 26 §3.4) consume for surface discovery and selection.
...
The state declaration is the surface's static description of the presentation units it can present, the selection kinds it produces, and the shape of the surface-state fields it presents. It is the static counterpart of the live `SurfaceState` (`world.surface-state`, File 18 §5): this file declares the shape, File 18 holds the values.

### 5.2 Required

- A `SurfaceContract` declares the `PresentationUnitKind`s its surface can present (§5.3), the `SelectionKind`s it can produce (drawn from the canonical `Selection`/`SelectionKind` set `world.surface-state` (File 18 §5.4) fixes, plus the surface's registered `Custom` selection kinds), and the typed state-field shape of each unit kind.
- The declaration is the *static shape*; the live values — which units are active and with what prominence, which is primary, what is focused and selected, the current `UiMode`, and the available-capability list — are the live `SurfaceState` (`world.surface-state`, File 18 §5.2) the world model holds and projects. A surface's units self-register their live state to the world model when they are activated and update it on focus, selection, and content change, and unregister when they are deactivated (`world.observation-state-update`, File 18 §8.1); the declaration tells the world model what kinds of units and state to expect, and the runtime self-registration tells it the current values.
...
### 5.3 `PresentationUnitKind`

A **presentation unit** is an independently addressable projection of substrate or work state that a surface presents; it assumes no pane, no tab, no region, and no simultaneity with any other unit. A `PresentationUnitKind` is the semantic classifier of a unit a surface can present, aligning with the `unit_kind` field of `PresentationUnitState` (`world.surface-state`, File 18 §5.3). The canonical baseline unit kinds are cross-surface interoperability roles naming the material the unit presents and the selection it yields — `editor`, `terminal`, `browser`, `inspector`, `document`, `canvas`, `list`, `board`, `timeline`, `graph`, `diff`, and `preview` — plus `Custom { namespace, name }` for every other unit kind, registered through the proposal-first mechanism (`capability.runtime-mutation`, File 05 §16.2); the control-rail and presentation roles a surface needs beyond the baseline, such as `activity`, `sources`, `monitors`, `file_tree`, and `search`, are registered `Custom` kinds, which is what makes §10.3's validation of a declared kind as canonical or registered decidable. The baseline is a content-role vocabulary, not a component taxonomy: a medium presents an `editor` unit's text however it can and a unit kind obliges no surface to declare it and no medium to present it.

A `PresentationUnitKindDeclaration` carries the kind id (`unit_kind`), the typed shape of the unit's compact state-field `data` summary (a file path, a url, a selected entry id — never the resource body, `world.surface-state` (File 18 §5.2)), the selection kinds the unit produces, the control affordances available in the unit, and structural semantics consumed by the world model and control rails: stable semantic role, accessible label/description key, interaction kinds, focus behavior, and the parent relationship hint (`parent_unit_kind_hint`). Renderer families and presentation in any medium are UI-owned. Two surfaces may declare the same baseline unit kind; the shared editor, terminal, browser, inspector, and canvas roles are shared substrate projections (`block.cross-surface-interoperability`, File 08 §12), not per-surface reimplementations (§12).

### 5.4 Cross-Surface Unit Sharing

A unit kind a surface declares is a projection over shared substrate, not a private store: a Coder `editor` unit and a Data Processor `list` unit both project `Block`s from the one pool (`block.cross-surface-interoperability`, File 08 §12) and self-register through the one state-awareness contract (`world.observation-state-update`, File 18 §8.1), whatever medium presents them. A surface may borrow another surface's unit — a Coder surface presenting a `browser` unit — without changing its primary surface (§11.4, `surface.subsystem-surface-spec` (File 07 §5.5)). The borrowed unit carries its own `surface_binding` in `PresentationUnitState`; user-invoked controls inside that unit resolve against the unit binding for capability availability and ledger attribution, without changing the host surface or any active run's execution context unless File 03/File 04 reroute occurs. A surface that needs a genuinely new unit kind registers it; it never forks a parallel projection of an existing kind.

### 5.5 Boundary

This section declares the static unit and state shape. File 18 owns the live `SurfaceState`, `PresentationUnitState`, `Selection`, and `UiMode` and the self-registration contract; UI specs own how a medium presents a unit. This file names the kinds; File 18 holds the values.
...
- A surface's contributed capabilities are the same capabilities every registered control rail resolves, whatever its entry class and whatever modality its grammar accepts (`controlrail.control-rail`, File 26 §3.4) — one declaration, all invocation paths (`core.extension-planes`, File 01 §6.14). A surface declares no out-of-band action path.
...
A view declaration is the surface's static description of the presentation shapes it offers: the `ViewPreset`s the user can switch between, the default inspectors it declares, the customization policy it permits, and the cross-cutting affordances it declares. A `ViewPreset` is a named, presentation-only startup intent; it is not an autonomy mode and does not silently change backend policy (§13).

### 7.2 `ViewPreset`

A `ViewPreset` is a named, registered, presentation-only startup intent a surface offers. It declares its `unit_set` — which presentation units are active — its `focus_unit` — which of them is primary — the inspection units it exposes, its presentation-only startup state, and optional profile hints, opaque outside the declaring profile. How a medium expresses primacy and ordering over that set is the medium's, and the profile hints carry no portable identity (realized in the windowed-desktop profile as a pane arrangement, §7.7). A session declaring no profile resolves the preset from its `unit_set`, `focus_unit`, inspection units and startup state alone, and loses no correctness by ignoring them. It may reference an associated settings or profile bundle (`settings.profiles`, File 15 §7), but applying the `ViewPreset` changes presentation only unless the user explicitly applies the associated settings/profile changes through the settings and policy paths. A surface ships built-in view presets, and plugin and user-authored presets register the same way; this section owns the `ViewPreset` declaration for all three authorship sources, while File 15 owns the saved-preset settings and defaults — the user may save custom presets and override the default per scope. Applying a saved preset commits through the settings path (`settings.events-snapshots`, File 15 §14.1) and is presented by the shell (`ui.events`, File 37 §21.1); this file declares no separate applied-preset event (§18.1).

A `ViewPreset` is a settings/profile/customization record or built-in declaration, not a backend autonomy control. It supersedes the specbase "participation level," "persona," and "personality preset" framing where those named a startup shape: a view preset names which presentation shape opens, never how autonomous the agent is (§13). Selecting a richer view preset is progressive disclosure — the user sees more state and asks different questions — not a mode change (`cross-cutting/state-awareness.md` realized; `world.surface-state` (File 18 §5.5)). Changing presentation mid-run never silently changes model selection, context policy, execution entry, budget, sandbox profile, approval posture, or instruction-source authority.

### 7.3 Surface Morphing Is a Projection

When a work surface becomes the active presentation surface, a presentation context may project that surface's declared units and active view preset (§11). Morphing is a pure projection driven by three inputs this file and adjacent files own: the surface's `SurfaceContract` (the declared unit kinds and presets, this file), the live `SurfaceState` (`open_units` and `primary_unit`, `world.surface-state` File 18 §5), and, for runs, the routing decision (the `primary_surface`, `routing.surface-capability-selection` File 03 §8). The same triple projects to the same presentation description, which is the morphing-as-projection round-trip §21 requires verified, and it is replayable from a recorded snapshot because all three inputs are recorded. How a context transitions between projections — any transition, composition, animation, or navigation behavior — is the UI specs'. Morphing changes presentation, not the work model (`intent.presentation`, File 02 §8.1, `intent.presentation` (File 02 §8.5)).

### 7.4 Customization Policy

A `SurfaceContract` declares a `customization_policy`, not concrete slot identifiers or geometry. The policy declares which kinds of customization the surface permits, naming each class by what it changes rather than by how a medium performs it — which units are presented, with what prominence and which one primary; what is contributed into a unit or into a presentation role (`customize.customization-policy`, File 38 §11); which unit kinds the surface admits as registered additions; and equivalent future extension classes — the maximum extension density or safety bound where needed, and whether user, plugin, or AI placement is allowed per kind. The policy is intentionally capability-like and extensible: a technically capable user should be able to customize any behavior or presentation facet for which a class names what it changes and a safe capability, policy gate, reversible commit, and source-trust boundary can be defined (realized in the windowed-desktop profile as that profile's own class set, §7.7).

Specific slot identifiers, region geometry, renderer constraints, contribution runtime, and placement algorithms belong to the UI Customization, Widgets, and Theming spec that consumes this policy (`customize.customization-policy`, File 38 §11). `SurfaceContract` owns what kinds of customization are allowed; the UI customization layer owns where and how each medium realizes them. AI-assisted surface, view-preset, contribution, or composition mutation is a capability flow: inspect current declaration/state, propose a diff or preview, pass policy, commit reversibly where possible, and record provenance. Direct mutation of Atlas core UI code, a surface manifest, or runtime state outside that flow is invalid.

### 7.5 Default Inspectors and Cross-Cutting Affordances

A `SurfaceContract` declares which substrate state the surface exposes for inspection (for example context, schema, influence, execution, or registry inspection) as `PresentationUnitKind: inspector` projections over that state, never private stores; how a medium presents them is that medium's, and no medium is obliged to give them a unit of their own. A surface declares the cross-cutting affordances it offers — teaching and observability — so control rails, other surfaces, and any attached presentation context can consume them uniformly, and each declaration names its affordance kind, the capability ids it binds, and the substrate state it exposes. These affordances are declarations that UI and substrate layers may consume; they introduce no surface-private behavior and force no particular UI rendering.

### 7.6 Boundary

This section declares the surface's view presets and customization policy. UI Shell/Layout/Presentation specs own shell, unit, and morphing presentation; UI Customization/Widgets/Theming specs define concrete placement and contribution mechanics; File 15 owns the settings and profile layers for saved presets and defaults. This file declares the shape and policy; settings resolve it.
...
Realizes: §7.2. In this profile a view preset's profile hints are an arrangement — which pane or tab each active unit occupies, and which of them opens focused — and the inspection units the preset exposes are its visible inspector panels; the split structure, the relative sizes and the divider mechanics of that arrangement are the composition model's (`ui.layout`, File 37 §5). The hints are opaque outside this profile and carry no portable identity (`customize.saved-layout`, File 38 §7.2).

Realizes: §7.4. In this profile the customization classes §7.4 names are panel rearrangement, widget placement, custom panel registration, and per-panel extension regions. A panel is the windowed-desktop realization of a presentation unit: a pane or a tab with a caret, tabs, bars, side-by-side placement and collapsibility.
...
- A `SurfaceContract` declares the surface's default run budgets (`run.budgets-limits`, File 04 §21, `run.settings` (File 04 §27)) and its spawnable subagent types (the `spawnable_subagent_types` of its `SubsystemSurfaceSpec`, §6, `run.child-runs-multi-agent-work` (File 04 §16)). Budgets are configurable, visible ceilings overridable per scope — not enforced by default, warning-first, enforced as a non-fatal stop when opted in, never hidden hard limits (`run.budgets-limits`, File 04 §21).
...
- Built-in surfaces (the canonical baseline) ship registered from `BuiltinPackage`. Plugin surfaces register when their plugin loads and unregister when it unloads (`capability.lifecycle`, File 05 §16). User-defined surfaces are discovered from a user-writable surface directory under the data root (the canonical default `<data-root>/surfaces/<surface_id>/`, placed by `storage.physical-layout-locality` (File 20 §8) and named here), each carrying a `SurfaceContract` manifest and its optional contributed skills, capabilities, unit kinds, settings definitions, and instruction sources; discovery is event-first (a file-system watcher, `perception.triggers` File 19 §8) with a flagged polling fallback only where no change events exist (`core.event-first-by-default`, File 01 §7.15). The shipped surfaces live alongside user surfaces and load the same way; the registry distinguishes them by `registration_source` and `trust_state`.
- Registry mutations emit surface-relevant events (§18) so every attached presentation context, every registered control rail, the router's enabled-surface list, and the world model react without polling. A surface registered, unregistered, enabled, disabled, or whose availability changed updates the routing frame's enabled-surface input (`routing.routing-frame`, File 03 §3.1) and the tool-surface composition (`surface.surface-relevant-events`, File 07 §13) on next computation.

### 10.3 Registration Validation

Registration validates the `SurfaceContract`: every required section present (§4.2), the `surface_id` well-formed and not colliding with an active or tombstoned registered surface id, the `surface_contract_version` unique for that surface, the `surface_kind` canonical or a registered `Custom`, required referenced capability ids resolvable in the registry or the same registration transaction, the declared `PresentationUnitKind`s and `SelectionKind`s canonical or registered, the surface-level `availability_predicate` and the `SubsystemSurfaceSpec.availability_predicate` parseable as pure `WorldPredicate`s or registered named checks (`world.state-aware-capability-availability`, File 18 §9.3), and the contract introducing no private substrate that §12 forbids. A `surface_id` that collides with a tombstoned surface is reclaimable only by the same `registration_source` and identity that registered it (§4.5), and the reclaiming registration continues that surface's version history rather than forking a new lineage; a collision with an active surface id, or with a tombstoned id from a different source, is rejected.

Plugin and user-defined surfaces may register as an atomic package containing the `SurfaceContract`, contributed capabilities, custom kinds, unit kinds, presentation contributions, settings definitions, and instruction sources. Required references must resolve against the existing registry or the same package transaction. Optional references may enter an `Unavailable` diagnostic state. If required validation fails, no partial registration is committed. Material contract updates from plugin or user-defined sources pass through source approval when they change capabilities, unit kinds, model-request contributions, sensors, sandbox profile, policy defaults, or other security-relevant declarations.
...
Surface activation has two meanings that must stay separate: presentation activation, where the user focuses or opens a surface in a presentation context, and execution binding, where a run uses the `primary_surface` selected by routing or explicit reroute. The shell relationship is the relationship the core owns between the always-available control rails, the primary work context, and the projections the core exposes to any attached context; a context binds each of them to a presentation role it declares it serves (`ui.shell`, File 37 §4.2), and to whatever affordance its medium has for that role. Multi-surface composition is how supporting surfaces and borrowed capabilities compose without changing a run's bound primary surface.

### 11.2 Activation

- Routing selects the run's `primary_surface` (`routing.run-intent`, File 03 §4.3, `routing.surface-capability-selection` File 03 §8), or resolves no work surface at all — discovery, capability-level, or conversation-baseline work, where `primary_surface` is absent. That execution binding selects the run's active `SubsystemSurfaceSpec` for tool-surface composition (`surface.subsystem-surface-spec`, File 07 §5), plus the surface defaults consumed by context, model, execution, and sandbox layers; when no work surface is bound, the run falls back to conversation — the always-available control rail and continuity surface (§1.1) — and composes against the always-present baseline `SubsystemSurfaceSpec` (`surface.subsystem-surface-spec`, File 07 §5) rather than a specific surface's spec. Changing that binding during execution requires File 03/File 04 reroute or explicit user override; opening or focusing a different surface in the UI does not silently change an existing run.
- User presentation activation opens or focuses a work surface for a conversation, workspace, or session, within the `PresentationContext` the gesture acts in (`world.surface-state`, File 18 §5.1). It updates live `SurfaceState` and the context's presentation (File 18), and may influence future routing or user-invoked capability resolution, but it does not rewrite existing run binding. Invocation lenses resolve against the focused surface of their own presentation context for that invocation, without changing the active run's execution context (realized in the windowed-desktop profile as the shell window the gesture acts in, §11.6).
- Activation is scope-resolved. Surface state, focus, and selection are resolved per conversation, run, or session and, within a scope, per `PresentationContext` (`world.surface-state`, File 18 §5.1, `world.exposure-consumption` (File 18 §11.5)); run binding is resolved per run. Multiple surfaces, sessions, and presentation contexts may be active concurrently; there is no single global active surface. A consumer resolves the relevant surface for an explicit scope, presentation context, and purpose.
...
### 11.3 The Shell Relationship

The core owes three guarantees to every attached presentation context, whatever its medium: one primary work context resolvable for the scope that context presents; invocation reachable through the control rails (`core.system-layers`, File 01 §2.1) registered for the entry classes the context's client session declares — directly, or through the profiles it declares (`controlrail.registry`, File 26 §14.2) — without displacing or discarding that work context; and typed projections of run observability (`run.presentation`, File 04 §25) and of the artifact pool (`artifact.per-surface-projections`, File 09 §17.2) available to any context that asks for them. A context binds each guarantee to a presentation role it declares it serves (`ui.shell`, File 37 §4.2); a context serving every role is a full context, and a context serving a declared subset never claims the set. A session declaring no profile is owed the three guarantees and is bound by none of the regions. Conversation is available as a control rail and presentation view; it is not the universal container. The same work may move between conversation-first and a work-surface focus over time without changing the work model (`intent.presentation`, File 02 §8). This file owns the shell relationship; UI specs own how a medium realizes it, including composition and navigation (realized in the windowed-desktop profile as a five-region shell, §11.6).
...
- A surface may present another surface's unit as a borrowed projection (a Coder surface presenting a `browser` unit) without a primary-surface change; the borrowed unit self-registers its state to the one world model (§5.4). An invocation from a borrowed unit carries the unit's `surface_binding` as invocation context for capability resolution and ledger attribution, but does not change the active run's execution context. The run's primary surface, context policy, model profile, budget, and sandbox profile remain the host surface's unless a formal reroute occurs through File 03/File 04. The unit binding affects which capabilities are available for that invocation; it does not affect how the run executes.
...
Realizes: §11.3. The application shell is the relationship: persistent control rails (conversation, command palette, voice, shortcuts — always available, `core.system-layers` File 01 §2.1), a focus surface, secondary inspectors, execution observability projections (`run.presentation` File 04 §25), and artifact-pool projections (`artifact.per-surface-projections` File 09 §17.2). Those five regions are this profile's binding of §11.3's three guarantees to the presentation roles it declares it serves (`ui.shell`, File 37 §4.2).

Realizes: §11.3. In this profile conversation is an always-available control rail and an expand/collapse view, never forced to occupy the primary pane; that expand/collapse view is this profile's binding of the invocation guarantee, and no surface restates it as a rule of its own.

Realizes: §11.2. A shell window is this profile's realization of an `Interactive` presentation context: the window a gesture acts in is the context that gesture's invocation resolves against, and two windows are two presentation contexts of one client session, each with its own focus, unit and selection state (`ui.shell`, File 37 §4.5).
...
A `WorkSurface` owns only its user-facing workflows, its specialized presentation units and views, its contributed capabilities, and its declared defaults. It must reuse, never privately reimplement, all of the following, through each one's canonical contract:
...
- **world model and state awareness** — the surface self-registers its presentation units and state to the one world model; no private state store, no screen-scraping of its own state (`world.consequences-for-later-specs`, File 18 §17).
...
A surface returns typed cross-boundary failures (`core.typed-errors`, File 01 §6.9); its user-facing strings are i18n keys, never hardcoded (`ui.i18n`, File 37 §15), and every presentation attribute it declares is a reference into the medium's declared token vocabulary rather than a literal value (`customize.design-tokens`, File 38 §4) — those two are what this file requires of every surface in every medium, and the token families, the themes over them and their per-medium mechanics are File 38's; its capability invocations carry per-call attribution keyed by model and provider identity (`ledger.per-call-model-call-attribution`, File 10 §6), so a surface's cost is accounted through the one mechanism, never a private scalar.
...
- **Progressive disclosure** — the simple-to-power-user spectrum — comes from which presentation units, views, and affordances are exposed in the current presentation (§7), not from a per-surface mode. The agent adapts to what is asked and to the live `SurfaceState`, not to a pre-declared mode (`cross-cutting/state-awareness.md` realized).
...
- A `ViewPreset` (§7.2) is a presentation-only startup intent, explicitly not an autonomy mode. There is no `ParticipationLevelChanged` event because there is no participation level to change (`world.state-change-events-reactivity`, File 18 §12).
...
A substrate-service management surface is any user-facing presentation of a substrate service's data and capabilities that is not a focus-presenting work environment; a memory browser, routing inspector, context-assembly inspector, settings inspector, knowledge-base browser, evaluation dashboard, capability-registry inspector, source manager, MCP manager, storage-accounting view, or world-model inspector are example realizations of that class, never its definition. It is not a primary work surface; it is a management presentation of an always-on substrate service.
...
- A management surface presents units, views, and control affordances, and three obligations bind it unconditionally the moment it presents anything at all: every unit it presents is a `PresentationUnitKind` projection that self-registers its live state to the one world model (§5), enforced at self-registration (`world.observation-state-update`, File 18 §8.1); every action it offers is a capability in the one registry (§6), enforced at capability registration (`capability.runtime-mutation`, File 05 §16.2); and every output it produces is a block in the one pool (§12) — an obligation no gate enforces, because there is no second pool to write to. It is classified as a substrate-service management presentation, not a `WorkSurface`. It does not register a `SurfaceContract`, does not enter the `SurfaceRegistry`, does not claim the focus-surface or workspace-first shape, does not register a `surface_kind` from the work-surface baseline, and a conversation never "binds to" a management surface the way it binds to a workspace. This file creates no second management-surface registry.
...
- A `WorkSurface` and the units it presents are `WorldEntity`s in the one world model (`world.world-entity`, File 18 §4.3 — `Surface` and `PresentationUnit` entity kinds). A surface self-registers its units, focus, selection, and state to the world model when a unit is activated and on focus, selection, and content change, and unregisters a unit when it is deactivated (`world.observation-state-update`, File 18 §8.1); a surface that fails to register its state is a blind spot the agent cannot use (`world.chosen-model`, File 18 §1). A surface's own state is learned through self-registration, never through perception screen-scraping (`perception.tiered-sensing`, File 19 §5.4).
...
- A surface declares the `ObservationKind`s it produces (`artifact.observation`, File 09 §13.2); process and sandbox snapshots a surface's runtime depends on are `Observation` blocks through the canonical path (`process.observation`, File 23 §14, `artifact.consequences-for-later-specs` (File 09 §22)). The active surface, the focus target, the active units, selection, available capabilities, and ui mode are the world snapshot routing, policy, tool-surface composition, and context assembly already consume (`world.exposure-consumption`, File 18 §11.3).
...
- A surface's durable state — its registered `SurfaceContract` versions, registered custom kinds, scoped enable state, scoped settings, and the blocks, artifacts, versions, and entities its work produces — persists as substrate families and content-addressed blobs through the one storage contract (`storage.durable-substrate`, File 20 §3, `storage.consequences` (File 20 §18)). Built-in view presets are contract declarations; a saved composition, a contribution placement, a default preset choice and every other client-authored presentation record is a settings/profile/customization record that references `surface_id`, `surface_contract_version` where needed, `unit_kind`, and the policy-owned placement identifiers the customization layer names by the role each serves (`customize.customization-policy`, File 38 §11.2) and its placement records carry (`customize.widget-placement`, File 38 §12). The record's realization payload — geometry and every other fact that means something only inside the authoring frontend profile — is opaque to this file and carries no portable identity (`customize.saved-layout`, File 38 §7.2). A surface introduces no private durable store. A surface's live state — its active units, focus, selection, runtime handles, the materialized presentation — is computed and rebuilt from self-registration and the version-graph projection, never a durable fact (§15, `world.persistence-contract` (File 18 §14.2)); its loss is a rebuild, never data loss (`core.projection`, File 01 §6.11).
- A surface's identity splits by locality the way a workspace's does (`workspace.locality`, File 24 §4, `storage.physical-layout-locality` (File 20 §8), `settings.locality-sync-export` (File 15 §18)): the surface's *logical* declaration and identity — its `surface_id`, contract versions, `surface_kind`, contributed capabilities, built-in view presets, and syncable settings — are portable and may sync when their owning specs classify them syncable (`portability.what-replicates`, File 21 §5.3); the surface's *device-local runtime* state — its active units, live handles, materialized presentation, and device-bound facts — is device-local and rebuilds per device and never syncs. World facts a surface produces are device-local by default (`portability.what-replicates`, File 21 §5.3 — displays, windows, sandboxes, browser sessions, capture state never sync).
...
- The work-surface layer exposes canonical capabilities through the one Capability Registry (`capability.declaration`, File 05 §3), declared as built-ins in the `Builtin` source, tier-gated by policy (File 06), surfaced through tool-surface composition (File 07), and invoked through the shared pipeline (`run.call-pipeline`, File 04 §8.2). Surface-management capabilities declare touched resources and permission floors by effect:
  ...
  - transient activate, deactivate, focus, and presentation-activation operations are presentation-state writes scoped to conversation/workspace/session, and a change to the presented composition is one of them; File 06 resolves the effective tier from touched resources and policy
  - saving or deleting user view presets, saved compositions, contribution placements, and customization preferences is a settings/profile/customization write
...
- Every surface-management capability is the single source for all its invocation paths — every registered control rail, the agent's tool surface, an automation trigger, an external protocol (`core.extension-planes`, File 01 §6.14); this file declares no out-of-band surface operation. Custom surface operations register through the proposal-first mechanism (`capability.runtime-mutation`, File 05 §16.2) and never bypass the policy layer.
...
- The work-surface layer emits typed events through the one event bus and ledger (`ledger.event-stream`, File 10 §5) with the canonical envelope (`ledger.event-envelope`, File 10 §5.2). Surface-relevant tool-surface events (`ToolSurfaceComposed`, `PrimarySurfaceChanged`, `SubsystemSurfaceSpecUpdated`, `SourceConnected`, `SourceDisconnected`) are owned by `surface.surface-relevant-events` (File 07 §13) and flow through that vocabulary; this file emits them when a surface's tool surface changes. Live surface-state events (`UiUnitRegistered`, `UiUnitUnregistered`, `UiPrimaryUnitChanged`, `UiSelectionChanged`, `UiModeChanged`, `UiAvailableCapabilitiesRecomputed`) are owned by `world.state-change-events-reactivity` (File 18 §12) and emitted by the world model from a surface's self-registration; this file consumes them.
- Surface-lifecycle facts this file owns register as `Custom { namespace: "surface", name, payload }` extensions (`ledger.custom-kind-registration`, File 10 §4.3): `SurfaceRegistered`, `SurfaceUnregistered`, `SurfaceTombstoned`, `SurfaceEnabledChanged`, `SurfaceAvailabilityChanged`, `SurfaceContractVersionRegistered`, `ViewPresetSaved`, and `ViewPresetDeleted`. Each declares its payload schema, cross-reference keys, default sensitivity, retention, and owner per File 10. This file reserves the `surface` namespace and declares these kinds. Run primary-surface changes use File 07 `PrimarySurfaceChanged`; live focus and unit changes use File 18 state events; applying a saved view preset is `ViewPresetApplied`, owned by `settings.events-snapshots` (File 15 §14.1) as a settings change and `ui.events` (File 37 §21.1) as a presentation fact; this file does not duplicate them.
...
- **A participation-level, autonomy-mode, interaction-shape, persona, or phase field on a surface, view preset, or surface state** — autonomy is permission tiers and leases plus user direction, progressive disclosure is which presentation units and affordances are exposed and which view preset is active, and interaction shape is a presentation lens varied freely by the UI (§13; `core.interaction-shapes`, File 01 §2.2; `core.explicit-rejections` (File 01 §8); `world.surface-state` (File 18 §5.5)).
...
- **Plugin, user, or AI UI injection outside declared customization policy** — customization is allowed broadly, but it must pass through declared `customization_policy`, capability policy, source trust, proposal-first mutation, and reversible or recoverable commit semantics (§7.4). Direct UI injection, direct Atlas core UI mutation, or untracked contribution or unit placement is invalid.
- **A presented unit or affordance that cannot be represented structurally** — every presentation unit and control affordance a surface presents, in any modality, must expose semantic roles, labels, interaction kinds, and state relationships sufficient for the world model and the control rails (§5.3, §6.4). How a medium presents them may vary; a presentation-only artifact with no structural representation is invalid.
- **A `ViewPreset` that silently changes backend policy** — presentation presets may reference settings/profile bundles, but applying a preset must not silently change model selection, context policy, execution entry, budget, sandbox profile, approval posture, or instruction-source authority (§7.2).
- **A presentation-focus change treated as execution reroute** — opening, focusing, or borrowing a surface or a presentation unit may affect presentation and invocation context, but an active run's primary surface and execution context change only through File 03/File 04 reroute or explicit user override (§11).
...
- **Conversation forced as the mandatory container of work** — conversation is an always-available control rail and a presentation view whose prominence the user controls in whatever medium presents it; the primary work context is whatever the work needs (§11; `core.product-thesis`, File 01 §1; `intent.presentation` (File 02 §8)).
...
- The **per-surface specs** each declare a complete `SurfaceContract` to the shape this file fixes (§4): identity and `surface_kind`, `PresentationUnitKind`s and selection kinds (§5), `SubsystemSurfaceSpec` and contributed capabilities and artifact, observation, and sensor kinds (§6), `ViewPreset`s, customization policy, and declared inspection surfaces (§7), and default context, model, execution, budget, sandbox, and workspace policies (§8, §9). They own specialized workflows, presentation units, runtimes, and capabilities, reuse every substrate through its canonical contract (§12), introduce no private architecture, and declare no participation-level or autonomy field (§13). Their history, comparison, and rollback views are projections over the one version graph; their files materialize through the one workspace mirror; their confined execution runs through the one `Sandbox` contract.
- The **Control Rails** spec defines the control rails — each registered with the entry class it offers and the gesture grammar it accepts — that invoke a surface's capabilities and compose with the primary work context (§11), consuming the `WorkSurface` and `SurfaceContract` this file defines without redefining them.
...
- The **Telemetry, Logging, and Observability** spec consumes the surface events and the per-call attribution this file and File 10 emit; the **Runtime Infrastructure and Lifecycle** spec orchestrates surface startup registration around the storage lifecycle File 20 owns; the **Evaluation and Benchmarking** spec verifies surface activation, cross-surface composition, the no-private-architecture invariant, and the morphing-as-projection round-trip — that one (`SurfaceContract`, live `SurfaceState`, routing decision) triple projects to one presentation description (§7.3) — over recorded snapshots, not live state.

## 22. Canonical Rule Anchors

Anchor: `worksurface.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `worksurface.chosen-model`, `worksurface.surface-disambiguation`, `worksurface.boundaries`, `worksurface.work-surface`, `worksurface.surface-contract`, `worksurface.state-declaration`, `worksurface.actions-declaration`, `worksurface.views-presets`, `worksurface.context-model-declaration`, `worksurface.runtime-execution-declaration`, `worksurface.registry`, `worksurface.activation-shell`, `worksurface.no-private-architecture`, `worksurface.no-autonomy-field`, `worksurface.management-surfaces`, `worksurface.world-perception-integration`, `worksurface.persistence-locality`, `worksurface.capability-surface`, `worksurface.events`, and `worksurface.settings`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
