...
- a **substrate service** (`core.system-layers`, File 01 §2.4) — Memory, routing, context assembly, knowledge, settings, world model, perception, storage. A substrate service may expose a management or inspector surface (§14) but is not a focus-presenting work surface.
...
This section owns the surface registry and lifecycle. File 05 owns the proposal-first registration mechanism and the subsystem-registration path; File 06 owns source approval; File 22 owns trust establishment; File 20 owns the physical persistence and the surface-directory placement; the future Extension and Plugin spec owns the plugin install lifecycle a plugin surface rides; the future Runtime Infrastructure spec orchestrates the startup registration. This file declares the registry contract; those files realize it.
...
A substrate-service management surface is a user-facing presentation of a substrate service's data and capabilities — a Memory browser, a settings inspector, a knowledge-base browser, a capability-registry inspector, a source manager, an MCP manager, a storage-accounting view, a world-model inspector. It is not a primary work surface; it is a management presentation of an always-on substrate service.

### 14.2 Rule

- Substrate services (Memory, routing, context assembly, knowledge, settings, world model, perception, storage, the capability registry, policy) are the subsystem flavor that is always available to every work surface and not shaped like a focus-presenting work surface (`core.system-layers`, File 01 §2.4, `memory.chosen-model` (File 14 §1)). A substrate service may expose management, inspector, browser, proposal, and graph surfaces (`core.current-major-area-classification`, File 01 §5.8); those are management surfaces, not work surfaces.
- A management surface presents panels, views, and control affordances and, where it does, follows the same declaration discipline this file fixes — its panels are `PanelKind` projections that self-register to the one world model (§5), its actions are capabilities in the one registry (§6), its outputs are blocks (§12) — but it is classified as a substrate-service management presentation, not a `WorkSurface`. It does not register a `SurfaceContract`, does not enter the `SurfaceRegistry`, does not claim the focus-surface or workspace-first shape, does not register a `surface_kind` from the work-surface baseline, and a conversation never "binds to" a management surface the way it binds to a workspace. If a future UI presentation spec defines a separate presentation-surface declaration, management surfaces may declare through that UI-owned contract; this file creates no second management-surface registry.
- Memory is the canonical example: it is a substrate service "always available to work surfaces but not shaped like a workspace-first surface" (`memory.chosen-model`, File 14 §1), with management surfaces (the memory browser, proposal review, influence inspection, `memory.privacy-sensitivity-user-control` File 14 §18) that every work surface reads from and contributes to, and no surface receives a private memory model (`memory.chosen-model`, File 14 §1). The inspector lens (`surface.inspector-lens`, File 07 §12.4) is a management surface over the capability registry; the routing inspector (`routing.minimum-visible-information`, File 03 §10.2) is a management surface over routing.

### 14.3 Boundary

This section fixes the management-surface-versus-work-surface boundary. The substrate-service specs (File 14 Memory, File 12 Knowledge, File 15 Settings, and the management lenses of Files 03, 05, 07, 18, 20) own their management presentations' content; UI specs own their rendering. This file classifies them and applies the shared substrate discipline where they present panels; it does not make them work surfaces.
...
- **Treating a substrate-service management surface as a primary work surface** — Memory, settings, knowledge, the registry inspector, and equivalent management surfaces are presentations of always-on substrate services, not focus-presenting work surfaces (§14; `core.current-major-area-classification`, File 01 §5.8; `memory.explicit-rejections` (File 14 §21)).
