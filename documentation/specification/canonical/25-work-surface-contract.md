...
- User presentation activation opens or focuses a work surface for a conversation, workspace, or session, within the renderer root the gesture acts in (`ui.shell`, File 37 §4.5). It updates live `SurfaceState` and shell presentation (File 18), and may influence future routing or user-invoked capability resolution, but it does not rewrite existing run binding. Invocation lenses such as command palette, shortcuts, voice, or UI controls resolve against the invoking root's focused surface for that invocation — its explicit `PresentationContext` (File 18 §5.1) — without changing the active run's execution context.
- Activation is scope-resolved. Surface state, focus, and selection are resolved per conversation, run, or session and, within a scope, per `PresentationContext` (`world.surface-state`, File 18 §5.1, `world.exposure-consumption` (File 18 §11.5)); run binding is resolved per run. Multiple surfaces, sessions, and renderer roots may be active concurrently; there is no single global active surface. A consumer resolves the relevant surface for an explicit scope, presentation context, and purpose.
