...
- `task_template` — the pinned `RunIntent` template (§6.3): the operation the automation performs when fired. It references a saved `RunIntent`, a workflow identity owned by File 34 (Workflows, Templates, and Reuse), or an inline prompt, with parameter slots.
