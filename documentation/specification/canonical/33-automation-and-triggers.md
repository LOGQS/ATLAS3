...
- the `RecurrenceRule` contract and the **event-first timing rule**: a `Schedule` trigger computes its next-fire instant as a pure function and arms a single timer to that instant; it never busy-polls a clock, honoring `core.workspace-model` (File 01 §3)'s no-polling constraint and `world.consequences-for-later-specs` (File 18) — current time is grounding, not a scheduler
...
- the reusable workflow or template **body**, parameterization grammar, template composition, or the template library — the future Workflows, Templates, and Reuse spec owns those; an `Automation` references a task template (a pinned `RunIntent`, a workflow identity, or an inline prompt), it does not own the workflow body
...
- the secret vault, credential lifecycle, the trust model, encryption, egress governance, the untrusted-content rule, or webhook/MCP transport mechanics — Files 22 and the future MCP and External Integrations spec own those; this file specifies the trust posture of an inbound trigger and references the vault
...
- UI rendering — the automations dashboard layout, the schedule editor, the trigger-config pane, widget chrome, notification presentation, and accessibility — the future UI specs own those; this file specifies the data and resolution contracts they consume
...
- A `Schedule` trigger does not busy-poll a clock. The `Scheduler` computes the trigger's next-fire instant and arms a single timer to that instant; when the timer elapses, the trigger fires and the next instant is recomputed and re-armed. The clock is read to compute a deadline, not evaluated continuously as a condition. This honors `core.workspace-model` (File 01 §3)'s no-polling constraint and `world.consequences-for-later-specs` (File 18) — current time is grounding, not a scheduler.
...
- A `Webhook` trigger declares how source deliveries produce a `source_event_id` and freshness or idempotency signal: provider event id, signed timestamp plus nonce, delivery id, or deterministic content key. Duplicate source deliveries map to the same `fire_id` and are recorded as duplicates, not re-run. Remote bindings require freshness validation appropriate to their authentication scheme. The future MCP and External Integrations spec owns concrete signature protocols; this file owns the no-duplicate-fire behavior.
- The endpoint binding and its verifying secret are device-local (§18). The transport mechanics — the listener, the protocol surface, the connector lifecycle — are the future MCP and External Integrations spec's; this file owns the trigger's existence, its authentication requirement, and its trust posture.
...
File 10 owns the bus and the event catalogue; File 24 owns the filesystem watcher; File 19 owns perception signals; File 22 owns the trust and secret contracts; the future MCP and External Integrations spec owns webhook transport. This section owns how those signals become trigger fires.
...
- `task_template` — the pinned `RunIntent` template (§6.3): the operation the automation performs when fired. It references a saved `RunIntent`, a workflow identity owned by the future Workflows spec, or an inline prompt, with parameter slots.
...
- An `Automation`'s `task_template` is a `RunIntent` template captured at save time. It pins the fields that routing would otherwise compute at fire time, so a later run is reproducible and not subject to drift: the primary surface and the recorded `surface_contract_version` (`worksurface.consequences-for-later-specs`, File 25), the `tool_surface_strategy` (`surface.presentation-in-user-facing-surfaces`, File 07 §12), the model route or profile (`model.model-selection-algorithm`, File 16 §4), the context and compaction policy (`context.context-policies`, File 13 §4), the sandbox profile (`sandbox.contract`, File 23 §3), and the budget (`run.budgets-limits`, File 04 §21).
...
File 04 owns the `RunIntent` field set and the automation-reuse proposal; File 03 owns pin-through routing; File 07 owns the tool-surface strategy; the future Workflows spec owns the workflow body the template may reference; File 08 and File 11 own the block and version graph the definition lives in. This section owns the automation object that binds them.
...
- **Arming.** For each enabled `Schedule` trigger, compute the next-fire instant and arm a single timer (§3.3). For each enabled `WorldCondition` and `Event` trigger, establish the world-model subscription (`world.watch`, File 18 §13.1) or bus subscription (File 10). For each `Webhook` trigger, register the inbound endpoint (§5.2).
...
File 03 owns routing; File 04 owns the run model and the execution-graph; File 02 owns conversations and intent threads; the future Workflows spec owns the workflow body. This section owns the trigger-to-run handoff and the target binding.
...
- **Promotion** — a recorded macro (`web.artifacts`, File 28 §11; `gui.macros`, File 31 §10) or a workflow template (the future Workflows spec) is promoted into an `Automation` by binding a trigger and the governing fields.
...
File 04 §26 owns the reuse proposal trigger; File 06 owns the approval and source-approval flows; the future Workflows spec owns the template the automation may be promoted from; Files 28 and 31 own the macros. This section owns the automation-creation contract.
...
File 10 owns the ledger and the run records; the future UI specs own the dashboard, inspector, and widget rendering. This section owns the observability data contract.
