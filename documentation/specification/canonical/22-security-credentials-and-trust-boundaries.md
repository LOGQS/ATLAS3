...
File 10 owns the `Public`/`Sensitive`/`Secret` taxonomy (`ledger.sensitivity-aware-persistence-retention`, File 10 §10), the rule that raw `Secret` payloads never persist to the durable ledger, the producer-seeded sensitivity stamping (`ledger.producer-seeded-sensitivity`, File 10 §10.2), the commit-time forgery guard that rejects any ledger representation that would persist raw `Secret` material while permitting File 10's redacted structural Secret entry (`ledger.forgery-guards`, File 10 §3.7), and the hash-chained audit overlay's structure, membership, and verification contract (`ledger.hash-chained-audit-log`, File 10 §16). This file owns the secret-detection patterns that drive the automatic stamping, the cryptographic primitive the audit chain hashes with, and the credential/secret operation entries File 10 §16.4 reserves for this file to register.
...
- **Forbidden destinations.** Raw `Secret` material must never be deliberately emitted, persisted, or transported by Atlas to the frontend or renderer process, the JavaScript heap, an IPC payload, model-request context, a log, an event payload, a ledger entry, telemetry, a settings value, the TOML overlay, block content, a retrieval index, a projection, an ordinary sync stream, an ordinary export package, the clipboard, or an ordinary substrate backup. Operating-system memory images are a distinct residual boundary: Atlas suppresses core-dump, swap, and hibernation routes where the platform permits; any route that remains open is governed by the suppression, observed-route, and residual-posture rules below and is never represented as closed.
...
- **Memory-to-disk suppression.** Because the operating system can spill process memory to disk, the backend suppresses the paths it can close from inside the process. The realizations, named for grounding and replaceable behind one process-hardening contract, are `PR_SET_DUMPABLE=0` together with `RLIMIT_CORE=0` on Linux, `RLIMIT_CORE=0` on macOS, and Windows Error Reporting exclusion of the running secret-holding executable on Windows. On Linux and macOS these are process-local and close the operating system's own core-dump path; no anti-debugging control is added, because the threat model does not defend against a local code-execution adversary (§3.3). Whether a named platform mechanism has been observed to behave as its documented interface promises is an implementation-verification obligation carried against that mechanism, not a claim this section makes. Memory locking (`mlock`/`VirtualLock`) may pin secret buffers out of the ordinary swap file, but it is best-effort with an explicit residual risk: it does not close hibernation, which images all of RAM to disk, so it is never presented as closure of the memory-to-disk exposure. Zeroization is the primary in-memory defense; these paths reduce the residual disk-spill surface, they do not replace it.
- **Observed operating-system memory-image routes.** Atlas inspects the registered platform routes that can image the secret-holding process independently of its ordinary crash-report suppression. On Windows the observed route set includes effective `LocalDumps` policy and both applicable registry views of the `AeDebug` postmortem debugger; other platforms register their corresponding route descriptors through the same typed set. Inspection is read-only, and Atlas never mutates or removes an administrator's or a user's diagnostic policy as a side effect. The normalized route set is device-local security state and contains safe descriptors only. Swap and hibernation are not members of this set: they remain the admitted best-effort residual of the preceding rule, not a startup gate.
- **Residual-route posture.** `security.memory_dump_residual_strictness` (§17.1) resolves to `Disclose`, `ConfirmOncePerMachine`, or `RefuseSecretCapableStartup`, with `ConfirmOncePerMachine` the safe default. An observed open route never passes silently: it is disclosed, acknowledged for that exact machine and route set before vault unlock, or causes secret-capable startup to be refused, according to the resolved posture. A session in which the vault never unlocks holds no vault-resolved secret; that is not a claim that the process holds no raw secret at all, since a bootstrap passphrase may be inherited in the process environment before startup, and that residual is disclosed rather than asserted away.
- **The acknowledgement record.** `MemoryImageRouteAcknowledgement` is the durable device-local acknowledgement `ConfirmOncePerMachine` consumes. It carries `acknowledgement_id` (the canonical hash over its identity fields), `device_id`, `route_set_hash` over the sorted safe `observed_routes`, those `observed_routes`, the `strictness` in force, `setting_snapshot_ref`, `actor`, `accepted_at` (display only, never validity), and `confirmation_evidence`. It is registered as `atlas.security.MemoryImageRouteAcknowledgement` v1 through `ledger.custom-kind-registration` (File 10 §4.3) as a device-local `Custom { namespace: "security" }` ledger fact with transactional identity uniqueness, and it is never synced and never exported (§11.5). An acknowledgement is valid only on an exact match of `device_id`, `route_set_hash`, and `strictness`; elapsed time is never part of its validity, so a changed route set or a tightened posture requires a new acknowledgement. File 20 realizes its storage; the security semantics are this file's.
...
- Under `ConfirmOncePerMachine` (§4.3, §17.1), an open observed memory-image route with no matching device-local `MemoryImageRouteAcknowledgement` keeps the vault in the typed deferred `VaultLocked` state until the acknowledgement is recorded. Non-secret work continues; secret-dependent work receives `VaultLocked`, and the unlock request such an operation raises against a locked vault routes to the acknowledgement rather than to a passphrase prompt — it is not grantable while the observed route set is unacknowledged. A changed observed route set requires a new acknowledgement. Under `RefuseSecretCapableStartup`, the runtime stops before opening or unlocking the vault. Under `Disclose`, the residual risk remains surfaced but no per-route acknowledgement is required, because selecting that weaker posture already passed the safety-direction typed-confirmation rule (§17.1). The gate is evaluated at every transition from a locked vault to a secret-capable unlocked state, not only during process startup: an eager, on-demand, deferred, or user-initiated unlock re-inspects the observed route set and applies the resolved posture before any session key is derived.
...
- `security.memory_dump_residual_strictness` — `Choice { Disclose < ConfirmOncePerMachine < RefuseSecretCapableStartup }`, default `ConfirmOncePerMachine`, `allowed_scopes: [Global]`, `locality: DeviceLocal`, posture-level agent exposure only. A move toward `Disclose` is `widening`; a move toward refusal is `narrowing`.
