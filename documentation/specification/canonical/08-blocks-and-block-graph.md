...
- is immutable after creation: EVERY field of the committed record (§2.2) is fixed at the moment the block is committed and never changes — identity, kind, content, parent, producer, the run/conversation/scope anchors, hashes, attribution, sensitivity, description, and schema version alike, never a subset. The facts that do change — lifecycle, pin state, sequence within a view — are VIEW state owned by the version graph (§6), not block fields
...
- is durable: blocks persist across run boundaries, conversation archival, process restart, and version-graph rewrites; the only operation that destroys a block's storage is explicit user-initiated hard deletion (§6.6). File 11 §20.5's version-payload deletion destroys VERSION-record payload and never reaches block storage — its payload scope excludes the block pool by rule, stated there
...
- `partial_orphan` — `Option<PartialOrphan>`, the §7.3 promoted-partial marker: present exactly when the block was promoted from a staged partial after producer failure; presence-only (the stranding cause — cancellation, error, timeout, crash — lives in the producing run's ledger facts, reached through `origin_run_id`, never duplicated here); fixed at creation
- `sensitivity_field_map` — the optional per-field sensitivity override map by canonical field path (§9.2); fixed at creation
...
- `CompositionSnapshot` — a system-produced, semantically lossless snapshot of a `Composed` block's fully resolved content, committed by the hard-delete flow when a `RetainResolvedSnapshot` disposition preserves a parent's resolved representation while a required child's payload is destroyed (§6.6). May be `Inline` or `External` per the inline threshold; never `Composed`; never a transcript anchor (the original parent remains the anchor). Not a consolidation (nothing is condensed), not an observation or excerpt; linked to its superseded parent by `materialized_by` (§5.2). Seeds `Sensitive` by default (fail-closed; the committed block carries the source parent's pre-delete effective sensitivity — raised across descendants, never lowered). The preservation it provides is retained content — explicitly not credential/secret expungement
...
- `materialized_by` — preservation carriage for the parked §6.6 disposition-bound hard-delete plan. Source: the superseded or dangling `Composed` parent; target: its preservation carrier (the `RetainStructure` exact child copy, or the `RetainResolvedSnapshot` `CompositionSnapshot`). Carries typed metadata identifying the replaced child, the chosen disposition, and the occurrence scope, and supplies historical fallback resolution only for parents whose confirmation disposition preserved content (hard deletion is irreversible for older versions, `version.version-switching`, File 11 §8.4 — the carrier is what an older version resolves through). An exact child copy additionally carries `derives_from` to the original block's tombstone with an `ExactRetentionCopy` transformation summary; the original's other edges stay orphan-marked (§6.6), and only outgoing relations required for the copied kind's validity are recommitted, auditably linked to the copy operation. The mechanism is currently unsupported (§6.6); the edge kind is defined now so the future resolution needs no catalogue change
...
- `Recover(block_id)` — transitions a `Masked` or `Dropped` block to `Recovered` in the current view (`Recovered` is semantically `Active` carrying the historical mark, §6.7 — File 11 §11 catalogues the same target). The block's appearance in surface presentations resumes; retrieval indexing re-enables
- `Unmask(block_id)` — transitions a `Masked` block directly back to `Active` (no historical mark). `Recover` answers "bring it back and remember it left"; `Unmask` answers "the mask was provisional" — the two returns are distinct operations, both catalogued by File 11 §11
...
Both are committed as version-graph entries emitting the registered `BlockGrouped`/`BlockUngrouped` ledger events (`ledger.entry-kind-catalogue`, File 10 §4.1). Re-grouping after an ungroup creates a NEW group block with a fresh `block_id` — group identity is never reused across a dissolve, the same redo-identity rule File 11 §12 applies to re-done view operations.
...
- recorded in the execution ledger as a `BlockHardDeleted` event whose payload is File 10 §4.1's enumeration — the deleting actor, the typed `deletion_reason` (File 11 §20's enum, never free text), the tombstone reference, the references that would be orphaned, and the per-parent disposition outcome; this file defers the payload shape to File 10 rather than restating it
- accompanied by a minimal tombstone retaining `block_id`, deletion time, deletion actor/source, `conversation_id`, `scope`, `origin_run_id` and `scope_anchor` exactly as committed, `parent_block_id`, prior kind if safe, and the typed `deletion_reason` with an optional sensitivity-safe description. `origin_run_id` and `scope_anchor` are the §8.2 rule-11 anchoring pair, validated by the same rule-11 matrix at tombstone write, and exist so §11.3 evaluates a tombstone's visibility through exactly the rules that governed the live block — without them, four of the seven scope kinds (`run`, `workspace`, and the anchored `intent_thread`/`task` forms) would be visibility-undecidable after deletion and §11.3's two outcomes would collapse. The retained pair is visibility-evaluation metadata only: it never appears in the deleted-block placeholder's rendering, in snippets, logs, telemetry, or exports, and the deletion-history cleanup below removes it with the rest of the tombstone. A tombstone written under an earlier tombstone schema that lacks the pair is not visibility-evaluable and fails closed — withheld from every resolution context, never guessed. Payload bytes, secret fields, embeddings, indexed text, and external blobs are removed. References resolve to a typed deleted-block placeholder, not an unexplained missing row
- constrained by the composition-materialization outage: hard-deleting a block that a live `Composed` block — one that EXISTS in the pool and is not itself hard-deleted; lifecycle view state is irrelevant, because this rule protects storage resolution, not visibility — still depends on as a child is currently **unsupported** — the operation fails closed with a typed `Unsupported` result rather than destroying content a composed parent still resolves through, and the child's payload is retained, until the complete atomic workflow below exists. The ruled design direction (the disposition-bound preservation plan) is recorded here so the feature stays visibly parked: when a hard-delete target is contained by `Composed` parents, the typed confirmation selects an explicit disposition **per affected parent** — `Expunge`, `RetainStructure`, or `RetainResolvedSnapshot`. `Expunge` leaves that parent the typed deleted-child placeholder. `RetainStructure` mints ONE shared exact-content child copy — a fresh `block_id` carrying the original kind, content hash, and a sensitivity floor no lower than the original's committed effective sensitivity — and each retaining parent is replaced by a fresh `Composed` sibling whose child list references the copy and whose `supersedes` edge targets the prior parent (§6.2 edit semantics; the version graph swaps the active reference). `RetainResolvedSnapshot` preserves the parent's resolved representation in a first-party `CompositionSnapshot` block (§3.1), intended for bulk structural reclamation; the snapshot carries the parent's pre-delete effective sensitivity under the same never-lowered floor. Any preservation disposition is explicitly **not** credential/secret expungement — equivalent content survives under a new identity — and the confirmation preview must describe it as retained content. The per-parent default disposition is settings-owned (`blocks.hard_delete_disposition_default`, §14; default ask-every-time); a setting may preselect a disposition but never lifts the typed confirmation, and the confirmation response binds the exact per-parent plan — a changed parent-impact set invalidates it and requires reconfirmation. Destruction ordering: the copy/snapshot, its edges, the parent siblings, the version-graph swaps, the original's tombstone, and the ledger fact commit atomically BEFORE the original's payload is released; a missing preservation carrier after a preservation disposition is an integrity error (fail closed or recover), never a normal state — there is no `MaterializationOrphaned` state
- accompanied by reference-edge cleanup that is DERIVED, never a mutation: committed edges are immutable (§7), so an edge's orphan state is computed at read and closure time from the target's tombstone — closure queries report the dangling state explicitly, and nothing rewrites an edge record
...
The lifecycle state transitions are explicit and deterministic. The INITIAL state is declared by the committing operation, never inferred: `Active` when the commit includes the block in the current view (the default for transcript and standard committed blocks), `Raw` when committed without inclusion (a staged partial, a held-back import, content committed for later addressing). The complete table — this table is THE normative transition set; §6.3's operation definitions, File 11 §11's `ContextOp` catalogue, and File 10's lifecycle events realize exactly these rows and no others:
...
- `Masked → Active` — explicit `Unmask` (§6.3 — the unmarked return)
- `Masked → Recovered` — explicit `Recover` (the marked return, File 11 §11)
...
- `Dropped → Active` — explicit re-inclusion: the version-op net effect of adding a previously removed block back to the view (File 11 §12's add-after-remove), unmarked because the re-inclusion is a fresh addressing decision, not a recovery
- `Recovered → Active` — explicit mark-clearing operation (never implicit — the mark is a recorded fact and its clearing is one too)
...
- if preserved as orphan: the runtime may promote the staged partial to a partial `Block` with kind matching the producer's declared output kind, content reflecting what was streamed before failure, and the `partial_orphan` marker set (`Option<PartialOrphan>`, §2.2 — a presence-only typed record; the stranding cause stays on the producing run's ledger facts, reached through `origin_run_id`). Promotion passes the normal commit validator (§8.2), which admits the marker only from the promotion path's typed authorization — validated against the capability's declaration and the staged-partial state, never a caller-settable flag. The orphan block participates in the block graph normally and may be inspected
...
7. **Producer validation**: `producer` is well-typed and corresponds to a known producer source (capability registry for capability commits, conversation/user-id for user messages, etc.); and producer coherence admits the `partial_orphan` marker (§2.2) only on a §7.3 promotion commit carrying its typed authorization — any other commit presenting the marker fails typed
...
SHA-256's collision resistance makes practical content hash collision negligible. The system does not encode a collision-recovery path beyond logging a high-severity `ContentHashCollisionSuspected` event when two distinct blocks with the same hash are detected whose canonical content is NOT structurally equal — the structural-equality discriminator is part of the event's predicate, because identical-content blocks sharing a hash are the expected deduplication case (§4.5, §13) and raise nothing. The storage layer may use the hash for deduplication only when blocks are also structurally equal; a hash match alone is not used as identity.
...
Anchor: `block.inheritance-through-composition`
...
A block may be promoted to a broader scope through an explicit operation (a user pins a `run`-scoped observation into `conversation` scope; an agent promotes a `task`-scoped plan into `workspace` scope). Promotion is ADMISSIBLE exactly when the target scope is strictly above the source in the §11.1 visibility-containment order — a move that does not broaden along that order is rejected, never silently reinterpreted. `reusable_policy_rule` is never reached by this generic promotion: admission into it is a dedicated policy-rule/template transition that validates the block KIND and its registered policy/workflow schema before creating the promoted block or reference (an arbitrary observation, message, dataset, or artifact cannot become executable policy by a visibility move — the kind-laundering guard), even where a promote-scope command is the exposed surface (`artifact.capability-surface`, File 09 §16). Promotion creates a new immutable block at the broader scope in one of two forms, each an ORDINARY block with no new persistence category (§13.1): a content copy (a fresh block carrying the content, linked to the original by `promotes_scope_of`) or a reference record — a thin block whose content references the original by id and carries no copy, linked by `scope_projection_of` (the addressability projection; resolution follows §11.3's cross-scope rules). The original remains valid at the original scope. The promotion TIER scales with the target scope — `workspace` and below at `WorkspaceWrite`, `global` and the §11.2 policy-rule admission at `UserApproval` or stricter — the one rule File 09 §16 and File 11 §11 both cite this section for. `supersedes` is reserved for content/version replacement, not visibility broadening.
...
- per-block metadata — `block_id`, `kind`, `content`, `parent_block_id`, `producer`, `origin_run_id`, `conversation_id`, `created_at`, `block_schema_version`, `content_hash`, `source_attribution`, `default_sensitivity`, `sensitivity_field_map`, `description`, `scope`, `scope_anchor`, `partial_orphan`
- the version graph — version nodes and the lifecycle and pin ACTION LOGS; survives restart. The per-version lifecycle and pin MAPS are materialized projections of those logs (§13.2, §13.3 — durable in effect because their substrate is, never stored as a second copy; the File 11 §18 `context_view` precedent)
- block hard-delete tombstones (§6.6) — the minimal retained record that outlives its destroyed payload
- block-related events recorded in the ledger — every block commit, every lifecycle transition, every edge commit, every pin change, every group/ungroup, and every hard delete produces a ledger entry (the File 10 §4.1 registered kinds: `BlockCommitted`, `BlockLifecycleChanged`, `BlockPinChanged`, `BlockGrouped`/`BlockUngrouped`, `BlockHardDeleted`)
...
- `blocks.hard_delete_confirmation_threshold` — the typed-confirmation requirements for hard delete (per `policy.permission-floor-typed-confirmation`, File 06 §7); a per-kind override may WIDEN the requirement set, never narrow it below the §6.6 four-clause floor, and no setting lifts a typed confirmation (File 04 §29)
- `blocks.hard_delete_disposition_default` — the preselected per-parent disposition offered when a hard-delete target is contained by `Composed` parents (§6.6: `Expunge` / `RetainStructure` / `RetainResolvedSnapshot`); default ask-every-time. A preselection never lifts the typed confirmation
...
- mutable block content — every observable content change is a new block in the same pool, linked by `supersedes`. In-place mutation of ANY committed §2.2 field is invalid (one rule, no per-field enumeration to drift)
...
Anchor: `block.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `block.chosen-model`, `block.block`, `block.block-kind`, `block.kind-catalogue`, `block.kind-composition-rules`, `block.block-content`, `block.inline-size-threshold`, `block.cross-reference-vs-containment`, `block.content-hash`, `block.block-edge-block-graph`, `block.canonical-edge-kinds`, `block.block-lifecycle-non-destructive-edits`, `block.edit-semantics`, `block.mask-drop-recover`, `block.pin-protect`, `block.group-ungroup`, `block.hard-delete`, `block.lifecycle-transition-rules`, `block.streaming-commit-boundary`, `block.partial-block-orphans`, `block.live-partial-write-capabilities`, `block.commit-boundary-set`, `block.identity-validation-hashing`, `block.block-commit-validator`, `block.sensitivity`, `block.per-field-override`, `block.inheritance-through-composition`, `block.block-description`, `block.description-immutability`, `block.block-scope`, `block.scope-promotion`, `block.cross-scope-references`, `block.cross-surface-interoperability`, `block.cross-surface-composition`, `block.block-persistence-contract`, `block.what-is-durably-stored`, `block.what-is-computed`, `block.reconstruction-across-restart`, `block.reconstruction-across-retry-edit-reroute-branch`, and `block.settings`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
