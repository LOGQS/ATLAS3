...
- `scope` — the broadest scope at which the entity is addressable, drawn from the canonical scope set (`run` | `intent_thread` | `task` | `conversation` | `workspace` | `global`), aligning with `block.block-scope` (File 08 §11) and `policy.lease-primitive` (File 06 §11)
...
The world model is multi-session and multi-scope. Surface state, focus, selection, and active-work facts are resolved per conversation, run, or session; environment, display, and connection facts are typically global or workspace-scoped; workspace and repository facts are workspace-scoped. There is no assumption of a single active stream (File 07's rejection of the single-active-stream anti-pattern, `run.parallelism` (File 04 §15)). A consumer always resolves world state for an explicit scope; cross-scope reads are explicit. Events carry the scope identifiers (conversation, run, workspace) so subscribers demultiplex correctly (`ledger.event-envelope`, File 10 §5.2).
