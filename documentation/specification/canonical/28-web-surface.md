...
- the web rails: token and custom commands, the browser and researcher binding contexts, capability and source discovery, URL and page mentions, the change-monitor trigger, and the speech grammar — bound to capabilities through the control-rail layer (File 26)
...
- the `WorldModel`, the `WorldEntity` catalogue (`BrowserSession`, `BrowserPage`, `Connection`), the durability tiers, the availability evaluator, or snapshot resolution — File 18 owns those; this file self-registers its presentation units and contributes the world entities its work produces
...
- the run lifecycle, the capability-call pipeline, child-run isolation, programmatic execution, the `plan` capability, budgets, cancellation, the approval flow, leases, permission tiers, the tool-surface composition, the control-rail mechanics, the `Trigger` rail and automation, or MCP transport — Files 03–07 and 26 own the written contracts, and File 33 (Automation and Triggers) and File 36 (MCP and External Integrations) own their deeper mechanics; this file declares the web contributions and references the rules
- the storage substrate, sync, the provider/HTTP transport, the ledger row format, or UI rendering (the chrome, geometry and arrangement of a presentation unit in any medium, the browser engine, the search backend, the content extractor) — Files 20, 21, 17, 10 and the UI specs and infrastructure own those, and the windowed-desktop realization of this surface is their profile material (`ui.frontend-profile`, File 37 §1.3); this file specifies only the data and behavior contracts they consume
...
- **A browser is a world entity over a confined process, observed through perception.** A browser session is a `BrowserSession` `WorldEntity` (`world.world-entity`, File 18 §4.3) bound to a `Sandbox`-confined `ManagedProcess` (`sandbox.contract`, File 23 §3); each tab or page is a `BrowserPage` entity; the page's state is read through `Perception`'s `BrowserPage` sensor (`perception.sensor`, File 19 §4.3) structured-first (§8). The Web surface owns no private state model and is never screen-scraped to learn its own units' state (`world.observation-state-update`, File 18 §8.1).
...
Web runs use the shared run lifecycle, ledger, cancellation, budgets, programmatic execution, and child-run model (`run.consequences-for-later-specs`, File 04 §29); the deep-research workflow and parallel browsing are run structures over shared semantics, never private execution. The surface's control affordances are capabilities reachable through the rails (`controlrail.consequences-for-later-specs`, File 26 §21); it registers surface-scoped binding contexts and custom commands and owns no private rail. A page or feed change monitor is a `Trigger`-rail entry (`controlrail.chosen-model`, File 26 §1, the `Trigger` rail kind) whose firing produces a `RunIntent` (`routing.trigger-kinds-routing`, File 03 §2.1); the deep monitoring and scheduling mechanics belong to File 33 (Automation and Triggers), and this file frames the web monitor as one consumer (§12).
...
- `keywords`: the routing- and discovery-relevant terms the surface is discovered by (representative: web, browse, search, research, fetch, scrape, extract, cite, source, page, browser, monitor)
...
The Web surface declares the presentation units it can activate and the selection kinds it produces. This is the static counterpart of the live `SurfaceState` (`world.surface-state`, File 18 §5): this file declares the shape, File 18 holds the live values. A presentation unit self-registers its live state to the world model when it is activated and updates on focus, selection, and content change (`world.observation-state-update`, File 18 §8.1); a unit the surface fails to register is a blind spot the agent cannot use.

### 4.2 Declared Presentation-Unit Kinds

The Web surface declares the following presentation-unit kinds, drawn from the canonical baseline (`worksurface.state-declaration`, File 25 §5.3) plus web `Custom` kinds registered through the proposal-first mechanism. Each declares the typed shape of its compact state-field summary (a url, a query, a selected source id — never the resource body, `world.surface-state` (File 18 §5.2)), the selection kinds it produces, and its control affordances; how any of them is realized in a given medium is declared by that client's frontend profile (`ui.frontend-profile`, File 37 §1.3):

- `browser` — the presented browsing context over a `BrowserPage`; carries the active session and page id, the current url and title, navigation availability (back/forward/reload), and a `canStop` affordance; produces `Element` and `Region` selections; the browser-engine rendering is a replaceable implementation behind the unit
- `search` — the web-search projection: the active query, its result set, and its declared scope (web or academic); produces `File`-equivalent source selections and `Region` selections over result snippets
...
- `reader` — the article projection of a fetched-and-extracted page: the clean extracted content with its source span addressable for citation; produces `Region` and `Text` selections
...
- `report` — the research-report projection of a `Report`/`WebDocument`/`BrowserExtract` artifact under construction, in which sections and per-claim citations are individually addressable
- `activity` — the web activity projection: the run's browsing, fetching, extraction, and synthesis progress, a projection over the ledger and event stream (`run.presentation`, File 04 §25), not a private log
...
A presentation-unit kind the Web surface declares is a shared substrate projection, not a private view: the `browser` and `canvas` kinds are canonical baseline unit kinds and the `search` and `reader` kinds are this surface's registered `Custom` kinds (`worksurface.state-declaration`, File 25 §5.3), and another surface may present any of them without changing its primary surface (`worksurface.state-declaration`, File 25 §5.4) — a Teacher surface presenting a `browser` unit for a source.
...
The declaration is the static shape; the live values — which presentation units are active, which is primary, what is focused and selected, the current `UiMode`, the active browser session and page, and the available-capability list — are the live `SurfaceState` File 18 holds. The Web surface declares no participation, autonomy, or interaction-shape field on its state (§22); `UiMode` is interaction state, never an autonomy control, and its members and the surface-mode registration path are File 18's (`world.surface-state`, File 18 §5.5). The researcher-versus-controller distinction is a view preset (§14), not a state-machine mode.

### 4.5 Boundary

This section declares the web presentation-unit and selection shape. File 18 holds the live values and owns the self-registration contract; the UI specs own how a unit is presented in a medium, and the windowed-desktop realization of these units is their profile material (`ui.layout`, File 37 §5); the browser engine, the canvas renderer, and the reader extractor are replaceable implementations behind the units.
...
- **Discovery and orchestration** (`Primary`): the shared `plan` capability (§13.5), the discovery capabilities (`tool.borrow`, `tool.borrow_persistent`, `tool.search`, `mcp.search`, `tool.inspect` per `surface.late-loading-runtime-discovery` (File 07 §7)), and the sub-agent-spawn capability for parallel research (§9)
...
- **Default-deferred families** (the surface's `default_deferred_families`, `surface.subsystem-surface-spec` (File 07 §5.1) — excluded from the surface's default zones, still reachable by explicit search and borrow with a typed denial where policy forbids): the GUI-control desktop-automation, data-processor spreadsheet, teacher, and system-agent capability families, which are out of scope for default web composition; desktop control is the GUI Control surface's, and the Web surface drives only the browser, not arbitrary desktop applications
...
- **`Raw`** (when the capture need requires pixels): the screenshot itself, for a vision model, for visual evidence, for coordinate grounding, or for presentation by a client that presents the page visually. Capture for a vision consumer, visual evidence, or coordinate grounding is independent of the client; a client that does not present the page visually incurs no raw capture for presentation. The screenshot is a `Screenshot` `Observation` captured only when needed.

The surface must not skip an available cheaper tier when that tier satisfies the need (`perception.tiered-sensing`, File 19 §5.2): the structured representation is read first, grounded detection is added when structure is insufficient, and a screenshot is captured only for a vision consumer, visual evidence, coordinate grounding, or presentation by a client that presents the page visually.
...
The research canvas (the `canvas` unit, §4.2) is a graph projection over the research session's blocks and entities: its queries, sources, extracts, claims, clusters, takeaways, and contradictions are the graph's vertices and their relations (cites, corroborates, refutes, derives) its edges, drawn from the block graph and the evidence links (`block.block-edge-block-graph` (File 08 §5), `artifact.evidence` (File 09 §11.3)); the core exposes the graph, never a layout (realized in the windowed-desktop profile as a node-and-edge canvas, `ui.substrate-rendering`, File 37 §9). The canvas is a presentation over the shared substrate, not a private store; pinning a canvas element to memory, composing it into a report, or exporting it operates on the underlying blocks. The graph-layout renderer is a replaceable implementation behind the unit.
...
A download triggered by a browser action is materialized as a file `Artifact`: the surface routes the download through a staged or quarantined location under the sandbox/workspace boundary, normalizes the network-provided filename, prevents path traversal, classifies MIME/content where possible, atomically materializes the accepted file into the workspace downloads directory through the filesystem boundary (`sandbox.filesystem-enforcement`, File 23 §7), registers the downloaded file as a file block in the version tree (`workspace.materialization`, File 24 §10), and exposes a download record with its workspace-relative path and progress as observable run state (`run.streaming-partial-execution`, File 04 §12). Downloads to the workspace downloads directory are `WorkspaceWrite`; a web-scoped download policy may tighten this (§17). Downloaded content is untrusted source material: executables, scripts, archives, and unknown file types are never auto-opened or executed, and an unsafe or unknown download remains in a typed quarantined state carrying its policy warning until the user or policy accepts the next step. A download is a durable file artifact, never an untracked file outside the workspace.
...
- **captcha, login, payment, and auth-expiry handoff** — the elicitation rail (§16, `controlrail.elicitation` File 26 §13) under the capture-consent and privacy contract (`perception.capture-privacy`, File 19 §10); a challenge is a handoff by default, with challenge assistance available only through a separately enabled governed capability
...
The Web surface declares the presentation shapes it offers: the view presets a user can switch between, the default inspectors, the customization policy, and the activity-projection and shell affordances. A view preset is a named startup presentation shape; it is not an autonomy mode and does not silently change backend policy (`worksurface.views-presets`, File 25 §7).

### 14.2 View Presets

The Web surface ships built-in view presets — representatively a default preset (`browser`, `search`, and `sources`), a researcher preset (`search`, `sources`, `reader`, and `canvas`, for passive research), a controller preset (`browser` and `activity`, for active browsing), a research-synthesis preset (`report`, `canvas`, and `sources`, for composing a cited report), and a monitors preset (`monitors` and `activity`). A view preset declares which presentation units are active, which is primary, and the presentation-only startup state, with any medium-specific arrangement carried as optional profile hints, opaque outside the declaring profile (`worksurface.views-presets`, File 25 §7.2); a preset enumerates units, while the conversation surface and the registered rails are declared separately (§14.4, §15); applying it changes presentation only. The researcher and controller presets are the canonical resolution of the prior `WebMode` — they are presentation shapes, never autonomy or participation modes (§22): switching from the researcher to the controller preset activates the `browser` and `activity` units; it does not change how autonomous the agent is, which model is selected, the context policy, the execution entry, the budget, the sandbox profile, or the approval posture. Selecting a richer preset is progressive disclosure (the user sees more state and asks different questions), never a mode change. The user may save custom presets and override the default per scope (`settings.profiles`, File 15 §7).

### 14.3 The Activity Projection and Inspectors

The Web surface presents an activity projection — the browsing or research run's execution as it unfolds — through the shared execution-presentation contracts (`run.presentation`, File 04 §25): typed step entries for search and fetch progress and for the act/observe/extract steps carrying their result references (a page action against the captured page, an extraction as its structured result), batched approval requests (§5.5), nested research sub-agent activity (§9.3), download progress (§11.3), and streaming synthesis. The projection is over the ledger and event stream, not a private log; how its steps and their results are presented, and what is shown expanded or condensed, is the UI specs' (`ui.streaming-presentation`, File 37 §10). The surface declares default inspectors (context, source/citation, network, and execution inspection) as `inspector` unit projections over substrate state, each projecting the substrate its owning file fixes — context assembly (File 13), the citation and evidence records (File 09), the network observations (§8.3), and execution (File 04).

### 14.4 Customization, Morphing, and the Shell

The Web surface declares a customization policy (`worksurface.views-presets`, File 25 §7.4) describing which classes of customization it permits — which presentation units may be added, removed, or recomposed, what may be contributed into a unit, and who may author a contribution — resolved by the UI customization layer; the surface owns which classes are allowed, never the realization. Surface morphing — projecting the web units and active view preset when the Web surface becomes the active presentation surface — is a UI projection driven by the `SurfaceContract`, the live `SurfaceState`, and the routing decision (`worksurface.views-presets`, File 25 §7.3); it changes presentation, not the work model. Conversation is the always-available continuity surface, never required to be the primary presentation during browsing or research (`worksurface.activation-shell`, File 25 §11.3); its rail is registered for a client session that declares the `Conversation` entry class (`controlrail.registry`, File 26 §14), and a research conversation may move between conversation-first and a web-surface focus over time without changing the work model.

### 14.5 Boundary

This section declares the web view presets and customization policy. The UI Shell and UI Customization specs own presentation, composition and morphing (`ui.layout`, File 37 §5), the rendering of every unit including the browser and canvas units (`ui.substrate-rendering`, File 37 §9), the customization policy that governs them (`customize.customization-policy`, File 38 §11), and the placement of what is contributed into a unit (`customize.widget-placement`, File 38 §12); File 04 owns the execution presentation the activity projection reads; File 15 owns the settings and profiles for saved presets.
...
The Web surface declares its control affordances — the user-facing ways to invoke its capabilities — and binds them to the control rails (`controlrail.consequences-for-later-specs`, File 26 §21). It declares which capabilities carry a `default_binding`, which resolve as token commands, and which are exposed as direct affordances, registers surface-scoped binding contexts and custom commands, and introduces no private rail, invocation registry, or rail autonomy field.

### 15.2 Token Commands, Custom Commands, and Mentions

The Web surface's control affordances include token commands resolved through the `TokenCommand` rail (`controlrail.consequences-for-later-specs`, File 26 §21) — representatively research, deep-research, browse, search, and monitor commands — and workspace-local custom commands stored in the workspace `.atlas/commands/` directory (`workspace.internal-layout`, File 24 §8.3), each a declarative definition resolved through the capability system, never an out-of-band execution path. A web mention — a url, a page reference, or a source reference supplied through any registered entry class — resolves into an attributed context attachment: supplied through the `Conversation` entry class it resolves through that rail's pre-dispatch transformation (`controlrail.boundaries`, File 26 §2.2, `intent.message` (File 02 §3.4)), and supplied through any other registered class it resolves through that grammar's argument extraction (`controlrail.registry`, File 26 §14.2) to the same attachment; a supplied url may be offered for fetch-and-attach, and a referenced source resolves to its citation; mentions are attributed context, not hidden model-request text (§16).

### 15.3 Binding Contexts and Capability Discovery

The Web surface registers surface-scoped binding contexts (`controlrail.keybinding-keymap`, File 26 §7.3) that the rail layer's resolver resolves a `Binding` against: representatively a browser context (navigate, back, forward, reload, scroll, find-in-page), a researcher context (new search, pin source, capture citation), and a reader context. A `Binding` is the grammar-typed token sequence the control-rail layer defines (`controlrail.keybinding-keymap`, File 26 §7.2), and this surface's declared bindings target capability ids; the `GestureGrammar` a rail declares, the context stack and the resolver are the control-rail layer's (`controlrail.keybinding-keymap`, File 26 §7) under its registration contract (`controlrail.registry`, File 26 §14.2). The surface's capabilities, custom commands, and captured sources and urls are discoverable by name through the Discovery rail (`controlrail.command-rail`, File 26 §6), whose rendered entry data the discovery lens fixes (`surface.presentation-in-user-facing-surfaces`, File 07 §12.1); the rail is subsystem-neutral, and how a client presents its results is the client's. The surface declares the default web workflow commands — representatively research a topic, summarize a page, extract structured data, monitor a page, and compose a cited report — as capabilities and command definitions resolvable through each entry class the client session registers (`controlrail.registry`, File 26 §14). Speech and external-protocol invocation of web capabilities are the rail layer's.

### 15.4 Boundary

This section declares the web rail bindings and control affordances. File 26 owns the rail primitive, the input-resolution contract, the binding resolver and its gesture grammars, the token-command grammar, the Discovery rail, the mention resolution, the speech grammar, and the trigger and elicitation rails; File 07 owns the discovery lens (`surface.presentation-in-user-facing-surfaces`, File 07 §12.1); File 24 owns the `.atlas/commands/` store; File 05 owns the capabilities the affordances invoke.
...
The Web surface does not programmatically capture credentials. A login, a multi-factor prompt, a payment, or a mid-session auth expiry is an elicitation handoff (`controlrail.elicitation`, File 26 §13) to the user, who completes the authentication in the browser; the surface then proceeds with the established session. A captcha or equivalent challenge follows the same handoff path unless a separately enabled challenge-assistance capability is explicitly invoked under §16.4. A credential the user does choose to provide for storage enters through a credential-input elicitation that is never-logged and never-stored outside the vault (`security.credentials`, File 22 §6). Profile authentication is a guided elicitation sequence: the core supplies the steps and the handoff points and the client presents them; it respects user privacy and handles multi-factor and challenge flows through handoff rather than credential capture.

### 17.4 Sensitive-Data Masking

When a browser action fills a form with a credential or other secret, the value is supplied from the vault and masked in every observable surface: the activity projection, the ledger, the session export, and the model context show a redacted placeholder, never the value (`ledger.sensitivity-aware-persistence-retention`, File 10 §10, `secret.backend-boundary` (File 22 §4)). A `Secret`-classified value never persists raw; a captured page or network response that contains a secret is redacted at capture per the perception secret-redaction contract (`perception.capture-privacy`, File 19 §10.4).
...
This section owns the web cross-surface composition. File 07 owns borrowing; Files 08 and 09 own the shared pools; File 03 and File 04 own reroute; File 31 (GUI Control) owns desktop automation. The borrowing surfaces own their own workflows.
...
- **A web autonomy, participation, interaction-shape, persona, or `WebMode` autonomy field** — at any layer, in any form; web autonomy is capability permission tiers, leases, and approval posture plus user direction, progressive disclosure is which presentation units are active and which view preset is open, researcher/controller are view presets, and the periodic planning interval is a run structure, not a phase machine (§13.5, §14.2; `worksurface.no-autonomy-field`, File 25 §13). There is no planner-versus-navigator phase machine, no `Drive`/`Supervise`/`Collaborate`/`Delegate` dial, and no per-surface "research mode enabled" autonomy toggle.
...
- **A fixed post-action delay as the page-readiness mechanism, or time-based monitoring as a correctness condition** — page readiness, document-change, and network observation are event-first; a change-monitor cadence is a flagged fallback only where a source emits no change events; the only permitted timer is the finite, configurable, killable wall-clock safety guard (`sandbox.resource-limits`, File 23 §9.3 — decisive only without a completion signal, otherwise a last-resort backstop) (§7.3, §8.3, §12.2; `perception.triggers`, File 19 §8, `world.explicit-rejections` (File 18 §16)).
- **A view preset that silently changes backend policy** — applying a web view preset (researcher, controller, synthesis) changes presentation only and never silently changes model selection, context policy, execution entry, budget, sandbox profile, backend, or approval posture (§14.2; `worksurface.views-presets`, File 25 §7.2).
...
- The **UI Shell, Layout, Presentation, and Interaction Models** (File 37) and **UI Customization, Widgets, and Theming** (File 38) specs present the projections this file fixes — its declared presentation units, the browsing context, the research canvas, the source and reader projections, the activity projection, capability discovery, the binding contexts, and the view presets — however their paradigm realizes them, the windowed-desktop realization being their profile material (`ui.frontend-profile`, File 37 §1.3); presentation may vary freely, the work model cannot. The browser engine, the canvas renderer, the reader extractor, and the search backend are their implementation choices behind the units and capabilities.
...
- The **Packaging, Platform, and Distribution** spec ships the built-in declarations for the canonical web capabilities, the Web `SurfaceContract`, the web `Custom` event and observation kinds, and the default web settings as the `Builtin` source in every install, and packages the browser engine, the search backend, challenge-detection and handoff implementations, the content extractors, and the browser extension behind their contracts; the browser-automation capabilities are gated by the browser-backend availability check (§3.2) and report unavailable where no backend is registered, and which backends a given package ships is a packaging decision, never a capability rule.

## 24. Canonical Rule Anchors

Anchor: `web.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `web.chosen-model`, `web.boundaries`, `web.surface-contract`, `web.state-declaration`, `web.actions-declaration`, `web.fetch-search-extract`, `web.browser-sessions`, `web.page-observation`, `web.research`, `web.evidence-citation-trust`, `web.artifacts`, `web.monitoring`, `web.policy-declaration`, `web.views-presets`, `web.rails`, `web.untrusted-content`, `web.credentials-egress`, `web.cross-surface`, `web.world-perception`, `web.persistence-locality`, and `web.capability-event-settings`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
