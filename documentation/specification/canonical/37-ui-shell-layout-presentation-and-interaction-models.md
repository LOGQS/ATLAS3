...
ATLAS3 has one presentation layer. It renders the one shared runtime and resolves the user's gestures against it. It is the realization of `core.invariants` (File 01 §7.5)'s flexible-presentation rule — "presentation shape may vary by surface, interaction shape, and request complexity without changing the underlying runtime model" — and the structural enforcement of `core.invariants` (File 01 §7.7)'s service-layer-ownership rule.
...
- the **presentation layer** (this file) — the rendering of substrate projections plus the resolution and presentation of gestures. It owns no business logic, no durable store, and no work model. It is the realization layer for the rendering every prior file delegates to "the UI specs."
...
- A supporting region may auto-reveal on a declared closed-canonical-plus-`Custom` set of reveal trigger classes per region: the execution console may reveal on run-started, the artifact navigator may reveal on artifact-produced or first-output, and custom regions declare their own trigger classes. Approval and elicitation remain the focused-dialog selector's domain (§12); at most, a pending request drives a badge on a relevant region, not region reveal. Auto-reveal never moves keyboard or assistive-technology focus off the focus surface, never blocks it, and falls back to badge-only behavior when disabled.
...
Each `Block` renders through its registered renderer keyed by `BlockKind` (§3.3). A renderer presents the block's content and its derived view-state (lifecycle `Raw`/`Masked`/`Dropped`/`Recovered`, pin state, sensitivity) as projections (`block.block-lifecycle-non-destructive-edits`, File 08 §6.1; `version.consequences-for-later-specs`, File 11); a masked block renders its description, not its content. External-content blocks render a reference and the description, never pulling the blob into the view unless the user opens it. A `Memory`-sourced block renders its content as natural prose and never renders the fact that the content came from memory (`unit11a-memory.md` D11.M.17's natural-awareness rule; `memory.consequences-for-later-specs`, File 14 §22) — this is a renderer-level constraint.
...
- The presentation layer renders the `policy.approval-ui-surface-contract` (File 06 §13) data contract verbatim — `ApprovalRequest`, `LeaseOption`, `ApprovalResponse`, `BatchApprovalRequest`, `BatchApprovalResponse`, and `ContradictionResolutionRequest` — and the `controlrail.elicitation` (File 26 §13) contract — the `Elicitation` with its closed kind set (approval, clarification, choice, correction, intervention, plus the credential, payment, confirmation, file-picker, and multi-step-wizard cases the sources name). It never invents a parallel approval or elicitation data shape (`policy.consequences-for-later-specs`, File 06 §18; the File 06 §13.6 rule that the policy layer emits typed events and the UI renders and responds).
...
Every visual property a renderer applies — color, radius, shadow, font, spacing, animation timing — comes from a semantic token, never a raw value (`atlas3-core/CONSTRAINTS.md` §3; `cross-cutting/theming.md` realized). This file owns the discipline (renderers consume only tokens); File 38 owns the token system and the themes that resolve the tokens. The discipline makes the presentation fully themeable and high-contrast-capable without renderer changes.
...
- The presentation layer's user-facing operations are canonical capabilities in the one Capability Registry (`capability.declaration`, File 05 §3), declared as built-ins, tier-gated by policy (File 06), surfaced through tool-surface composition (File 07), and invoked through the shared pipeline (`run.call-pipeline`, File 04 §8.2). Presentation capabilities declare touched resources and effect by kind:
  ...
  - saving or deleting a user presentation preference (a layout, a default view preset, a density or theme reference) is a settings/customization write (composed with File 15 and File 38, not duplicated)
...
- **A view that polls a substrate on a timer for live state, or time-based correctness in presentation** — views are event-first; a periodic refresh is a flagged, configurable fallback only where a source emits no change events; auto-continue countdowns and animation timings are conveniences, never correctness conditions (§3.2, §10; `core.workspace-model`, File 01 §3 constraint).
