...
- **Structured destinations and resolved traversal policy.** A `NetworkDestination` is the machine-resolvable endpoint expression over protocol, host or domain pattern, port, IP or CIDR range, and loopback/private/public address class. DNS behavior and redirect handling are typed enforcement policies over how a request resolves and traverses a destination; they are not fields of destination identity, and they resolve **per destination entry**. Before connection, the runtime resolves the applicable `NetworkPolicy` into one transient `ResolvedNetworkEnforcement` context — `None`, `LoopbackOnly`, `AllowList` of `ResolvedDestinationRule { destination, dns_behavior, redirect_handling }`, or `Unrestricted { dns_behavior, redirect_handling }` — taking each rule's initial modes from safe general and profile defaults and any policy-permitted per-entry or per-action override, which may narrow or explicitly widen them, and records the resolved context with the invocation. DNS resolution and DNS egress are part of enforcement. Every resolved address and CNAME must satisfy its destination rule and the File 22 egress governance, every redirect hop is re-evaluated against the applicable destination rule and the credential/secret destination floor, no mode may bypass that floor, and an unknown mode fails closed. If a platform cannot enforce the resolved context directly for a spawned process, the runtime uses a mediated proxy, denies network, or records the unenforced gap and re-gates through policy; it never treats a host-only allowlist as complete enforcement.
