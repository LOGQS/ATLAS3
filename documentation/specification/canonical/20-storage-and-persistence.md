...
- All mutations pass through a single serialized writer. Concurrent writes are serialized, not lost; silent last-write-wins over concurrent mutations of shared state is rejected (`core.explicit-rejections`, File 01 §8). A commit boundary (§5.3) executes as one immediate-mode storage transaction and holds its substrate's writer throughout, releasing it before the boundary operation returns, so a read-modify-write inside one boundary is atomic. A business workflow spanning several boundaries never retains the writer between them; shared mutable state that outlives a boundary must be protected by the precondition-checked paths §3.4 requires, never by a held lock.
...
A **derived constraint index** is distinct from an ordinary `Projection`. It contains no independent semantic fact and is reconstructable from its owning source-of-truth records, but an owning canonical guard may use its unique-key reservation as write-integrity evidence: the reservation is committed in the same storage transaction as the guarded source fact, and a conflicting reservation rejects that source commit. Derived constraint indexes have explicit rebuild generation and readiness state; a dependent guarded write fails closed while its required index is absent, incomplete, corrupt, or known to describe a different source generation. Rebuild machinery may be shared with projections, but a derived constraint index does not implement the semantic rule that a projection failure cannot compromise an otherwise-valid source commit.
...
- No retention or pruning is time-based without explicit user or selected-profile opt-in (`core.non-destructive-by-default`, File 01 §7.13; `version.garbage-collection-pruning`, File 11 §20.6). The default retention keeps everything unless an owning canonical rule explicitly permits bounded or disposable storage, as for rebuildable projections and disposable caches (§7, §8.3) and the device-local diagnostic stream (`observability.retention`, File 41 §9.2). Bookmarked and labeled versions are exempt from policy-driven pruning regardless of policy. Every retention invocation is itself a durable, recorded fact, and one whose fact cannot be recorded does not run — a bounded store it would have reclaimed continues to enforce its configured admission and hard storage bounds, visibly refuses writes exceeding them, and reclaims nothing until a recorded invocation reclaims room; no layer silently prunes `Sensitive` or safe-description `Secret` records without a recorded policy transition (`ledger.sensitivity-aware-persistence-retention`, File 10 §10.4).
...
- A silent fresh-start over recoverable data is rejected. A corruption event is always typed, surfaced, and recorded; the user is never told all is well when data was lost. The unrecordable-condition case (`runtime.capability-surface-events`, File 42 §19.2) is the sole exception to the requirement to record a corruption event; the duties to type and surface it remain.
...
- Shutdown flushes before exit: set a shutting-down flag, apply the shutdown admission rules (`runtime.shutdown`, File 42 §12.3), request cooperative drain for in-flight storage work, flush buffered events to the durable substrate, commit or roll back pending transactions at safe boundaries, attempt the write-ahead-log checkpoint for each substrate, leave the substrate open for the remaining shutdown records and the diagnostic drain (File 42 §12.3–§12.4), then close the substrate and release the lock. If shutdown must force termination, eligible processes are cancelled or killed through File 04's killability contract; every acknowledged commit remains durable and staged partials are discarded, orphaned for reconciliation, or surfaced with typed recovery state.
