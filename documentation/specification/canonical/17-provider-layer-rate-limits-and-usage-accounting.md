...
## 27. Canonical Rule Anchors

Anchor: `provider.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `provider.provider-layer`, `provider.boundaries-with-adjacent-layers`, `provider.provider-adapter`, `provider.provider-profile`, `provider.provider-instance-provider-registry`, `provider.provider-sourcing`, `provider.provider-request`, `provider.parameter-serialization`, `provider.streaming`, `provider.provider-error-classification`, `provider.transport-level-retry-backoff`, `provider.provider-health`, `provider.rate-limit-state-header-reconciliation`, `provider.credentials-accounts-pools`, `provider.model-catalog-capability-normalization`, `provider.cache-marker-translation`, `provider.tokenizers-token-counting`, `provider.token-source`, `provider.token-usage-record`, `provider.cost-as-derived-projection`, `provider.multimodal-usage`, `provider.provider-runtime-snapshot-provider-offering-projection`, `provider.events-emitted`, `provider.sensitivity-redaction-secret-boundary`, and `provider.settings-dimensions`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
