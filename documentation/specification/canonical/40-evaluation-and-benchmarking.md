...
- the closed `EvalFamily` set and each family's primary artifact — discharging the offline-evaluation deferrals left by Files 03, 07, 12, 13, 14, 16, 17, 21, 24, 25, 26, 27, 32, 34, 35, 36, 38
...
- the `Hook` primitive, the ledger and event substrate, the `LedgerEntryKind` catalogue, the event envelope, or the custom-kind registration mechanism — File 10 owns those; this file registers `Custom { namespace: "eval" }` kinds
...
- live metrics, traces, structured logs, observability projections, or the observatory rendering surface plumbing — the future Telemetry, Logging, and Observability spec owns those; this file owns offline suites, benchmarks, scoring, and judge optimization
...
This model realizes `core.substrate-services` (File 01 §2.4), which names "logging and evaluation" among substrate services, and `core.evidence-provenance` (File 01 §7.12). It discharges the offline-evaluation deferrals named by `qc.boundaries-with-adjacent-layers` (File 39 §2.6), `qc.judge-discipline` (File 39 §6.4), `qc.events-metrics` (File 39 §17.3), `qc.consequences-for-later-specs` (File 39 §22), `ledger.replay-semantics` (File 10 §11.6), `version.consequences-for-later-specs` (File 11 §24), and the "evaluation specs should/must" consequences of Files 03, 07, 12, 13, 14, 16, 17, 21, 24, 25, 26, 27, 32, 34, 35, 36, and 38.
...
- `Coding` — primary artifact: the code-artifact `ArtifactVersion` and the `Test`/`Lint`/`Build` validations. Discharges File 27 §"consequences" (the edit-to-artifact-revision-to-materialization-to-external-edit-to-version round-trip, code-search retrieval, the test-and-validation gate, revert-as-version-switch, worktree-backed multi-agent merge, session-export projection).
...
- `Latency` — primary artifact: the recorded timing facts (per-call latency, time-to-first-token, end-to-end run duration). Latency and resource-budget regressions against declared targets.
...
- `UX` — primary artifact: the customization events and the structural-understanding projection (File 38; File 37). Discharges File 38 §"consequences" and the UX-level interaction evals for critical surfaces; consumes the events with no parallel customization store.
...
- folding inline runtime validation, the validation gate, or the live-metric projections into this layer — inline validation that gates live execution is File 39's; live metrics, traces, and observability projections are the future Telemetry, Logging, and Observability spec's; this layer is offline measurement over recorded and fixture inputs
...
- the future **Telemetry, Logging, and Observability** spec must compute live metrics, traces, and quality-metric projections (validator-accuracy, false-positive rate, latency, pass-rate aggregates) over the ledger and events without introducing a parallel store, and must consume — never redefine — this file's `EvalScore`/`EvalReport`/leaderboard contracts where the Observatory renders evaluation results; the 40↔41 seam is fixed (offline-suites-and-comparison versus live-observation-and-aggregation)
- the future **Runtime Infrastructure and Lifecycle** spec must orchestrate evaluation-run execution through the one run model and the one scheduler/trigger substrate (File 33), never a parallel evaluation scheduler, and must realize the replay-engine execution within the run lifecycle
- the future **Packaging, Platform, and Distribution** spec must bundle built-in suites and golden fixtures into the image as ship-with content, and must not introduce a parallel evaluation distribution path
...
- the **per-surface specs** (Files 25–32) — already written — register their family scorers through File 39's path, declare the family and primary artifact their work belongs to, and may ship built-in suites; none introduces a private evaluation pipeline; the surface, perception, coding, system-operation, control-rail, and connector families discharge their offline-evaluation consequences over recorded snapshots
