...
- `success_criteria` — the typed thresholds a run must meet to be reported as passing (a minimum pass rate per scorer, a maximum regression delta, a latency or cost ceiling, and the denominator policy the rates are read under, §13.2); success criteria are declarative and inspectable, never prose-only
...
An `EvalCase` is one fixture plus its expected outcome and the scorers that apply to it. Required fields: `case_id`, the `fixture` (§4.2), the `expected` outcome (a golden artifact reference, a set of expected properties, or both; §5), the `applicable_scorers` (a subset of the suite's scorers, or all), and an optional `case_weight` (an absent weight is the unit weight, a protocol constant and never a setting; a weight of zero is invalid and refused when the case is declared, because a case its author wants uncounted is left out of the suite; a pass rate is the weight of the passing cases over the weight of the charged ones, reported beside the unweighted counts) and `tags` (the error-mode or difficulty labels used for grouping and Pareto-aware selection; §11). A case's `expected` outcome may be empty when the case is scored only by intrinsic property assertions (a "the output must be valid JSON and tests must pass" case needs no golden reference).
...
The comparison aggregate — win rate, win/tie/loss distribution, ranking, regression delta — is an `EvalScore` projection (§13) over the per-case relative verdicts, never a per-case continuous score (§6.3). A per-case relative verdict exists only between two decided outcomes, a rejected pass counting as a failure; any other pair is not comparable, with its reason, and never a tie. The `Improved`/`Unchanged`/`Regressed` set is not widened: a comparison that cannot be made is carried by the report's compatibility state — compatible, stale with its reasons, or not comparable. A comparison evaluation is replayable: the recorded per-case relative verdicts (including the recorded blind label assignment and the model-mediated judge's recorded verdict and replay key) reproduce the aggregate on replay (§7.5).
...
A suite carries a `RegressionBaseline`: a pinned prior `EvalRun` against which a new run is compared. The baseline is set explicitly (an `EvalRun` the user accepts as the reference) and is itself a versioned, audit-visible designation. A suite without a baseline reports absolute scores; a suite with a baseline additionally reports a regression comparison. A baseline is compatible only with the same suite version or with a declared compatibility or migration relation. Changes to scorer identity, the resolved scorer roles, the resolved case weights on shared cases, expected properties, golden artifact version, or the suite's declared success criteria — each a fact of the suite version (§3.3) — make older baselines stale unless an explicit compatibility relation says otherwise. A difference only in a settings-level threshold or in the resolved denominator policy (§13.2) does not stale a baseline. Every regression comparison recomputes both runs under the new run's resolved policy and case weights over the cases the two runs share and applies the new run's resolved thresholds; the report shows the baseline's own recorded policy and thresholds beside the figure. A stale baseline remains inspectable but cannot gate a change.

### 10.2 The `RegressionReport`

A new `EvalRun` against a baselined suite computes a `RegressionReport`: per-case and per-scorer deltas versus the baseline, classified as `Improved`, `Unchanged`, or `Regressed`, plus the aggregate regression figure (the net pass-rate delta, the set of newly-failing cases, the set of newly-passing cases). The deltas cover only the cases both runs share: a case added or removed since the baseline is counted and listed and never enters a delta, and a case `Passed` in the baseline and non-decisive in the new run under §9.4 is listed as newly unverified. The report is a projection over the new run's verdicts and the baseline run's recorded verdicts (`core.projection`, File 01 §6.11). A regression that crosses a declared threshold (a suite `success_criteria`, §3.2) flags the run as a regression, and — where the suite is used as a gate (§10.4) — blocks the change that produced it.
...
An `EvalScore` is the derived aggregation view over an `EvalRun`'s per-case verdicts (`Validation`/`Critique` blocks) and golden-comparison results, read with the not-scored records and the resolved policy, success criteria, scorer roles, and case weights the run recorded (§13.2). It is computed on read, exactly as `ValidationReport` is computed over validation blocks (`qc.validation-report`, File 39 §12) and `Provenance` over the block graph (`artifact.provenance`, File 09 §15). It is not a stored primitive and not a parallel result ledger; the per-case verdicts are the source of truth, and any score cache is a rebuildable projection (`core.projection`, File 01 §6.11) invalidated event-driven on new verdicts.

### 13.2 What the Projection Computes

An `EvalScore` summarizes: pass rate per scorer, per family, and per suite; the set of decisive failing cases with their failure details; the per-scorer false-positive context where annotation feedback exists; the latency distribution and the cost figures (from `TokenUsageRecord`/`PricingSnapshot`, File 17); the regression deltas versus the baseline (§10); and, for comparison evaluations, the win/tie/loss distribution and any ranking projection (Elo or Bradley-Terry over the pairwise outcomes; §9). It carries the denominator policy, the resolved case weights, and the resolved scorer roles used for aggregation, and counts, per scorer, the `Inconclusive` verdicts and each §6.4 diagnostic (`DisabledBySetting`, `BudgetExceeded`, `BackendUnavailable`, `NotApplicable`) separately from the `Passed` and `Failed` verdicts. At case level it counts the `NeedsReview`, `NotValidated`, not-scored, and `Pending` cases separately from the `Passed` and `Failed` cases, and breaks the not-scored count down by `NotScoredReason`. No bound applies to counting: a bound applies only to an enumerated list of cases, each bounded list carrying its shown and total counts and the bound, and run incompleteness is the `Pending` count, never a truncation; a truncated report is not proof that no further failures exist beyond the bound (the rule of `qc.validation-report`, File 39 §12.2).

Every case the run selected is in exactly one case class, taken in this order: `NotValidated` when none of its applicable scorers is `Required` — a fact of the resolved suite, so such a case is outside the case-level rate, whatever its other scorers found and whether or not it ran; this exclusion does not disable an explicit per-scorer success criterion permitted by §6.1; `NotScored`, carrying its `NotScoredReason`, when the run recorded that it did not score the case; `Pending` while the run is not terminal and the case has neither a scored outcome nor a not-scored record; and otherwise `Failed`, `NeedsReview`, or `Passed` as §6.4 derives them, a `DisabledBySetting`, `BudgetExceeded`, or `BackendUnavailable` result of a `Required` scorer leaving the case `NeedsReview` as an `Inconclusive` verdict does. A `NotApplicable` result of a `Required` scorer leaves the case `NeedsReview` too, charged as a `DisabledBySetting` result is. A case the sampling policy did not select is in no class; it is reported through the selected count (§14.2). `NotScoredReason` is the closed set of reasons a selected case goes unscored:

- `DroppedByBudget` — a budget dropped the selected case before it was scored (§14.2)
- `Unreplayable` — the case could not be replayed or had no recorded output to score, including a fixture whose referenced substrate is unavailable (§4.4)
- `IntegrityRejected` — the eval-forgery guard rejected the case's scored pass (§7.6)

A not-scored record is a fact of the `EvalRun`: the run records it when it disposes of the case, at most one per selected case, identified by the run and the case, and the score lists the not-scored cases, like every per-case list, in the run's recorded case order. It lives in the `EvalRun` record with that record's locality, replication, and retention (§18) and adds no rule of its own; replicas of the same run-record version carry the same not-scored records. Devices derive the same case classes when they use the same recorded case population, resolved scorer roles, run state, per-case outcomes and not-scored records. It is not a verdict and never counts as `Passed`: it states only that the run did not score the case, and why. `Pending` is derived from the run's state and never recorded; on a terminal run, a selected case with neither a scored outcome nor a not-scored record is a typed error, never a silent omission (§14.2).

The denominator policy decides which selected cases a pass rate charges. Under every policy a `Passed` case counts in both the numerator and the denominator; a `Failed` case and a case `NotScored` as `IntegrityRejected` are charged to the denominator, so a rejected pass never scores better than an honest failure; and a `NotValidated` case is excused, outside both and counted separately. `DenominatorPolicy` is the closed set of policies:

- `AllSelected` — the default and the strictest reading: it excuses nothing further, so every other selected case left undecided is charged, whatever left it so
- `ExcludeUnavailable` — additionally excuses what unavailability alone left undecided: a `NeedsReview` case whose every undecided `Required` result is `BudgetExceeded` or `BackendUnavailable`, a case `NotScored` as `DroppedByBudget` or `Unreplayable`, and a `Pending` case; a `NeedsReview` case held by an `Inconclusive` verdict or a `DisabledBySetting` or `NotApplicable` result stays charged, because a deliberate disable is not an outage and a `Required` scorer that cannot judge its case is a defect of the suite, never an outage
- `DecidedOnly` — additionally excuses a `NeedsReview` case held by an `Inconclusive` verdict or a `DisabledBySetting` or `NotApplicable` result, so it charges only the decided cases and the rejected passes

A per-scorer pass rate applies the same policy over the cases that scorer applies to and excuses its `NotApplicable` results under every policy. `NotScoredReason` and `DenominatorPolicy` are closed canonical sets with no `Custom` extension (`core.closed-canonical`, File 01 §6.16); widening either is a revision of this file.

The policy and success criteria are resolved before execution and recorded on the `EvalRun`, together with the resolved case weights and the scorer roles §6.1 records there; scoring and reports read those resolved inputs from the run, never from live settings. `AllSelected` applies unless a suite declaration or a setting resolves another, so a weaker policy is always a recorded declaration or setting, never silent. Every score exposes the pass rate under all three policies. The resolved policy's reading is the one that headlines the score, is held to the success criteria (§3.2), and classifies regressions (§10.2); an alternative reading never replaces it, and the report names the resolved policy beside every rate with each excused class and its count. A rate whose denominator is empty is unmeasured — never zero and never a full pass rate — and a success criterion checked against an unmeasured rate is not met. A success criterion over a run with any `Pending` case is also not measured and is not met under a policy that charges that case; the report states that the run is incomplete. A suite's per-scorer minimum pass rates (§3.2) are read under the same resolved policy and case weights, and the case-level criterion and every per-scorer criterion must all hold.
...
Given the same recorded inputs used by the projection (§§13.1–13.2), including the run state, case population, per-case results, not-scored records, resolved scoring inputs, golden-comparison results, and latency and cost facts, the same baseline and annotation position, and the same bounds on enumerated report lists, two `EvalScore` computations return identical results; this is the load-bearing property for replay and audit (mirroring `qc.validation-report`, File 39 §12.3). Changing only a list bound changes only the enumerated list and its truncation details, never the counts, rates or success-criterion results (§13.2). The annotation position is the ledger position as of which the per-scorer false-positive context (§13.2) reads the annotations (§12), and it is a recorded input of the report: an annotation recorded after that position changes a report computed at a later position, never a reproduction of this one. An `EvalReport` is the rendered, user-facing form of one or more `EvalScore`s plus the run's configuration and regression context.
...
An `EvalRun` runs under the canonical run and per-stage budgets (`run.budgets-limits`, File 04 §21). A suite declares its sampling policy (§3.2): `EveryCase` for a regression suite (every fixture, deterministic), `Sampled { fraction }` for a cheap monitoring suite, and `DeepDive { fraction }` for an expensive sampled deep-dive (a fraction, run with capable models for drift and trend analysis). File 33 or explicit user action decides when the suite is triggered; the sampling policy decides which cases within that trigger are selected. A suite may declare a fixed per-case budget so that arms are compared fairly under the same resource ceiling. Silent truncation of the case set is forbidden, so a partial run never reads as full coverage: a selected case a budget drops is recorded on the run as `NotScored` with the reason `DroppedByBudget` and reported in the not-scored count (§13.2), and the cases a sampling policy leaves unselected are recorded and reported as the selected count of the case set, outside every case class.
...
- the denominator policy (§13.2), per suite above the suite's own declaration and evaluation-wide beneath it, and the bound on enumerated report lists; case weights, the absent-weight default, and the aggregation arithmetic are suite facts or protocol, never settings
