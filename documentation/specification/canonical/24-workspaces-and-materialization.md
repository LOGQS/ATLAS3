...
Every prior file uses `workspace` as a scope label (`block.block-scope` File 08 §11, `policy.lease-primitive` File 06 §11, `ledger.execution-ledger` File 10 §3.5, `world.world-entity` File 18 §4.2, `MemoryScope::Workspace` File 14, `KnowledgeScope::Workspace` File 12) and as a foreign key (`workspace_id` on memory, knowledge, retrieval, log, and artifact records; `ScopeAnchor::Workspace` on `workspace`-scoped blocks, `block.block` File 08 §2.2), yet none defines what the identifier resolves to. A durable `WorkspaceRecord` gives that identifier a single owner, a lifecycle, and a relocation-safe identity, so a moved or renamed directory does not orphan everything keyed to it.
...
- `availability_status` — device-local availability state: `Available`, `Missing { reason }`, `NeedsRebind`, with the `Custom { code }` extension reserved; an unreachable-path cause (permission denial, an unmounted drive) is always encoded as the `Missing.reason` value (§6.3), never a sibling top-level variant, so exactly one encoding exists and the §6.3 recovery affordance always triggers
- `materialization_head` — optional device-local pointer `{ conversation_id, version_id, selected_by, selected_at }` naming the conversation/version projection that currently owns the bound root's mutable disk mirror; `selected_by` is a producer drawn from the canonical producer vocabulary (`block.block`, File 08 §2.2)
- `profile_id` — optional binding to a settings/layout profile (`settings.profiles`, File 15); syncable
- `default_conversation_id` — optional soft pointer into the bound-conversation set naming the workspace's default conversation, resolved when work targets the workspace without naming one (for example a workspace-bound automation conversation); part of the syncable logical identity (§4.3)
...
- This locality rule is the workspace-specific realization of the replication-eligibility contract (`portability.what-replicates`, File 21 §5): stable workspace definitions are syncable, device-specific bindings are not.
...
1. allocates a `workspace_id` and a directory under the data root (the canonical default is `<data-root>/workspaces/<workspace_id>/`, placed by File 20 §8 and named here);
...
A quick-start workspace is a `Temporary` workspace created without a user-chosen permanent location, with a disposable device-local binding under the data root's scratch area when materialization is needed (the canonical default `<data-root>/tmp/quickstart-<workspace_id>/`). It is `is_temporary = true`, excluded from the persistent workspace list, and never synced (§4). On conversation close, the user is offered to save it to a permanent location (promoting it to `Managed` or `Mounted`, which assigns a durable local binding and clears `is_temporary`) or to discard it. On discard, Atlas closes the temporary workspace's live handles, recursively removes only its confined scratch mirror, releases that scratch root's device-local claim, and leaves the device-local `Temporary` workspace identity with an `Unbound` local binding. The bound conversation, `workspace_id`, blocks, version tree, artifacts, ledger history, and conversation–workspace binding survive. Discard is not `workspace.delete`, does not create a workspace tombstone, and does not invoke a substrate record-hard-delete primitive. Successful removal emits `WorkspacePayloadDeleted` with `DeletionReason::UserRequested`. Incomplete removal is surfaced and reconciled and is never treated as a completed discard.
...
- Workspace visibility and addressability resolve from the durable conversation–workspace binding and the target record's durable workspace identity; they do not depend on device-local `binding_state` or `availability_status`. `Unbound`, `NeedsRebind`, `Missing`, and archived workspaces retain read access to their durable conversations, blocks, memories, knowledge, versions, artifacts, and already-committed indexed records subject to those records' own lifecycle, sensitivity, freshness, and policy rules; filesystem access, materialization, watching, and other root-dependent operations alone defer or refuse. A run captures the resolved `workspace_id` from the durable binding in its immutable resolution context; a later conversation rebind or local-root availability change cannot change that run's visibility boundary. If no unique durable conversation–workspace binding resolves, workspace visibility is not inferred from a local root and resolution fails closed. Tombstoning does not itself hard-delete or widen retained conversation or block visibility; their own lifecycle and deletion contracts continue to govern.
...
- `commands/` — custom command definitions (the workspace's slash commands), each a declarative definition resolved through the capability system, never an out-of-band execution path; this workspace layer composes over the global `<data-root>/commands/` definitions (§8.2), the workspace definition overriding the global on a name collision
...
- A conversation that is not the current `materialization_head` never silently materializes into the bound mutable root. When two independent conversations contend for one mutable root, an attempt to write the root from a non-head conversation raises the typed `NotMaterializationHead` condition, which surfaces three named resolutions: a conflict-previewed head switch that reassigns the `materialization_head` to the requesting conversation (the conflict preview of this section), an isolated worktree for the requesting conversation's work (§15), or read-only substrate access without materializing. The default is the typed denial surfacing these three resolutions (deny-and-surface); the system never auto-switches the head or spawns a worktree on its own.
...
- Case and normalization path aliasing is detected at the sink, not by a global pre-rejection. When the actual host filesystem folds two distinct tracked workspace-relative paths onto one physical entry — a case-insensitive volume, or one that folds Unicode normal form — materialization raises a typed path-alias conflict scoped to that workspace on that machine. The substrate keeps both paths distinct; the conflict surfaces for resolution on the affected device, and every platform is honored per its real filesystem semantics (a path pair valid on a case-sensitive volume is not rejected there).
...
- On each flush, the loop compares the disk state to the materialized view's expected state and, per changed path role, commits the substrate consequence:
  ...
  - **Modified** — a file whose content hash differs from its active block commits a sibling block (`block.edit-semantics`, File 08 §6.2), swaps the active reference, and records the `ExternalEdit` `VersionOpSummary` (`version.version-op-summary-commit-boundary-set`, File 11 §5.2).
...
- A worktree has a device-local durable `WorktreeRecord` (a substrate family, §22): `worktree_id` (stable identifier), `parent_workspace_id`, the associated `run_id`/child-run and `agent_id` where applicable, `root_path`, `base_commit_ref`, `branch_name`, `status` (a closed canonical enum: `Active`, `Merged`, `Discarded`, `Orphaned`, `Quarantined`, `Adopted`, `KeptDetached`), `payload_availability` (a device-local availability field orthogonal to `status`, recording whether the worktree's working-copy directory is currently present on disk: `Present` or `Missing { reason }`; a missing payload never mutates the `status` disposition — availability and disposition are independent, and there is no `Removed` status), `created_at`, `closed_at`. The path fields are device-local (§4); the record does not sync. Process and sandbox handles inside the worktree are transient and reaped at restart, but the directory payload is preserved unless explicit cleanup policy permits deletion.
...
- **Re-activate.** `Adopted` and `KeptDetached` are not terminal: either status may transition back to `Active` to resume work, re-associating the worktree with a live child run. Re-activation verifies the working copy is present (`payload_availability: Present`) and the recorded `base_commit_ref` still resolves; it is fail-closed — a missing payload, an unresolvable base ref, or contention with the parent workspace's current `materialization_head` surfaces a typed `Conflict` rather than silently rebuilding or overwriting (`run.isolation`, File 04 §16.2). It is a `WorkspaceWrite`-tier transition.
...
- Content deduplicates by content hash, and the disk mirror plus all indexes rebuild on receive as projections — they are never transported (`portability.what-replicates`, File 21 §5; `core.projection`, File 01 §6.11). Import is non-destructive and produces an `ImportPlan` preview (`portability.import-pipeline`, File 21).
...
- `WorkspaceRecord` identity mutations conform to File 20 §3.4's append-only entity-revision vehicle rather than mutating a committed record in place: a change to a syncable-identity field (the §4.3 authoritative set less only the immutable `workspace_id`: `name`, `profile_id`, `kind` — a §5.4 promotion, `lifecycle_status`, stable `vcs_binding`, `default_conversation_id`, the bound-conversation set) commits a superseding revision, and `updated_at`/`last_accessed_at` are the latest revision's timestamps rather than fields overwritten in place, preserving the immutable-source-of-truth rule File 20 fixes. The device-local pointer fields — `binding_state`, `availability_status`, `materialization_head`, and `relocation_history` (§4.3) — are typed device-local mutable pointers updated in place, the declared exception to the append-only rule: they carry no cross-device history and introduce no second history store.
