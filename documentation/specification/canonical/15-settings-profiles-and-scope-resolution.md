...
A refused inline-secret entry may offer two resolution affordances, neither of which weakens the secret boundary. The first, migrating to the vault, is a typed-confirmed human action that transfers the value through transient backend secret memory directly into an owner-scoped vault key and returns the safe `vault:<key>` replacement (File 22 §5.4). The second, binding to a vault reference, selects an existing vault key — or creates one through the normal write-only credential flow — without ingesting the file's literal at all, and yields the same `vault:<key>` replacement. Atlas never force-accepts the literal and never edits the read-only TOML file; the entry remains refused until the user replaces the literal with the returned reference and scrubs it.
