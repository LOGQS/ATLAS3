...
- the coder rails: token commands, the binding contexts, capability and workspace discovery, code-aware mentions, and the default coder workflow set — bound to capabilities through the control-rail layer (File 26)
...
- UI rendering — editor/terminal/diff chrome, the geometry and arrangement of a presentation unit in any medium, theming, the concrete editor engine, terminal renderer, content-search engine, or version-control library — the UI specs and infrastructure own those, and the windowed-desktop realization of this surface is their profile material (`ui.frontend-profile`, File 37 §1.3); this file specifies only the data and behavior contracts they consume
...
- `keywords`: the routing- and discovery-relevant terms the surface is discovered by (representative: code, programming, edit, refactor, debug, test, build, repository, terminal, diff, review)
...
The Coder surface declares the presentation units it can activate and the selection kinds it produces. This is the static counterpart of the live `SurfaceState` (`world.surface-state`, File 18 §5): this file declares the shape, File 18 holds the live values. A presentation unit self-registers its live state to the world model when it is activated and updates on focus, selection, and content change (`world.observation-state-update`, File 18 §8.1); a unit the surface fails to register is a blind spot the agent cannot use.

### 4.2 Declared Presentation-Unit Kinds

The Coder surface declares the following presentation-unit kinds, drawn from the canonical baseline (`worksurface.state-declaration`, File 25 §5.3) plus coder `Custom` kinds registered through the proposal-first mechanism. Each declares the typed shape of its compact state-field summary (a path, a working directory, a selected entry — never the resource body, `world.surface-state` (File 18 §5.2)), the selection kinds it produces, and its control affordances; how any of them is realized in a given medium is declared by that client's frontend profile (`ui.frontend-profile`, File 37 §1.3):

- `editor` — a code buffer over a `FileAttachment`/`Artifact` block; carries the active file reference, the focus position and selection where the client has them, the region of the file currently presented, and whether uncommitted changes exist; produces `CodeRange` and symbol selections; the editor engine is a replaceable implementation behind the unit
- `terminal` — a pseudo-terminal session (§10); carries the working directory, the running command if any, and a `canStop` affordance
- `file_tree` — a workspace directory projection; carries the directory scope currently presented and the selected files, with per-file version-control status surfaced from the repository observation (§9); produces `File` selections; honors the workspace ignore file
- `diff` — a change view between two versions (working-vs-staged, working-vs-head, or two `ContextVersion`s); addresses changes at chunk granularity, where a chunk-granular revert is a forward edit (§8.3); produces `CodeRange` selections
...
- `preview` — a projection of a running application or build output (§10), carrying the spawn context and the captured output
- `worktree_comparison` — a comparison over multiple parallel-agent results, exposing per-result diff, change counts, and preview; a projection over `WorktreeRecord`s and the version tree (§13)
- `history` — a version-tree projection (§8), carrying the timeline and per-version metadata

A presentation-unit kind the Coder surface declares is a shared substrate projection, not a private view: the `editor` and `terminal` kinds are canonical baseline unit kinds (`worksurface.state-declaration`, File 25 §5.3), and another surface may present a coder unit without changing its primary surface (`worksurface.state-declaration`, File 25 §5.4).
...
The declaration is the static shape; the live values — which presentation units are active, which is primary, what is focused and selected, the current `UiMode`, and the available-capability list — are the live `SurfaceState` File 18 holds. The Coder surface declares no participation, autonomy, or interaction-shape field on its state (§22); `UiMode` is interaction state, never an autonomy control, and its members and the surface-mode registration path are File 18's (`world.surface-state`, File 18 §5.5).

### 4.5 Boundary

This section declares the coder presentation-unit and selection shape. File 18 holds the live values and owns the self-registration contract; the UI specs own how a unit is presented in a medium, and the windowed-desktop realization of these units is their profile material (`ui.layout`, File 37 §5); the editor engine, terminal renderer, and content-search engine are replaceable implementations behind the units.
...
### 8.2 History as a Version-Tree Projection

The history unit projects the conversation's version tree: each commit boundary (`version.version-op-summary-commit-boundary-set`, File 11 §5) — an accepted agent turn, a context edit, an external edit, an artifact-version commit — is a point in the timeline, and switching to a version restores all workspace files to their state at that point (`version.version-switching`, File 11 §8), with the materialized view, the on-disk files, and any client presentation of those files updating to match. The projection addresses the change at three granularities, all derived from version diffs rather than a per-chunk store: feature-level (an agent turn), tool-call level (an individual edit within a turn), and chunk level (a line range within an edit). A "checkpoint" in coder vocabulary is exactly one version-graph node; the prior "one checkpoint per tool call" and shadow-directory models are superseded (§22).

### 8.3 Undo, Revert, and Restore

Within an in-progress turn, undo of an uncommitted operation walks the pending-operations buffer (`version.pending-operations-buffer`, File 11 §6); after a turn commits, the way to undo is to switch to or branch from a prior version. Reverting a single file is `coder.file_reverted` (`version.sibling-block-versioning-over-block-pool`, File 11 §12.2), resolved by one of two paths: a forward revert that commits a new context-edit version swapping the active file block back to a historical sibling (advancing the current branch), or a switch revert that moves the conversation's active version to one where the historical sibling was active (both branches surviving). Any client-side undo or save affordance resolves to this version mechanism; no client holds a separate file-history store. Revert is invocable at file and chunk granularity; a chunk-level revert is a forward edit that swaps the affected lines.
...
A review may be performed by the user, by a reviewer sub-agent (§13), or by an external reviewer. It produces a `Critique` block of kind code-review (`artifact.validation-critique`, File 09 §14) carrying findings with severity, location, description, and an optional suggested resolution, and a recommended action (accept as is, revision recommended, revision required, or reject and restart). Each finding carries an anchor to the code range it concerns, so any client may present it in place, and the findings compose into a review summary; they are advisory and do not by themselves gate state (realized in the windowed-desktop profile as an inline comment, `ui.substrate-rendering`, File 37 §9).
...
The Coder surface declares the presentation shapes it offers: the view presets a user can switch between, the default inspectors, the customization policy, and the activity-projection and shell affordances. A view preset is a named startup presentation shape; it is not an autonomy mode and does not silently change backend policy (`worksurface.views-presets`, File 25 §7).

### 15.2 View Presets

The Coder surface ships built-in view presets — representatively a minimal preset (the editor unit), a default preset (editor, terminal, file tree, and context inspector), and a worktree preset (editor, terminal, the worktree comparison board, and context inspector for multi-agent parallel sessions). A view preset declares which presentation units are active, which is primary, and the presentation-only startup state, with any medium-specific arrangement carried as optional profile hints, opaque outside the declaring profile (`worksurface.views-presets`, File 25 §7.2); a preset enumerates units, while the conversation surface and the registered rails are declared separately (§15.4, §16); applying it changes presentation only. View presets are presentation seeds, not autonomy controls: selecting a richer preset is progressive disclosure (the user sees more state and asks different questions), never a mode change, and switching presets mid-run never silently changes model selection, context policy, execution entry, budget, sandbox profile, or approval posture (§22). The user may save custom presets and override the default per scope (`settings.profiles`, File 15 §7).

### 15.3 The Activity Projection and Inspectors

The Coder surface presents an activity projection — the coding run's execution as it unfolds — through the shared execution-presentation contracts (`run.presentation`, File 04 §25): typed step entries for tool calls and their results carrying their content references (an edit's diff, a command run's captured output), batched approval requests (§12.3), nested sub-agent activity (§13), programmatic-execution inner calls, and streaming write progress (§6.6). The projection is over the ledger and event stream, not a private log; how its steps and their results are presented, and what is shown expanded or condensed, is the UI specs' (`ui.streaming-presentation`, File 37 §10). The surface declares default inspectors (context, execution, and registry inspection) as `inspector` unit projections over substrate state, each projecting the substrate its owning file fixes — context assembly (File 13), execution (File 04), and the capability registry (File 05).

### 15.4 Customization and Morphing

The Coder surface declares a customization policy (`worksurface.views-presets`, File 25 §7.4) describing which classes of customization it permits — which presentation units may be added, removed, or recomposed, what may be contributed into a unit, and who may author a contribution — resolved by the UI customization layer; the surface owns which classes are allowed, never the realization. Surface morphing — projecting the coder units and active view preset when the Coder surface becomes the active presentation surface — is a UI projection driven by the `SurfaceContract`, the live `SurfaceState`, and the routing decision (`worksurface.views-presets`, File 25 §7.3); it changes presentation, not the work model. Conversation is the always-available continuity surface, never required to be the primary presentation during coding (`worksurface.activation-shell`, File 25 §11.3); its rail is registered for a client session that declares the `Conversation` entry class (`controlrail.registry`, File 26 §14).

### 15.5 Boundary

This section declares the coder view presets and customization policy. The UI Shell and UI Customization specs own presentation, composition, morphing, and every unit's rendering (`ui.layout`, File 37 §5) and the customization policy that governs them (`customize.customization-policy`, File 38 §11); File 04 owns the execution presentation the activity projection reads; File 15 owns the settings and profiles for saved presets.
...
The Coder surface declares its control affordances — the user-facing ways to invoke its capabilities — and binds them to the control rails (`controlrail.consequences-for-later-specs`, File 26 §21). It declares which capabilities carry a `default_binding`, which resolve as token commands, and which are exposed as direct affordances, registers surface-scoped binding contexts and custom commands, and introduces no private rail, invocation registry, or rail autonomy field.

### 16.2 Token Commands and Custom Commands

The Coder surface's control affordances include token commands resolved through the `TokenCommand` rail (`controlrail.consequences-for-later-specs`, File 26 §21), and workspace-local custom commands stored in the workspace `.atlas/commands/` directory (`workspace.internal-layout`, File 24 §8.3): each a declarative command definition resolved through the capability system — a prompt-template command that expands to model-facing instruction, or a capability-binding command that invokes a coder capability with templated arguments — never an out-of-band execution path. Custom commands carry their parameters, preview and expected-postcondition metadata where declared, and an optional `default_binding` (`capability.display-fields`, File 05 §3.2), and are resolvable through each entry class the client session registers (`controlrail.registry`, File 26 §14) and by the agent through the one invocation path.

### 16.3 Binding Contexts

The Coder surface registers surface-scoped binding contexts (`controlrail.keybinding-keymap`, File 26 §7.3) that the rail layer's resolver resolves a `Binding` against: representatively an editor context (save, find, format, go-to-definition, rename, stage-hunk), a terminal context, and a diff context (next/previous change, revert chunk). The `Binding` itself, the `GestureGrammar` that types its tokens, the context stack, the conflict resolution, and the resolver are the control-rail layer's (`controlrail.keybinding-keymap`, File 26 §7) under its registration contract (`controlrail.registry`, File 26 §14.2). The surface declares which capabilities carry a `default_binding` (`capability.display-fields`, File 05 §3.2); a user-declared binding overrides the default through settings.

### 16.4 Capability Discovery and Mentions

The Coder surface's capabilities, custom commands, and workspace files are discoverable by name through the Discovery rail (`controlrail.command-rail`, File 26 §6), whose rendered entry data the discovery lens fixes (`surface.presentation-in-user-facing-surfaces`, File 07 §12.1); the rail is subsystem-neutral, and how a client presents its results is the client's. A code-aware mention — a file, folder, symbol, problem, repository, terminal, or open-file reference supplied through any registered entry class — resolves into an attributed context attachment (§7.6): supplied through the `Conversation` entry class it resolves through that rail's pre-dispatch transformation, and supplied through any other registered class it resolves through that grammar's argument extraction (`controlrail.registry`, File 26 §14.2) to the same attachment. The surface declares the default coder workflow commands — representatively code review, test generation, refactoring, codebase explanation, debugging assistance, and commit-message generation from staged changes — as capabilities and command definitions resolvable through each entry class the client session registers (`controlrail.registry`, File 26 §14).

### 16.5 Boundary

This section declares the coder rail bindings and control affordances. File 26 owns the rail primitive, the input-resolution contract, the binding resolver and its gesture grammars, the token-command grammar, the Discovery rail, and the mention resolution; File 07 owns the discovery lens (`surface.presentation-in-user-facing-surfaces`, File 07 §12.1); File 24 owns the `.atlas/commands/` store; File 05 owns the capabilities the affordances invoke. Speech and external-protocol invocation of coder capabilities are the rail layer's.
...
- **A view preset that silently changes backend policy** — applying a coder view preset changes presentation only and never silently changes model selection, context policy, execution entry, budget, sandbox profile, or approval posture (§15.2; `worksurface.views-presets`, File 25 §7.2).
...
- **Naming a specific editor engine, terminal renderer, content-search engine, version-control library, or language-structure parser as the canonical semantics** — these are replaceable implementations behind the surface's presentation units and capabilities; the canonical contract is the behavior, never the library.
...
- The **UI Shell, Layout, Presentation, and Interaction Models** (File 37) and **UI Customization, Widgets, and Theming** (File 38) specs present the projections this file fixes — its declared presentation units, the activity projection, the change and comparison projections, history, capability discovery, and view presets — however their paradigm realizes them, the windowed-desktop realization being their profile material (`ui.frontend-profile`, File 37 §1.3); presentation may vary freely, the work model cannot. The concrete editor engine, terminal renderer, and search engine are their implementation choices behind the units.
