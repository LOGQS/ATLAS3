...
- **Responsiveness** — a named latency-sensitive operation class over the source-emitted timing facts its owning layer produces (File 42 §19.2 for runtime-owned seams), decomposed by a bounded boundary locus. Its reference responsiveness envelope is a declared attribute of the registered `MetricInstrument` (§6.2) resolved from settings (§15.1), never a separate primitive: it is a performance and diagnostic target, never a correctness deadline, a liveness signal, a remediation trigger, or a source of authority, and a comparison against a responsiveness baseline (§16.1) is `DiagnosticOnly` and gates no correctness, policy, routing, evaluation, or release decision. The reference-envelope comparison and any responsiveness-baseline comparison are reported separately, and neither suppresses the other
...
The performance monitor renders the responsiveness instruments, reference envelopes, and baseline comparisons §§6.3, 12.2, and 16.1 define; it owns no envelope, baseline, persistence, or control state of its own, and activating the developer affordance adds no capture and no authority.
...
A responsiveness instrument additionally declares its bounded operation-class and boundary-locus dimensions, the source timing fact or span it projects, its timing-accuracy rule, the reference envelope it resolves from `observability.responsiveness.envelopes` (§15.1), and which responsiveness baselines (§16.1) are computed over it. Request ids, paths, uniform resource locators, command strings, process and executable identities, vendor names, user content, and arbitrary error strings are high-cardinality exemplars or forbidden values under §6.2, never default aggregate labels; a registration that would make one an aggregate dimension is rejected.
...
- the responsiveness settings: `observability.responsiveness.envelopes`, a schema-validated map keyed solely by registered `MetricInstrumentId` carrying each reference envelope with its aggregation and percentile policy, and `observability.responsiveness.baseline_policy`, controlling which responsiveness baselines are computed and the bounded source window they consume; both are `DeviceLocal` and claim no portability, and the computed baseline state is itself never a setting and never syncs
...
The current-session responsiveness baseline is a computed, bounded, `DiagnosticOnly` projection held in memory, capped per registered instrument and boundary locus so that aggregation replaces older samples rather than accumulating them, and discarded when the application session ends. A retained-device responsiveness baseline may additionally be computed as a rebuildable `DiagnosticOnly` projection over eligible `HostBoundaryTimingObserved` observations persisted through File 41's existing bounded, device-local, rotating diagnostic stream. An eligible retained observation is only a post-classification `Public` or `Sensitive` observation whose corresponding `LogRecord` passes §4.5 redaction. The timing producer emits the runtime timing fact and, when `ResponsivenessBaselinePolicyV1.retained_device` is `Enabled`, requests capture of the corresponding structured `LogRecord` through the existing `DebugLog` path under §§4.4–4.6's redaction, sensitivity, and retention rules; `retained_device: Disabled` stops that capture, and any higher File-41 or privacy capture disablement wins. An effective-`Secret` timing observation remains live-only, is never lowered merely to retain it, and makes the retained-device comparison typed unavailable. The retained-device baseline rebuilds solely from eligible retained source records. This creates no new store. If eligible capture is disabled or absent, if a required source observation is effective-`Secret`, or if the relevant diagnostic segment has rotated or been pruned, the retained-device comparison is typed unavailable. It has no separate durable record or time-series store, never syncs, and its effective retention is exactly the bounded retention of its source diagnostic facts. No responsiveness baseline is ever synced, none introduces a durable baseline record or a metric time-series store of record, and an unavailable baseline is a typed absence of comparison rather than a fabricated norm.
...
Replay never substitutes a live responsiveness baseline for a historical one and never regenerates a responsiveness comparison from present conditions. A responsiveness projection is replayed only from the source timing its instrumentation declaration names. A historical device-baseline comparison is reconstructed only from the historical source timing and applicable historical settings context that remain available; if either has been pruned or is unavailable, the comparison is typed unavailable, exactly as a pruned diagnostic-stream query is, and the current live baseline is never substituted.
...
- **Backpressure is diagnostic.** If diagnostic delivery cannot keep up, the system degrades or drops diagnostic data and reports each drop typed and counted — `EventBufferOverflow` where a bus subscriber's buffer overflowed (File 10 §5.4), and the diagnostic stream's own loss count on the health surface (§11.1) and in the shutdown outcome, and in the stream itself before the next record the stream writes, where the stream refused or lost a record (§4.6); it never blocks the observed operation to preserve diagnostic detail.
