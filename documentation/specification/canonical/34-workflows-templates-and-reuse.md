...
- The node-and-edge structure is acyclic; iteration is the `Loop` node's bounded sub-body, carried as `sub_body: SubWorkflowRef` (§3.4), never inline; an unbounded loop, a `Loop` whose `sub_body` is missing or does not resolve under any version policy, a `SubWorkflow` node whose pinned reference does not resolve, or a structural cycle is a validation error.
...
Automatic per-node retry-on-failure is File 04 §20.2.1 execution recovery, not the user-facing four-scope retry operation of §13.3. Same-run automatic retry attempts therefore create no workflow version branch merely because they retry a node; File 04's `branch_retry` shape remains the explicit exception that may create linked branch or child-run lineage. For an automation-originated workflow run, an explicit node retry policy replaces the enclosing automation's default retry policy for that node in its entirety, including an explicit maximum attempt count of `1`; absence of a node policy lets the automation default apply, and the two caps never multiply. A `Loop` node's `failure_tolerance` first decides whether an iteration failure is tolerated or becomes a failure of the `Loop` node as a whole; only a resulting `Loop`-node failure reaches that node's effective retry policy. A `SubWorkflow` node's effective policy governs failure of that invocation as the parent execution unit, while nodes inside the child workflow resolve their own effective policies. Parent and child policies are resolved independently at their respective execution-unit boundaries; they are not combined into one synthetic cap. A parent retry of a failed `SubWorkflow` invocation starts a new child invocation, so child work may repeat and consumes the configured run, stage, and stuck-detection budgets.
