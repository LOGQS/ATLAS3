...
- `backend`: serializable backend descriptor — kind plus the parameters the registry needs to resolve a live binding at registration (`ServiceMethod { service_id, method_name }`, `Wasm { module_id, entry_point }`, `Shell { program, args, cwd, env_overlay }`, `McpProxy { connector_id, remote_tool_name }` — `connector_id` is the connector's registry-assigned stable slug (`integration.connector`, File 36 §3.1), `HttpEndpoint { url_template, method, auth_ref }`, `Closure { closure_id }`)
...
Each extension resource-class entry also declares `privacy_exposure` as `Static { exposure }` or `ByScope { default_exposure, resolver_id }`, where `exposure` is the closed vocabulary `PublicOnly | UserPrivateCapable`. A `ByScope` resolver is registered, deterministic over the resolved class scope, and evaluated for each concrete touched resource. Missing metadata, a missing, unresolved, or failed resolver, or an unknown resolver result resolves fail-closed to `UserPrivateCapable`. The source-approval proposal surfaces the declaration. A settings-resolved minimum exposure may tighten the effective result by source, class, scope, or invocation overlay; it cannot weaken `UserPrivateCapable` to `PublicOnly`. This privacy classification does not replace content sensitivity; it supplies the user-private-data fact consumed by File 06.
...
- `McpServer { connector_id, server_uuid, server_version }` — sourced from an external Model Context Protocol server (connector); `connector_id` is the connector's registry-assigned stable slug (`integration.connector`, File 36 §3.1) and `server_uuid` the separate provenance identity; registered when the connector is added; unregistered on connector removal — a transient disconnect disables the entry (§16.3) rather than unregistering it (§16.5)
...
- `UserDefined { backend, scope }` — registered at runtime by the user or, with explicit user approval, by the agent through a capability-registration capability (the proposal-first runtime-mutation rule, `capability.runtime-mutation`, §16.2); backend is `Wasm` or `Shell` (`capability.backend-descriptor`, §3.12); scope is `conversation`, `workspace`, or `global`
...
- `source_instance`: the registered source-instance reference (which loaded plugin, which connected MCP server, which loaded API definition file). Every registered source instance carries a stable `source_id` matching `^[a-z0-9_-]+(?:\.[a-z0-9_-]+)+$`: the first segment names the source class of §9.1 and the remaining segments are that instance's stable slug, never a mutable display name. The identifier-bearing classes supply the slug from their declared identity — `Subsystem { subsystem_id }`, `Plugin { plugin_id }`, `McpServer { connector_id }` (the connector's registry-assigned stable slug, `integration.connector`, File 36 §3.1), and `Api { api_name }`. Registration rejects a `source_id` that does not conform to the grammar or that collides with a registered one. Settings-key families that index a source insert the complete `source_id` verbatim (`capability.settings-key-convention`, §18.2) — the grammar File 07 §18.2's worked example `surface.zone_source_override.mcp.linear_server` already prints. The canonical source-class segments are builtin, subsystem, plugin, mcp, api, and user. Builtin is the implicit instance builtin.core. A UserDefined source receives user.<slug> when its durable source identity is created; that slug is persisted and remains stable across restart and deterministic re-registration until that source identity is deleted.
...
Anchor: `capability.canonical-rule-anchors`

Load-bearing rules defined by this file carry stable anchors: `capability.chosen-model`, `capability.capability`, `capability.declaration`, `capability.display-fields`, `capability.permission-policy-fields`, `capability.execution-semantic-fields`, `capability.availability-fields`, `capability.composition-fields`, `capability.backend-descriptor`, `capability.schemas`, `capability.input-schema`, `capability.error-vocabulary`, `capability.permission-tier-declaration`, `capability.tier-resolver`, `capability.permission-floor`, `capability.touched-resources`, `capability.extension-resource-classes`, `capability.resource-expressions`, `capability.execution-semantic-metadata`, `capability.classification-mode`, `capability.replay-class`, `capability.validation-postconditions`, `capability.input-validators`, `capability.sourcing`, `capability.capability-source`, `capability.trust-source-approval-flow`, `capability.sourcing-equivalence`, `capability.platform-availability`, `capability.registered-capability`, `capability.backend-binding-lifecycle`, `capability.invocation-record`, `capability.capability-registry`, `capability.events`, `capability.identity-namespacing-versioning`, `capability.id`, `capability.family`, `capability.version`, `capability.override-resolution-conflicts`, `capability.discovery`, `capability.availability-predicate`, `capability.prerequisite-capabilities`, `capability.lifecycle`, `capability.startup-registration`, `capability.runtime-mutation`, `capability.contract-composition`, `capability.output-kinds`, `capability.adapter-capabilities`, `capability.settings`, and `capability.settings-key-convention`. Cross-references should prefer the anchor and may cite the section number secondarily. An anchor names exactly one canonical rule and is stable across spec revisions.
