...
- `idempotency_key` — required for consequential writes that may be retried; scoped by producer, boundary, operation, and source request where applicable. Duplicate keys reject duplicate durable facts or link repeated attempts to the original entry. An automation fire's `idempotency_key` IS its `fire_id` (`automation.fire-pipeline`, File 33 §9.3) — one identity, never a second carrier.
...
- `automation_id` and `fire_id` — for automation-originated entries: the firing `Automation` and the claimed fire. Canonical keys for the same reason `child_run_id` is one — usable by every automation-originated entry rather than an automation-only custom key. REQUIRED on every automation-fire lifecycle entry (claim, run binding, run creation, terminal status, skip/duplicate/queue/park records) and propagated across the fired run's entries the way `run_id` is, so the File 33 §17.1 projection reaches artifacts and tool results the `Executor` producer commits; an automation-originated entry carrying one of the pair without the other is rejected, never silently tolerated. The `producer` variant stays `Automation { trigger_id }` — producer names the producing instance; these keys carry correlation and causality.
