...
- sensitivity-aware — every entry carries a `sensitivity` tag (`Public`, `Sensitive`, `Secret`); raw `Secret` payload content does not persist. A `Secret` fact persists only in §10's redacted durable form: structural entry fields plus `safe_description`.
...
- `cross_references` — typed map naming the canonical primitives this entry depends on or refers to: `(block_id, artifact_id, version_id, lease_id, capability_id, capability_version, source_instance_id, invocation_id, run_id, task_id, intent_thread_id, conversation_id, workspace_id, automation_id, fire_id, probe_id)`; entries unused for a kind are absent rather than null-padded. `probe_id` is File 33's circuit-recovery execution identity and is mutually exclusive with `fire_id` on one automation execution.
...
- `idempotency_key` — required for consequential writes that may be retried; its value is constructed through a typed domain identity that scopes producer class, boundary, operation, and source request where applicable. On a duplicate-key hit, the original committed entry and the attempted entry are decoded through their supported schema versions and compared under the entry kind's versioned idempotency-equivalence contract. The default contract compares stable semantic fact fields — `kind`, `scope`, effective `sensitivity`, canonical cross-references, `supersedes`, and normalized typed payload — and excludes attempt identity, sequencing and timing metadata, exact producer-instance attribution, storage record version, and physical or wire encoding. A narrower contract is valid only when the guarded writer declares it canonically and fixes every omitted semantic dimension. Equal key plus equivalent semantic fact links the attempt to the original entry; equal key with a non-equivalent fact is a typed conflict. An automation fire's identity IS its `fire_id` (`automation.scheduler`, File 33 §9.3) — one fire identity, never a second carrier. A File 33 circuit probe is not a fire; its identity IS deterministic `probe_id`, and it carries no `fire_id`. No automation execution uses both.
...
- `child_run_id` — the spawned child `Run` a child-run entry names; the one cross-cutting reference every child-run form (`ChildRunSpawned`, `ChildRunStatusChanged`, `ChildRunMerged`) uses to identify its child, distinct from `parent_run_id` and usable by every child-run kind rather than a workflow-only custom key
- `route_id` — the `RouteRecord` that produced the entry's run
- `automation_id` and one automation-execution identity — canonical keys for the same reason `child_run_id` is one: they are usable by every automation-originated entry rather than an automation-only custom key, and let the File 33 §17.1 projection reach artifacts and tool results committed by `Executor`.
  - A trigger fire carries `automation_id` + `fire_id`, required on claim, fire-to-run binding, run creation, terminal status, skip/duplicate/queue/park records, and propagated across the fired run. Its origin producer is `Automation { trigger_id }`.
  - A File 33 circuit probe carries `automation_id` + `probe_id`, required on binding, run creation, terminal status, artifacts, and tool results, and propagated across the bound run. Its origin producer is `AutomationRecovery { automation_id }`.

  A trigger-fire entry carrying only one member of its pair, a probe entry carrying only one member of its pair, or an entry carrying both `fire_id` and `probe_id` is rejected. Producer names the immediate producing instance; the canonical references carry correlation and causality.
  File 04 §20.2.1 retry decisions and entries from same-unit subsequent attempts remain entries of the same automation-originated run and carry the existing `automation_id`, `fire_id`, and `run_id` correlation with their actual immediate producer. A `branch_retry` child is a distinct linked run correlated through the canonical parent/child references; it creates no new `fire_id`, `AutomationFireRunBound`, `automation_run_ordinal`, or retry-specific automation producer.
...
**Status-transition forgery guard** (per `run.termination`, File 04 §22): a run's termination as `completed` is verified against the run's latest authorized `RunCompletionContract` (`run.completion-contract`, File 04 §2.7). On a `Run` whose contract required action, termination as `completed` is rejected if no `ToolCallExecuted` or `ToolCallCompleted` entry exists in the run's scope, no `ArtifactVersionCommitted` entry exists in the run's scope, no `ModelCallCompleted` entry beyond pure-text response exists in the run's scope, and no `BlockCommitted` whose envelope carries a producing `node_id` (a committed workflow-node output block) exists in the run's scope. The forgery guard fires at the ledger boundary, not only in the executor: a hook subscriber attempting to record `RunStatusChanged { to: Completed }` against a no-evidence run produces a `LedgerCommitRejected` error and a `RunCompletionForgeryAttempted` audit-log entry; the run remains in `running` until evidence is recorded or another terminal status (`failed`, `cancelled`, `superseded`) is committed.

The action floor is also satisfied transitively for a run that delegates its work to child runs: `grounded(run)` holds when the run carries direct action evidence — one of the classes above — or when a valid `ChildRunMerged(run, child)` edge exists and `grounded(child)` holds. A `ChildRunMerged` edge is valid only when the child was spawned by that parent (a `ChildRunSpawned` naming the same `child_run_id`), the child reached a durable `Completed` status, the merge references an existing child output that was incorporated into the parent, and the spawn, merge, and produced-block entries agree on the `child_run_id` and `parent_run_id`; the satisfying evidence path is captured in the parent's completion evidence (`run.completion-contract`, File 04 §2.7), never recomputed as an unexplained boolean. The transitive recursion is bounded by the maximum child-run depth (`run.budgets-limits`, File 04 §21) — not a workflow composition depth — carries a visited set, and treats any parent/child cycle as integrity corruption. Transitive grounding satisfies only this generic anti-hollow action floor; it never satisfies a specific capability postcondition, artifact, validation, or side-effect requirement, each of which continues to match under its own rule (`qc.completion-gate`, File 39 §14).
...
- raw `Secret` payload material never persists to the durable ledger (§10). The commit boundary rejects any representation that would encode or persist raw `Secret` material. The consequential fact itself persists in §10's redacted durable form: the entry's structural fields remain intact and its persisted payload contains only `safe_description`. Redaction is selected before durable payload encoding; the validator refuses an unsafe representation rather than sanitizing already-encoded secret bytes.
...
- a ledger entry whose payload references blocks, artifacts, claims, or other primitives must reference primitives that exist at commit time (or were previously committed; tombstoned primitives remain referenced via their preserved identity per `block.hard-delete` (File 08 §6.6) and `artifact.artifact-tombstones` (File 09 §8)). Completion evidence splits against that tombstone tolerance by what the cited requirement MEANS: a requirement asserting a historical commit fact — a block or artifact version was committed in the run's scope, the status-transition guard's own evidence classes — is substantiated by the append-only ledger entry that recorded the commit, which no later hard delete rewrites, so a run that deliberately destroys its local payload after committing or publishing it (the ephemeral/privacy pattern) still completes on the history that genuinely happened; a requirement whose meaning depends on the referent's CURRENT availability — the citation leans on the payload, not on the fact of its commit — substantiates only while the referent is live (a tombstone preserves identity for reading, not the destroyed payload), so for referents on destructible substrates (blocks, and artifact version payloads, whose `version_id` is the version-block's `block_id` per `artifact.artifact-version`, File 09 §6.2) that resolution takes its liveness verdict within the committing boundary itself (`storage.transactional-guarantees`, File 20 §5.3), and a completion citing a hard-deleted referent to satisfy an availability-dependent requirement is rejected exactly like one citing a fabricated reference, while no tombstone ever invalidates a commit-fact citation
...
Every ledger entry declares its `kind` at commit. Each kind — canonical or registered extension — declares an injective `settings_key_segment` matching `[a-z0-9]+(?:_[a-z0-9]+)*`, which is the `<entry_kind>` substituted into every settings family that indexes this catalogue (`settings.setting-definition`, File 15 §3.2); `RunCreated` contributes `run_created` and `ToolCallStreamingPartial` contributes `tool_call_streaming_partial`. The canonical closed catalogue:

**Run lifecycle:**

- `RunCreated` — run instantiated; payload includes trigger kind, route id, capability families, model route, attachment kind
- `RunStatusChanged` — status transition; payload includes from-status, to-status, stop reason, partial-output retention outcome, and an optional `execution_retry_decision_entry_id`. `execution_retry_decision_entry_id` is present when a File 04 §20.2.1 decision is the terminalizing retry decision for this status transition, including a surfaced result later terminalized by its owning posture. It is absent when cancellation, independent supersession, `process_restart_orphan`, or another unrelated cause produced the transition. The referenced entry must be validated at guarded append time as earlier, same-run, and of kind `ExecutionRetryDecided`; projection-time checking alone is insufficient. A transition to `completed` additionally references the `run_completion_contract_id` (or an embedded contract snapshot), the satisfied-requirement list, and the ledger, block, artifact, or policy evidence satisfying each requirement (per `run.completion-contract`, File 04 §2.7)
...
- `PolicyDecisionMade` — approval router emitted a decision (per `policy.approval-router`, File 06 §3.4); payload includes decision (`Continue`, `Substitute`, `Block`, `RedirectSuggestion`), contributing scope, any granted lease used, applicable policy-template/projection references, contradictions detected, and `redirect_advisories` when present. An advisory is evaluation metadata: it never changes the terminal decision and never becomes an approval choice (`policy.approval-ui-surface-contract`, File 06 §13.2)
...
- `ApprovalGranted` / `ApprovalDenied` — user resolved the approval; payload includes choice, customized constraints, and the typed-confirmation strings if applicable, one per confirmation requirement (redacted per sensitivity rules)
...
- `ExecutionRetryDecided` — execution evaluated `run.execution-retry-policy` (File 04 §20.2.1); payload includes failed attempt reference, retry kind, retryability source, outcome-safety basis, normalized-argument reference, policy snapshot reference, the 1-based `retry_episode_ordinal` within the run, the 1-based `attempt_ordinal` of the failed execution attempt within that episode, the 1-based `decision_ordinal` for decisions about that failed attempt, `resolved_delay_ms` when execution selected a retry-pacing delay, and action (`retried`, `branched`, `surfaced`, `stopped`). `retry_episode_ordinal` is allocated monotonically and uniquely per `run_id` when a new logical retry episode is first recorded; replay of that episode reuses the recorded allocation. `attempt_ordinal` is unique within that episode, and `decision_ordinal` is unique within that failed attempt. The failed-attempt reference resolves to a durable attempt or failure entry of the same run, never only to an in-memory handle. `decision_ordinal` permits a previously selected retry to be superseded by a later revalidation decision for the same failed attempt when pause, resume, intervention, budget, safety, freshness, or another File 04 gate changed before the next attempt started. The consequential append is idempotency-keyed from `{ run_id, retry_episode_ordinal, attempt_ordinal, decision_ordinal }`; reuse of that key with a different payload is an integrity conflict. `resolved_delay_ms` records only File 04 execution pacing: provider `retry_after_ms` and connector-suggested timing remain consumed inside their owning transport layers and are never copied into this field. `action: retried` selects the automatic execution-retry path inside the current execution lineage; it is not File 04 §19.1's `RetryAttempted`
...
- `BlockSensitivityAutoEscalated` — the block commit validator determined that effective sensitivity exceeded the producer-declared sensitivity and raised it before persistence (per `block.inheritance-through-composition`, File 08 §9.3); payload includes `block_id`, `declared_sensitivity`, and `effective_sensitivity`. It is `lossless_consequential`, commits in the same semantic boundary as the affected block and its `BlockCommitted` fact, and mirrors only after that boundary commits.
...
- `PendingOpUndone`, `PendingOpsDiscarded`, `ConversationForked`, `VersionLabelled`, `VersionUnlabelled`, `VersionBookmarked`, `VersionUnbookmarked`, `MaterializedViewRebuilt`, `MaterializedViewIntegrityViolated`, `VersionTombstoned`, `VersionRangeCompacted`, `VersionPayloadHardDeleted`, `RetentionPolicyApplied`, `PendingOpsInconsistencyDetected` — the remaining version-graph kinds, catalogued here by name with `version.events` (File 11 §21.1) owning each exact payload field set (where a bullet's prose differs, §21.1 governs; the same ownership split as the artifact events below); first-party canonical kinds, never routed through the `Custom` extension mechanism
...
- `ChildRunSpawned` — child run created (per `run.child-runs-multi-agent-work`, File 04 §16); references the child through the canonical `child_run_id` cross-reference (§3.6); payload includes parent run id, child run id, declared output contract, isolation primitive, and `iteration_index` — a zero-based index that is `Some` for a `Loop` iteration child (mandatory) and `None` for an ordinary child (mandatory), less than the loop's `max_iterations` when present and unique within `(parent_run_id, node_id)`. Legacy v1 `ChildRunSpawned` records that predate this field decode as `None`. `iteration_index` identifies the loop position only; it never encodes retry-attempt identity, which rides the run's own retry/branch lineage (`run.retry-reroute-branch`, File 04 §19)
- `ChildRunStatusChanged` — mirrors `RunStatusChanged` for child runs; references the child through the canonical `child_run_id` cross-reference (§3.6)
- `ChildRunMerged` — parent run incorporated child output (per `run.merge`, File 04 §16.4); references the merged child through the canonical `child_run_id` cross-reference (§3.6); payload includes the merge mode (summary / artifact / patch / evidence-set / validation-report / proposed-task-update / proposed-workflow-step) and the incorporated child-output block reference
...
- `RecoveryStrategyApplied` — a recovery strategy fired (per `run.recovery`, File 04 §20.2); payload includes the strategy (`retry_same_unit`, `expose_to_model`, `switch_model_profile`, `switch_capability_implementation`, `narrow_capability_scope`, `revoke_and_narrow_lease`, `request_user_clarification`, `branch_strategy`, `restore_or_rollback`, `stop_with_typed_failure`) and an optional `execution_retry_decision_entry_id` when this strategy application carries out a File 04 §20.2.1 decision. `ExecutionRetryDecided` remains the authority for the evaluated retryability, retry shape, attempt coordinates, policy basis, and resulting action; `RecoveryStrategyApplied` records the concrete recovery operation that actually fired and neither substitutes for nor increments the retry decision. Each `RecoveryStrategyApplied` entry carrying this reference also carries a 1-based `recovery_application_ordinal` unique within that decision. Its consequential append is keyed from `{ execution_retry_decision_entry_id, recovery_application_ordinal }`; reuse with divergent facts is a typed conflict. One retry decision has no recovery-application companion when execution is interrupted before application, and more than one when its recovery path performs more than one typed strategy
...
- `PolicyDenyOverridden` — successful typed-confirmation override of one or more policy denies; payload carries `overridden_elements`, a canonical-order duplicate-free list of `PermissionFloor { capability_id }` or `PolicyRule { template_ref }`, together with the satisfied confirmation-requirement references (per `policy.denied-carve-out`, File 06 §7.4)
...
Every event is an `AppEvent` variant. The closed canonical catalogue is the same set as the `LedgerEntryKind` catalogue (§4), with the addition of transient-coordination kinds that do not commit to the ledger. A kind shared with §4 uses that catalogue's `settings_key_segment`; each transient-only kind below, and each registered custom event kind, declares its own injective token matching `[a-z0-9]+(?:_[a-z0-9]+)*`, injective across the union of both catalogues, which is the `<event_kind>` substituted into every settings family that indexes app-event kinds (`settings.setting-definition`, File 15 §3.2); `MessageChunk` contributes `message_chunk`. The transient-coordination kinds:
...
- **Historical redelivery**: resolving an idempotent retry to an already-committed ledger entry does not directly reinsert that historical event into the ordinary live stream. Redelivery of a committed historical occurrence is performed through the owning checkpointed reconciliation path in monotonically increasing `sequence` order within its `sequence_scope`, retaining the source entry's original `event_id` and `sequence`. A retry may recover the original envelope for reconciliation, but it does not mint or directly publish a second live occurrence.
...
`RedirectSuggestion`'s terminality here is the hook-decision layer's rule and is unconditional: once a blocking hook has returned one, the current proposal is over. Whether a policy-internal template validator's redirect *verdict* becomes such a decision at all is the approval router's own rule and is mode-gated (`policy.approval-policy-templates`, File 06 §12.3). This section governs the emitted decision, never the verdict that may or may not produce it.
...
Per-error-class retry behavior is configurable (per `run.hook-integration`, File 04 §23.3 and `policy.approval-router` (File 06 §3.5)): a hook that fails because of a known transient cause (provider rate limit, sandbox temporary unavailability, recoverable transport failure) may be configured to retry within its safety guard rather than fail immediately. The retry classification is part of the hook's declaration. That classification is drawn from one closed typed error-class vocabulary; each member declares an injective `settings_key_segment` per `settings.setting-definition` (File 15 §3.2), and those segments are the `<hook_error_class>` component of §15.1's hook retry family. A fatal classification is never retryable, and cancellation is not a member of the vocabulary at all — a cancelled call carries no error class (`provider.provider-error-classification`, File 17 §10) and is a stop, not a retry direction. That vocabulary is `HookErrorClass`, the closed set `TransientInfrastructure`, `RateLimited`, `TemporarilyUnavailable`, and `Fatal`; it is hook-local because a hook handler is not a provider call and its transient causes include sandbox and transport conditions a provider taxonomy does not name. HookErrorClass is the stable hook-configuration identity only. A concrete hook failure produces an ErrorClassification (File 17 §10.2), and that typed classification—not HookErrorClass—remains the only retry-direction signal consumed by retry execution.
...
- `hooks.timeout_default_ms` per declared hook_category (the catalogue of §7.1) as a safety guard, not a correctness condition
...
- `hooks.retry.<hook_category>.<hook_error_class>` — retry override per hook category and hook error class, where `<hook_category>` is a member of the `hook_category` catalogue §7.1 declares and `<hook_error_class>` is the `settings_key_segment` of a member of the retry-classification vocabulary §7.5 declares (e.g., a transient network failure may retry once before fail-closed)
...
- `ledger.retention.<entry_kind>` per kind, where `<entry_kind>` is the kind's `settings_key_segment` (§4.1) — for example `ledger.retention.tool_call_streaming_partial` for `ToolCallStreamingPartial`, which takes shorter retention
...
- every `PolicyDenyOverridden` (the typed-confirmation override path through a `Denied` floor or a policy rule's deny)
