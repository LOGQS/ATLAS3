...
- a ledger entry whose payload references blocks, artifacts, claims, or other primitives must reference primitives that exist at commit time (or were previously committed; tombstoned primitives remain referenced via their preserved identity per `block.hard-delete` (File 08 §6.6) and `artifact.artifact-tombstones` (File 09 §8)). Completion evidence is the carve-out from that tombstone tolerance: a reference cited to satisfy a completion requirement (the status-transition guard above; `run.completion-contract`, File 04 §2.7) substantiates its requirement only while the referent is live — a tombstone preserves identity for reading, not the destroyed payload the citation leans on — so for referents on destructible substrates (blocks, and artifact version payloads, whose `version_id` is the version-block's `block_id` per `artifact.artifact-version`, File 09 §6.2) the resolution takes its liveness verdict within the committing boundary itself (`storage.transactional-guarantees`, File 20 §5.3), and a completion citing a hard-deleted referent is rejected exactly like one citing a fabricated reference
