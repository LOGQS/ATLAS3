...
A chord in progress (a multi-key keybinding sequence not yet complete, §7.4) is a transient rail-session state, not a `RailResolution` outcome: it yields no outcome until the sequence completes, is cancelled, receives incompatible input, loses its active context, or reaches its safety guard.
...
3. **Argument and content reference expansion.** If the gesture's arguments or content carry references, mentions, slash macros, or pasted tokens, the rail expands them (`PreDispatchTransform`) before the schema check, so schema resolution sees the resolved arguments; each expansion is recorded in the ledger.
...
A keybinding context is a named scope within which a binding is active (for example: a global context, the conversation input, an editor, a modal dialog, the command palette, a search overlay, a vim mode, a permission dialog, an elicitation panel). Contexts compose as a **stack** derived from live presentation and focus state, per renderer root (`ui.shell`, File 37 §11.2): a surface, panel, or dialog that owns a context contributes it to its own root's stack while active, so each input-capable root has its own active context set with that root's most-recently-active context on top. The keybinding context is informed by the live `UiMode` and the active `SurfaceState`/`PanelState` (`world.surface-state`, File 18 §5) — opening the command palette contributes the palette context as it sets the `CommandPalette` `UiMode`, a modal contributes a modal context — but the context stack is the keymap's resolution input, not a field File 18 owns.

### 7.4 The Resolver

Keybinding resolution is a deterministic, pure function of the parsed input, the active context stack (top first), the registered bindings, and any pending chord. It walks the active contexts top-down and returns the first matching binding's outcome; the first binding that matches wins, so a context higher on the stack overrides a binding in a lower context (a modal's Escape binding wins over the global Escape). A multi-key chord extends non-durable pending-chord rail state and returns a chord-in-progress result until the sequence completes, is explicitly cancelled, receives incompatible input, loses its active context, or reaches a configurable safety guard. The resolver holds no state itself; the rail session owns pending-chord state, and there is one rail session per renderer root, so a chord begun in one root never completes on a keystroke in another (`ui.shell`, File 37 §11.2). A resolved capability or steering action is dispatched through §4. This single resolver supersedes scattered per-component key listeners; conflicts are resolved by context-stack priority, not by registration order across components.

### 7.5 Conflict Resolution, Unbind, and Platform Reality

- Two collision cases are distinct. A **declaration collision** — two capabilities declaring the same `default_shortcut` — is detected where the shortcut lens is composed (`surface.presentation-in-user-facing-surfaces`, File 07 §12.3) and surfaces before any user binding; because a `default_shortcut` is a default and never a contract (§7.2), the declaration collision is resolved by user rebinding or a declared priority, never by silently letting one declaration win. A **binding collision** — two active bindings mapping the same chord in the same context, whether from defaults, user rebindings, or a plugin — is the keymap conflict this section resolves: registration detects it and emits `ShortcutConflict` (`surface.surface-relevant-events`, File 07 §13), the user resolves it through a declared priority order in settings, and the Command rail may present a disambiguation choice (§6.2).
- Until the user declares that priority, a same-context binding collision has no silent winner: the chord surfaces the `ShortcutConflict` and resolves through the §6.2 disambiguation rather than firing an arbitrarily chosen binding, and registration order never breaks the tie (§7.4). A user-bound shortcut overrides a capability's `default_shortcut`.
...
- The Voice rail session proceeds: optional wake-word activation, audio capture, voice-activity-gated transcription, registry-derived intent resolution, optional confirmation, execution through §4, and optional spoken output. Audio capture, voice-activity detection, wake-word detection, chunking, and transcription are owned by `perception.sensor` (File 19 §4.3) and the transcription capability (`perception.capability-surface`, File 19 §14); the rail consumes them and owns the session that turns a transcript into a `RailResolution`. Spoken output is a voice-rail built-in: the rail contributes and owns the spoken-output capability that renders a result as speech, dispatching synthesis through File 17's deferred TTS adapter family (`provider.provider-layer`, File 17 §1) and owning its playback, output-device, and caching settings (§18.1); it does not re-own the synthesis engine or the provider adapter File 17 defines.
- **Intent resolution is registry-derived.** A transcript resolves to a voice-invokable capability in the current available-capability list (`world.state-aware-capability-availability`, File 18 §9, filtered to the `Voice` lens, `voice-invokable` tag) using the live `SurfaceState` of the session's invoking presentation context — its graphical renderer root, or its typed noninteractive context for a companion or external-entry form (`world.surface-state`, File 18 §5.1) — as context: "click the second tab" resolves against that root's panel list, "format this" against that root's current `Selection`. There is no fixed spoken-intent enumeration: the deletion of a hardcoded `SpokenIntent` set in favor of registry-derived intents (`unit15-ux-distribution-files-glossary.md` D15.UX.4) is canonical, because a fixed enum would gatekeep against `core.extension-planes` (File 01 §6.14) and duplicate the registry. When confidence is below the configured threshold, the rail falls back to a disambiguation elicitation (§13) — for the command palette, the top match highlighted — rather than guessing.
